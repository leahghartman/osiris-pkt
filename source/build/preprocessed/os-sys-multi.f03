# 1 "system/os-sys-multi.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "system/os-sys-multi.f03" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! system module
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!---------------------------------------------------------------------------------------------------
  ...
  if ( .not. options%test ) then
    ...
    if (mpi_node()==0) print *,"Osiris running on host ",trim(hostname())
    ...
    if ( trim(options%work_dir) /= '' ) then
      ...
      if (ierr/=0) then
        print *, "[",mpi_node(),"] ","(*error*) ", trim(strerror(ierr))
        print *, "[",mpi_node(),"] ","(*error*) Unable to change directory to ",trim(options%work_dir)
        call abort_program()
      endif
    endif
    ...
    if (ierr /= 0) then
      print *, "[",mpi_node(),"] ","(*error*) Unable to get current working directory:"
      print *, "[",mpi_node(),"] ","(*error*) ", trim(strerror(ierr))
      print *, "[",mpi_node(),"] ","(*error*) Aborting..."
      call abort_program()
    endif

    if (mpi_node()==0) print *,"Current working directory = ", trim(osiris_wdir)

    if (mpi_node()==0) print *,'Timer resolution is ', timer_resolution(), ' seconds'
# 427 "system/os-sys-multi.f03"
  endif
