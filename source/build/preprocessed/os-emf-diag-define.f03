# 1 "emf/diagnostics/os-emf-diag-define.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "emf/diagnostics/os-emf-diag-define.f03" 2
# 1 "./os-config.h" 1
...
# 2 "emf/diagnostics/os-emf-diag-define.f03" 2
...
# 3 "emf/diagnostics/os-emf-diag-define.f03" 2
...
module m_emf_diag
  ...
  integer, public :: diag_emf_ev = 0, diag_emf_boost_ev, diag_emf_boost_extract_ev
...
character(len=8), dimension(6), parameter :: p_boosted_report_quants = &
   (/ 'e1_boost', 'e2_boost', 'e3_boost', 'b1_boost', 'b2_boost', 'b3_boost' /)
...
subroutine restart_write_diag_emf( this, restart_handle )
  ...
  if( this%if_use_boosted_diag ) then
    call this % boosted_diag % restart_write( restart_handle )
  endif
...
subroutine restart_read_diag_emf( this, restart_handle )
  ...
  if( this%if_use_boosted_diag ) then
    call this % boosted_diag % restart_read( restart_handle )
  endif
...
subroutine init_diag_emf( this, no_ext_fld, part_fld_alloc, interpolation )
  ...
  case default
    interp_offset = 0.0_p_double
    write(err_buf__,*) 'Interpolation value not supported';call err__("emf/diagnostics/os-emf-diag-define.f03",169)
  ...
  do
    ...
    select case ( report%quant )
      ...
      case ( p_ext_e1, p_ext_e2, p_ext_e3 )
        if ( no_ext_fld ) then
if (mpi_node()==0) print *,'(*warning*) External field diagnostic requested, but external fields not in use'
          report%ndump = 0
        else
      ...
      case ( p_part_b1, p_part_b2, p_part_b3 )
        if ( .not. part_fld_alloc ) then
          if (mpi_node()==0) print *,'(*warning*) Particle fields diagnostic requested, but external/smoothed'
          if (mpi_node()==0) print *,'            fields are not in use.'
          report%ndump = 0
        else
...
!-----------------------------------------------------------------------------------------
  ...
  ! this initializes this%if_use_boosted_diag to .false. if there is no boosted_diag namelist.
  ! the if_use_boosted_raw parameter is unused here
  call this % boosted_diag % read_input( input_file, this%boosted_report_quants, gamma, &
                                         p_boosted_fields, this%if_use_boosted_diag, &
                                         if_use_boosted_raw, p_x_dim )
