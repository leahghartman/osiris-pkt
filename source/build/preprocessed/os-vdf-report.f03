# 1 "vdf/os-vdf-report.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-vdf-report.f03" 2
...
!---------------------------------------------------------------------------------------------------
  ...
  call alloc(new_item,"vdf/os-vdf-report.f03",282)
  new_item%type = type
  new_item%name = name
...
!---------------------------------------------------------------------------------------------------
   ...
   if ( .not. associated(report) ) then
     call alloc(report,"vdf/os-vdf-report.f03",324)
     report%quant = quant
     ...
     do
       ...
       if ( .not. associated(rep%next) ) then
         call alloc(rep%next,"vdf/os-vdf-report.f03",334)
...
subroutine cleanup_report( report )
  ...
  do
    ...
    do
      ...
      call freemem(report%list,"vdf/os-vdf-report.f03",536)
      report%list => tmpB
    enddo
    ...
    tmpA => report%next
    call freemem(report,"vdf/os-vdf-report.f03",543)
    report => tmpA
  enddo
  ...
  if ( root( no_co ) ) then
    ...
    do node = 2, no_num( no_co )
       ...
       if ( LineoutHasLocalData(gipos, direction, vdf%x_dim_, nodeIdx ) ) then
          ...
          call alloc(segment, (/ count /),"vdf/os-vdf-report.f03",948)
          ...
          call freemem(segment,"vdf/os-vdf-report.f03",955)
    ...
    if ( LineoutHasLocalData(gipos, direction, vdf%x_dim_, nodeIdx ) ) then
       ...
       call alloc(segment, (/count/),"vdf/os-vdf-report.f03",968)
       ...
       call freemem(segment,"vdf/os-vdf-report.f03",995)
...
!---------------------------------------------------------------------------------------------------
  ...
  lineData => null()
  if ( root( no_co ) ) then
    call alloc(lineData, (/ grid%g_nx( direction ) /),"vdf/os-vdf-report.f03",1051)
  endif
  ...
  if ( root( no_co ) ) then
     ...
     ! free memory
     call freemem(lineData,"vdf/os-vdf-report.f03",1086)
...
!---------------------------------------------------------------------------------------------------
  ...
  if ( root( no_co ) ) then
    ...
    call alloc(sliceData, (/ grid%g_nx( i1 ), grid%g_nx( i2 ) /),"vdf/os-vdf-report.f03",1154)
  ...
  if ( root( no_co ) ) then
     ...
     ! free memory
     call freemem(sliceData,"vdf/os-vdf-report.f03",1197)
  ...
  if ( root( no_co ) ) then
    ...
    do node = 2, no_num( no_co )
       ...
       if ( SliceHasLocalData(gipos, direction, nodeIdx ) ) then
          ...
          call alloc(tile, tileSize,"vdf/os-vdf-report.f03",1347)
          ...
          call freemem(tile,"vdf/os-vdf-report.f03",1357)
    ...
    if ( SliceHasLocalData(gipos, direction, nodeIdx ) ) then
       ...
       select case (direction)
         case(1)
           ...
           call alloc(tile, tileSize,"vdf/os-vdf-report.f03",1372)
         ...
         case(2)
           ...
           tileSize(2) = vdf%nx_(3)
           call alloc(tile, tileSize,"vdf/os-vdf-report.f03",1380)
         ...
         case(3)
           ...
           call alloc(tile, tileSize,"vdf/os-vdf-report.f03",1388)
       ...
       call freemem(tile,"vdf/os-vdf-report.f03",1398)
...
subroutine alloc_vdf_report( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_vdf_report ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_vdf_report" , &
                        bsize, &
                        file, line )

end subroutine alloc_vdf_report
# 364 "./memory/mem-template.h"
subroutine freemem_vdf_report( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   call log_deallocation( addr, "t_vdf_report", &
                        bsize, file, line )
...
end subroutine freemem_vdf_report
# 1452 "vdf/os-vdf-report.f03" 2
...
! Generated automatically by memory.py on 2019-03-06 18:36:58
# 24 "./memory/mem-template.h"
subroutine alloc_vdf_report_item( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_vdf_report_item ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_vdf_report_item" , &
                        bsize, &
                        file, line )

end subroutine alloc_vdf_report_item
# 364 "./memory/mem-template.h"
subroutine freemem_vdf_report_item( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   ! Decrease global memory counter
   call log_deallocation( addr, "t_vdf_report_item", &
                        bsize, file, line )
...
end subroutine freemem_vdf_report_item
# 1461 "vdf/os-vdf-report.f03" 2
