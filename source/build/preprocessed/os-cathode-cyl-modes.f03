# 1 "cyl_modes/os-cathode-cyl-modes.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "cyl_modes/os-cathode-cyl-modes.f03" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! cathode class
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 8 "cyl_modes/os-cathode-cyl-modes.f03" 2
...
# 9 "cyl_modes/os-cathode-cyl-modes.f03" 2
# 1 "./memory/memory.h" 1
...
# 19 "./memory/memory.h"
use memory
# 10 "cyl_modes/os-cathode-cyl-modes.f03" 2
...
!---------------------------------------------------
  ...
  call alloc(this%ppos_cell, (/ 1, lnum_par_x(p_r_dim) /),"cyl_modes/os-cathode-cyl-modes.f03",95)
  ...
  if (restart) then
    ...
    if (.not. associated(this%species)) then
      write(err_buf__,*) 'species is not associated in setup_cathode';call err__("cyl_modes/os-cathode-cyl-modes.f03",112)
...
subroutine deposit_current_cathode_cyl_modes(this, ip0, ip1, jay_cyl_m, gdt)
  ...
  class is (t_species_cyl_modes)
    spec_cyl => obj
  class default
    write(err_buf__,*) 'inject_cyl_modes was called with a species of the wrong class';call err__("cyl_modes/os-cathode-cyl-modes.f03",216)
    call abort_program(p_err_invalid)
  end select
  ...
  do ip = ip0, ip1, p_cache_size
    ...
    do mode = 1, spec_cyl%n_cyl_modes
      ...
      case default
        write(err_buf__,*) 'Not implemented yet';call err__("cyl_modes/os-cathode-cyl-modes.f03",337)
...
subroutine inject_cathode_cyl_modes_lowroundoff(this, jay_cyl_m, jay_tmp_cyl_m, t, gdt, &
  ...
  class is (t_species_cyl_modes)
    spec_cyl => obj
  class default
    write(err_buf__,*) 'inject_cyl_modes was called with a species of the wrong class';call err__("cyl_modes/os-cathode-cyl-modes.f03",435)
    call abort_program(p_err_invalid)
  end select
...
subroutine create_particles_cathode_cyl_modes(this, t, dt, coords)
  ...
  class is (t_species_cyl_modes)
    spec_cyl => obj
  class default
    write(err_buf__,*) 'inject_cyl_modes was called with a species of the wrong class';call err__("cyl_modes/os-cathode-cyl-modes.f03",544)
    call abort_program(p_err_invalid)
  end select
