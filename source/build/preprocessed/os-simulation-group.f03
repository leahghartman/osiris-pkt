# 1 "tiles/os-simulation-group.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "tiles/os-simulation-group.f03" 2
! #define DEBUG_FILE
...
# 6 "tiles/os-simulation-group.f03" 2
...
# 7 "tiles/os-simulation-group.f03" 2
# 1 "./memory/memory.h" 1
...
# 19 "./memory/memory.h"
use memory
# 8 "tiles/os-simulation-group.f03" 2
...
subroutine iter_sim_group( sim )
  ...
  if ( sim%tiles(sim%til_id(1))%t%emf%if_charge_cons( sim%tiles(sim%til_id(1))%t%tstep ) ) then
    ...
    write(err_buf__,*) "Charge conservation diagnostic is temporarily disabled";call err__("tiles/os-simulation-group.f03",523)
    call abort_program()
...
# 655 "tiles/os-simulation-group.f03"
...
! if (mpi_node()==0) print *,""
! if (mpi_node()==0) print *," Verifying dynamic load balance (A)..."
...
! if (mpi_node()==0) print *,""
! if (mpi_node()==0) print *," Verifying dynamic load balance (B)..."
...
! call wait_for_all( this%no_co )
! if (mpi_node()==0) print *," No errors found!"
! if (mpi_node()==0) print *,""
...
subroutine reshape_group( this )
  ...
  if ( all(no_co%ti_co%g_til_aid_min_max_old==no_co%ti_co%g_til_aid_min_max) ) then
    if (mpi_node()==0) print *,' Skipping dynamic load balance'
    return
  endif
  ...
  case (p_topology_viktor)
    ...
    select case ( p_x_dim )
    case (2)
      call alloc(diff2, (/ no_co%ti_co%g_til(1), no_co%ti_co%g_til(2) /),"tiles/os-simulation-group.f03",1087)
      ! compute diff2
      ...
      call freemem(diff2,"tiles/os-simulation-group.f03",1094)
    case (3)
      call alloc(diff3, (/ no_co%ti_co%g_til(1), no_co%ti_co%g_til(2), no_co%ti_co%g_til(3) /),"tiles/os-simulation-group.f03",1096)
      ! compute diff3
      ...
      call freemem(diff3,"tiles/os-simulation-group.f03",1103)
    end select
  case default
    call alloc(diff1, (/ product(no_co%ti_co%g_til) /),"tiles/os-simulation-group.f03",1106)
    ...
    call freemem(diff1,"tiles/os-simulation-group.f03",1114)
  end select
...
subroutine prep_dlb_v( this, diff, difff )
  ...
  select case ( p_x_dim )
    case (2)
      ! call alloc(diff, lb, ub,"tiles/os-simulation-group.f03",1162)
      ...
      ! endif
    case (3)
      ! call alloc(difff, lb, ub,"tiles/os-simulation-group.f03",1190)
  ...
  select type(no_co => this%no_co); class is(t_node_conf_tiles)
    ...
    if ( associated(no_co%omp_patt%heavy_tils) ) then
      ...
      if ( size(no_co%omp_patt%heavy_tils) < this%til_id(3) ) then
        call freemem(no_co%omp_patt%heavy_tils,"tiles/os-simulation-group.f03",1365)
        call freemem(no_co%omp_patt%light_tils,"tiles/os-simulation-group.f03",1366)
        call alloc(no_co%omp_patt%heavy_tils, (/this%til_id(3)/),"tiles/os-simulation-group.f03",1367)
        call alloc(no_co%omp_patt%light_tils, (/this%til_id(3)/),"tiles/os-simulation-group.f03",1368)
      endif
    else
      ! allocate if not allocated
      call alloc(no_co%omp_patt%heavy_tils, (/this%til_id(3)/),"tiles/os-simulation-group.f03",1372)
      call alloc(no_co%omp_patt%light_tils, (/this%til_id(3)/),"tiles/os-simulation-group.f03",1373)
    endif
...
subroutine init_sim_group( sim, grid_gc_min )
  ...
  if (mpi_node()==0) print *," Setting up group no_co"
  call sim % no_co % init()
  ...
  if (mpi_node()==0) print *," Setting up group grid"
  ...
  if ( .not. restart ) then
    ...
    if ( sim % grid % needs_int_load( 0 ) ) then
      if (mpi_node()==0) print *,' - generating initial integral tile load...'
    ...
    if (mpi_node()==0) print *,' - setting group grid boundaries...'
  ...
  if (mpi_node()==0) print *," - setting up group grid boundaries for diagnostics..."
  ...
  if (mpi_node()==0) print *," Setting up tiles no_co"
  do i = sim%til_id(1), sim%til_id(2)
  ...
  if (mpi_node()==0) print *," Setting up tiles grid"
  do i = sim%til_id(1), sim%til_id(2)
  ...
  if (mpi_node()==0) print *,' - setting tile grid boundaries...'
  ...
  if (mpi_node()==0) print *," Setting up zpulses"
  do i = sim%til_id(1), sim%til_id(2)
  ...
  if (mpi_node()==0) print *," Setting up antenna array"
  do i = sim%til_id(1), sim%til_id(2)
    call sim%tiles(i)% t % antenna_array % init( restart, restart_handle )
  enddo


  if (mpi_node()==0) print *," Setting up group diagnostics utilities"
  ...
  if (mpi_node()==0) print *," Setting up tile boundary information"
