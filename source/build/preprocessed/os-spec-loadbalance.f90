# 1 "spec/os-spec-loadbalance.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/os-spec-loadbalance.f90" 2
...
# 5 "spec/os-spec-loadbalance.f90" 2
...
# 6 "spec/os-spec-loadbalance.f90" 2
...
# 19 "./memory/memory.h"
use memory
# 10 "spec/os-spec-loadbalance.f90" 2
...
public :: add_density_load, deposit_cell_load, init_send_idx
...
!-------------------------------------------------------------------------------
  ...
  call alloc(ppos, (/ p_x_dim, npar_cell /),"spec/os-spec-loadbalance.f90",165)
  call alloc(ppos_cell, (/ p_x_dim, npar_cell /),"spec/os-spec-loadbalance.f90",166)
  call alloc(pcharge, (/ npar_cell /),"spec/os-spec-loadbalance.f90",167)
  ...
  call freemem(ppos,"spec/os-spec-loadbalance.f90",295)
  call freemem(ppos_cell,"spec/os-spec-loadbalance.f90",296)
  call freemem(pcharge,"spec/os-spec-loadbalance.f90",297)
 ...
 case(3)
  ...
  subroutine add_particle_index( list, i )
     ...
     if ( list%nidx + 1 > list%buf_size ) then
        ...
        call alloc(tmp_idx, (/ list%buf_size /),"spec/os-spec-loadbalance.f90",476)
        if ( list%nidx > 0 ) call memcpy( tmp_idx, list%idx, list%nidx )
        call freemem(list%idx,"spec/os-spec-loadbalance.f90",478)
        list%idx => tmp_idx
     endif
...
subroutine pack_particles( species, local_idx )
  ...
  species%num_par = k
# 555 "spec/os-spec-loadbalance.f90"
...
# 611 "spec/os-spec-loadbalance.f90"
  ...
  if ( msg_patt%n_recv > 0 ) then
    ...
    call alloc(recv, (/msg_patt%n_recv/),"spec/os-spec-loadbalance.f90",618)
  ...
  if ( msg_patt%n_send > 0 ) then
    ...
    call alloc(send, (/msg_patt%n_send/),"spec/os-spec-loadbalance.f90",647)
    call alloc(send_idx, (/msg_patt%n_send/),"spec/os-spec-loadbalance.f90",648)
    ...
    do j = 1, msg_patt%n_send
      call freemem(send_idx(j)%idx,"spec/os-spec-loadbalance.f90",673)
    enddo
    call freemem(send_idx,"spec/os-spec-loadbalance.f90",675)
  ...
  if ( local_idx%buf_size > 0 ) call freemem(local_idx%idx,"spec/os-spec-loadbalance.f90",685)
  local_idx%start = 1
  local_idx%nidx = 0
  local_idx%buf_size = 0
...
! Wait for each stage 1 receive message to complete and post 2nd receive
  ...
  if ( msg_patt%n_recv > 0 ) call freemem(recv,"spec/os-spec-loadbalance.f90",726)

  ! Wait for any remaining send messages and cleanup message buffers
  if ( msg_patt%n_send > 0 ) then
    call wait_spec_msg( send )
    call cleanup( send )
    call freemem(send,"spec/os-spec-loadbalance.f90",732)
  endif
  ...
  do
    ...
    report => report%next
  enddo
# 778 "spec/os-spec-loadbalance.f90"
...
end subroutine add_particle_load_spec
!-----------------------------------------------------------------------------------------
