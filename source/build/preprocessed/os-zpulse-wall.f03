# 1 "zpulse/os-zpulse-wall.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "zpulse/os-zpulse-wall.f03" 2
# 1 "./os-config.h" 1
...
subroutine read_input_zpulse_wall( this, input_file, g_space, bnd_con, periodic, grid, sim_options )
  ...
  if (disp_out(input_file)) then
    if (mpi_node()==0) print *," - Reading zpulse_wall configuration..."
  endif
...
!-----------------------------------------------------------------------------------------
  ...
  do ip = 0, b%nx_(iperp)+2
    ...
    b12 = lenv * rdtdx * this % per_envelope_2d( this%inj_time, z, r ) * sin_pol
    b3 = lenv * rdtdx * this % per_envelope_2d( this%inj_time, z, r_2 ) * cos_pol
  ...
  if ( my_ngp( no_co, this%direction) == inj_node ) then
    ...
    do ip2 = 0, b%nx_(iperp2)+2
      do ip1 = 0, b%nx_(iperp1)+2
        ...
        b0 = lenv * rdtdx * this % per_envelope_3d( this%inj_time, z, r1, r2_2 ) * sin_pol
        b90 = lenv * rdtdx * this % per_envelope_3d( this%inj_time, z, r1_2, r2 ) * cos_pol
