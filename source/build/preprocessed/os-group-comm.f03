# 1 "tiles/os-group-comm.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "tiles/os-group-comm.f03" 2
module m_group_comm
...
subroutine irecv_til_size( sim, patt, bsize )
  ...
  call alloc(bsize, (/patt%n_recv/),"tiles/os-group-comm.f03",143)
  ...
  do i = 1, patt%n_recv
    ...
    if (ierr/=0) then
      write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",157)
...
subroutine isend_til_size( sim, patt, bsize)
  ...
  call alloc(bsize, (/patt%n_send/),"tiles/os-group-comm.f03",182)
  ...
  do i = 1, patt%n_send
    ...
    if (bsize(i) < 0) then
      print *, "[",mpi_node(),"] ",'Invalid value for tile buffer size, possible integer overflow.'
      print *, "[",mpi_node(),"] ",'Message to node ',patt%send(i)%node-1
    ...
    if (ierr/=0) then
      write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",223)
      call abort_program( p_err_mpi )
    endif
...
subroutine irecv_til_msg_dlb( patt, bsize, tiles )
  ...
  do i = 1, patt%n_recv
    ...
    call alloc(patt%recv(i)%buffer, (/ bsize(i) /),"tiles/os-group-comm.f03",252)
    ...
    if (ierr/=0) then
      write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",261)
      call abort_program( p_err_mpi )
    endif
...
subroutine isend_til_msg_dlb( no_co, patt, bsize, tiles )
  ...
  call alloc(preface, (/ num_spec+1 /),"tiles/os-group-comm.f03",295)
  ...
  do i = 1, patt%n_send
    ...
    call alloc(patt%send(i)%buffer, (/ bsize(i) /),"tiles/os-group-comm.f03",303)
  ...
  call freemem(bsize,"tiles/os-group-comm.f03",409)
  call freemem(preface,"tiles/os-group-comm.f03",410)
  ...
  select type(grid); class is (t_grid_group)
    ...
    if ( group_vdf%f_dim_ == 1 .and. fc_msg(1,1) > 1 ) then
      ...
      if ( n_sub > 1 .or. fc_msg(1,1) /= fc_msg(2,1) ) then
        write(err_buf__,*) 'Problem passing single field component into group_vdf';call err__("tiles/os-group-comm.f03",470)
      endif
      fc_off = 1 - fc_msg(1,1)
    else
      fc_off = 0
    ...
    do i = 1, n_recv
      ...
      call alloc(recv(i)%sub_type, (/ n_sub * n_tils /),"tiles/os-group-comm.f03",483)
...
subroutine isend_til_msg_diag( vdf_arr, til1_id, no_co, send, n_send, n_sub, fc_msg, &
  ...
  case(3)
    call isend_til_msg_diag_3d( vdf_arr, til1_id, no_co, send, n_send, n_sub, fc_msg, &
      n_fc, gc_num )
  case default
    write(err_buf__,*) 'Invalid value for vdf % x_dim_';call err__("tiles/os-group-comm.f03",576)
    call abort_program( p_err_invalid )
  end select
...
subroutine isend_til_msg_diag_1d( vdf_arr, til1_id, no_co, send, n_send, n_sub, fc_msg, &
  ...
  do i = 1, n_send
    ...
    call alloc(send(i)%vdf_diag_buffer, (/bsize/),"tiles/os-group-comm.f03",639)
    buffer => send(i)%vdf_diag_buffer

    ! Allocate receive type array to hold committed MPI subarray types
    call alloc(send(i)%sub_type, (/ n_tils /),"tiles/os-group-comm.f03",643)
...
subroutine isend_til_msg_diag_2d( vdf_arr, til1_id, no_co, send, n_send, n_sub, fc_msg, &
  ...
  do i = 1, n_send
    ...
    ! Allocate comm_buffer
    call alloc(send(i)%vdf_diag_buffer, (/bsize/),"tiles/os-group-comm.f03",750)
    buffer => send(i)%vdf_diag_buffer

    ! Allocate receive type array to hold committed MPI subarray types
    call alloc(send(i)%sub_type, (/ n_tils /),"tiles/os-group-comm.f03",754)
...
subroutine isend_til_msg_diag_3d( vdf_arr, til1_id, no_co, send, n_send, n_sub, fc_msg, &
  ...
  do i = 1, n_send
    ...
    ! Allocate comm_buffer
    call alloc(send(i)%vdf_diag_buffer, (/bsize/),"tiles/os-group-comm.f03",866)
    buffer => send(i)%vdf_diag_buffer

    ! Allocate receive type array to hold committed MPI subarray types
    call alloc(send(i)%sub_type, (/ n_tils /),"tiles/os-group-comm.f03",870)
...
subroutine wait_free_recv_diag( patt )
  ...
  do i = 1, patt%n_recv
    ...
    call freemem(patt%recv(i)%sub_type,"tiles/os-group-comm.f03",954)
...
subroutine wait_free_send_diag( patt )
  ...
  do i = 1, patt%n_send
    ...
    ! Free memory at the end
    call freemem(patt%send(i)%sub_type,"tiles/os-group-comm.f03",986)
    call freemem(patt%send(i)%vdf_diag_buffer,"tiles/os-group-comm.f03",987)
...
subroutine wait_irecv_vdf_group( vdf_arr_a, til1_id, msg, recv_msg, vdf_arr_b )
  ...
  call alloc(msg_sum, (/msg%n_tils/),"tiles/os-group-comm.f03",1014)
  msg_sum(1) = 1
  do i = 1, msg%n_tils-1
  ...
  call freemem(msg_sum,"tiles/os-group-comm.f03",1050)
...
subroutine isend_vdf_group( vdf_arr_a, til1_id, no_co, msg, send_vdf, vdf_arr_b )
  ...
  call alloc(msg_sum, (/msg%n_tils/),"tiles/os-group-comm.f03",1078)
  msg_sum(1) = 1
  do i = 1, msg%n_tils-1
  ...
  call freemem(msg_sum,"tiles/os-group-comm.f03",1104)
...
subroutine isend_1_spec_group( sim, spec, send_spec, msg, dim )
  ...
  if ( size_max > 0 ) call alloc(pos_scan, (/ size_max /),"tiles/os-group-comm.f03",1160)
  ...
  if (ierr/=0) then
    write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",1197)
    call abort_program( p_err_mpi )
  endif
  ...
  if ( size_max > 0 ) call freemem(pos_scan,"tiles/os-group-comm.f03",1226)


  ! write(err_buf__,*) "TRACKS not implemented";call err__("tiles/os-group-comm.f03",1229)
  ! call abort_program( p_err_invalid )
  ! TODO
...
subroutine unpack_spec_group( sim, spec, recv_spec, n_tils )
  ...
  if ( size_max > 0 ) then
    call alloc(pos_scan, (/ size_max /),"tiles/os-group-comm.f03",1270)
    call alloc(tile_nums, (/ size_max /),"tiles/os-group-comm.f03",1271)
  ...
  if ( size_max > 0 ) then
    call freemem(pos_scan,"tiles/os-group-comm.f03",1402)
    call freemem(tile_nums,"tiles/os-group-comm.f03",1403)
...
subroutine wait_til_msg( msg )
  ...
  if ( msg%request /= MPI_REQUEST_NULL ) then
    ...
    if ( ierr /= 0 ) then
      write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",1437)
      call abort_program( p_err_mpi )
    endif
  endif
...
subroutine wait_msg_array( msg, n_msg )
  ...
  if (j > 0) then
    call alloc(status, (/MPI_STATUS_SIZE, j/),"tiles/os-group-comm.f03",1469)

    call MPI_WAITALL(j, request, status, ierr)
    if (ierr/=0) then
      write(err_buf__,*) "MPI error";call err__("tiles/os-group-comm.f03",1473)
      call abort_program( p_err_mpi )
    endif

    call freemem(status,"tiles/os-group-comm.f03",1477)
...
subroutine unpack_tiles_dlb( patt, bsize, tiles )
  ...
  num_spec = tiles(lbound(tiles,1)) % t % part % num_species
  call alloc(preface, (/ num_spec+1 /),"tiles/os-group-comm.f03",1504)
    ...
    do j = 1, size( patt%recv(i)%tils )
      ...
      do
        ...
        if (associated(spec%x)) then
          write(err_buf__,*) " --in unpack_tiles, species particle arrays already allocated",til;call err__("tiles/os-group-comm.f03",1572)
          call abort_program( p_err_invalid )
        endif
  ...
  call freemem(preface,"tiles/os-group-comm.f03",1597)
  call freemem(bsize,"tiles/os-group-comm.f03",1598)
...
subroutine check_bsize_overflow( bsize, pack_size )
  ...
  if ( bsize > bsize_max - pack_size ) then
    print *, "[",mpi_node(),"] ",'Invalid value for tile buffer size. Integer overflow.'
    call abort_program( p_err_invalid )
  endif
