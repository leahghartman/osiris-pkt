# 1 "cyl_modes/os-emf-bound-cyl-modes.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "cyl_modes/os-emf-bound-cyl-modes.f03" 2
# 1 "./os-config.h" 1
...
# 2 "cyl_modes/os-emf-bound-cyl-modes.f03" 2
...
# 3 "cyl_modes/os-emf-bound-cyl-modes.f03" 2
...
!-----------------------------------------------------------------------------------------
  ...
  call restart_io_write("p_bcemf_cyl_modes_rst_id", p_bcemf_cyl_modes_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-bound-cyl-modes.f03",76)

  call restart_io_write("this%type", this%type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-bound-cyl-modes.f03",79)

  call restart_io_write("this%global_type", this%global_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-bound-cyl-modes.f03",82)
...
!-----------------------------------------------------------------------------------------
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-bound-cyl-modes.f03",130)

  if ( rst_id /= p_bcemf_cyl_modes_rst_id ) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("cyl_modes/os-emf-bound-cyl-modes.f03",133)
    write(err_buf__,*) 'from incompatible binary (emf_bound_cyl_modes)';call err__("cyl_modes/os-emf-bound-cyl-modes.f03",134)
    write(err_buf__,*) 'rst_id = "', rst_id, '"';call err__("cyl_modes/os-emf-bound-cyl-modes.f03",135)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-bound-cyl-modes.f03",140)

  call restart_io_read(this%global_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-bound-cyl-modes.f03",143)
  ...
  if ( restart ) then
    ...
    if ( grid % coordinates == p_cylindrical_b ) then
      ...
      if ((this%type(p_lower, p_r_dim) == p_bc_periodic) .or. &
        ...
        write(err_buf__,*) 'no periodic b.c. for radial direction';call err__("cyl_modes/os-emf-bound-cyl-modes.f03",331)
    ...
    do i_bnd = p_lower, p_upper
      if ( this%type( i_bnd, i_x_dim ) == p_bc_lindman ) then
        ...
        if ( previous > 0 .and. previous /= i_x_dim ) then
          write(err_buf__,*) "Lindman boundaries are only allowed in 1 direction";call err__("cyl_modes/os-emf-bound-cyl-modes.f03",372)
          call abort_program( p_err_invalid )
        endif
