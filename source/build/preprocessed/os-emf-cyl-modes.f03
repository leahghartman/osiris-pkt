# 1 "cyl_modes/os-emf-cyl-modes.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "cyl_modes/os-emf-cyl-modes.f03" 2
# 1 "./os-config.h" 1
...
# 2 "cyl_modes/os-emf-cyl-modes.f03" 2
...
# 3 "cyl_modes/os-emf-cyl-modes.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "cyl_modes/os-emf-cyl-modes.f03" 2
  ...
  if (ierr == 0) then
    ...
    if (ierr < 0) then
      ...
      if (disp_out(input_file)) then
        if (mpi_node()==0) print *," - emf parameters missing, using default"
      endif
    endif
  endif
...
!-----------------------------------------------------------------------------------------
  ...
  call restart_io_write("p_emf_cyl_modes_rst_id", p_emf_cyl_modes_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",899)
...
!-----------------------------------------------------------------------------------------
  ...
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",925)

  ! check if restart file is compatible
  if ( rst_id /= p_emf_cyl_modes_rst_id ) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("cyl_modes/os-emf-cyl-modes.f03",929)
    write(err_buf__,*) 'from incompatible binary (emf_cyl_modes)';call err__("cyl_modes/os-emf-cyl-modes.f03",930)
    write(err_buf__,*) 'rst_id = "', rst_id, '"';call err__("cyl_modes/os-emf-cyl-modes.f03",931)
...
subroutine init_diag_emf_cyl_modes( this, no_ext_fld, part_fld_alloc, interpolation )
  ...
  case default
    interp_offset = 0.0_p_double
    write(err_buf__,*) 'Interpolation value not supported';call err__("cyl_modes/os-emf-cyl-modes.f03",1145)
  ...
  do
    ...
    case ( p_ext_e1_cyl_m, p_ext_e2_cyl_m, p_ext_e3_cyl_m )
      if ( no_ext_fld ) then
        if (mpi_node()==0) print *,'(*warning*) External field diagnostic requested, but external fields'
        if (mpi_node()==0) print *,'            not in use.'
        report%ndump = 0
      else
    ...
    case ( p_part_e1_cyl_m, p_part_e2_cyl_m, p_part_e3_cyl_m )
      if ( .not. part_fld_alloc ) then
        if (mpi_node()==0) print *,'(*warning*) Particle fields diagnostic requested, but external/smoothed'
        if (mpi_node()==0) print *,'            fields are not in use. Use main field diagnostics instead.'
        report%ndump = 0
      else
...
subroutine reshape_emf_cyl_modes( this, old_lb, new_lb, no_co, send_msg, recv_msg )
  ...
  if ( this%ext_fld /= p_extfld_none ) then
    ...
    if ( this%ext_fld /= p_extfld_static ) then
      ...
      write(err_buf__,*) 'Not implemented yet';call err__("cyl_modes/os-emf-cyl-modes.f03",1725)
...
subroutine update_psi_cyl_modes( emf, no_co, send_msg, recv_msg )
  ...
  if ( no_num( no_co ) > 1 ) then
    ...
    if ( nodes_x > 1 ) then
       ...
       call alloc(accQ3D, (/ nx2, total_num_modes /),"cyl_modes/os-emf-cyl-modes.f03",1928)
       call alloc(acc_outQ3D, (/ nx2, total_num_modes /),"cyl_modes/os-emf-cyl-modes.f03",1929)
      ...
      !copy rest of modes - real + im
       ...
       call freemem(accQ3D,"cyl_modes/os-emf-cyl-modes.f03",1976)
       call freemem(acc_outQ3D,"cyl_modes/os-emf-cyl-modes.f03",1977)
...
!-----------------------------------------------------------------------------------------
  ...
  if (test_if_report( tstep, this%diag%ndump_fac_ene_int ) ) then
    ...
    if ( p_f_dim /= 3 ) then
      write(err_buf__,*) 'report_energy_emf_cyl_modes not supported for ',p_f_dim,' field dimensions';call err__("cyl_modes/os-emf-cyl-modes.f03",2031)
      call abort_program( p_err_notimplemented )
    endif
...
subroutine write_checkpoint_emf_gridval_cyl_modes( this, restart_handle )
  ...
  call restart_io_write("p_emf_gridval_cyl_modes_rst_id", p_emf_gridval_cyl_modes_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2504)

  call restart_io_write("this%uniform_e0_re", this%uniform_e0_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2507)

  call restart_io_write("this%uniform_e0_im", this%uniform_e0_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2510)

  call restart_io_write("this%uniform_b0_re", this%uniform_b0_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2513)

  call restart_io_write("this%uniform_b0_im", this%uniform_b0_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2516)

  call restart_io_write("this%mfunc_expr_b_re", this%mfunc_expr_b_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2519)

  call restart_io_write("this%mfunc_expr_b_im", this%mfunc_expr_b_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2522)

  call restart_io_write("this%mfunc_expr_e_re", this%mfunc_expr_e_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2525)

  call restart_io_write("this%mfunc_expr_e_im", this%mfunc_expr_e_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2528)
...
subroutine restart_read_emf_gridval_cyl_modes( this, restart_handle )
  ...
  call restart_io_read(rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"cyl_modes/os-emf-cyl-modes.f03",2548)

  ! check if restart file is compatible
  if ( rst_id /= p_emf_gridval_cyl_modes_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file';call err__("cyl_modes/os-emf-cyl-modes.f03",2552)
    write(err_buf__,*) 'from incompatible binary (emf_gridval_cyl_modes)';call err__("cyl_modes/os-emf-cyl-modes.f03",2553)
    write(err_buf__,*) 'rst_id = "', rst_id,'"';call err__("cyl_modes/os-emf-cyl-modes.f03",2554)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%uniform_e0_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2559)

  call restart_io_read(this%uniform_e0_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2562)

  call restart_io_read(this%uniform_b0_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2565)

  call restart_io_read(this%uniform_b0_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2568)

  call restart_io_read(this%mfunc_expr_b_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2571)

  call restart_io_read(this%mfunc_expr_b_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2574)

  call restart_io_read(this%mfunc_expr_e_re, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2577)

  call restart_io_read(this%mfunc_expr_e_im, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"cyl_modes/os-emf-cyl-modes.f03",2580)
