# 1 "particles/os-particles-define.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "particles/os-particles-define.f03" 2
!-----------------------------------------------------------------------------------------
...
# 11 "particles/os-particles-define.f03" 2
# 1 "./os-config.h" 1
...
# 12 "particles/os-particles-define.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 16 "particles/os-particles-define.f03" 2
...
use m_bnd
  ...
  use m_neutral, only : t_neutral, list_algorithm
...
use m_species, only : move_window, delayed_injection
...
type :: t_particles
  ...
  class( t_cathode ), dimension(:), pointer :: cathode => NULL()
...
contains
  ...
  procedure :: add_load => add_load_particles
  procedure :: reshape => reshape_part
  ...
  procedure :: delayed_injection => delayed_injection_part
  procedure :: get_spec_by_idx => get_spec_by_idx
  procedure :: copy_zpulse_to_spec => copy_zpulse_to_spec_part
  procedure :: inject_cathode => inject_cathode_part
...
interface
  ...
  import t_particles, t_space, t_emf, t_vdf, t_grid, t_node_conf, t_restart_handle, &
         p_double, t_options, t_current, t_bnd, t_time_step, t_zpulse_list
...
interface
  subroutine report_particles( this, emf, g_space, grid, no_co, tstep, t, tmin, &
                               send_msg, recv_msg )
...
interface
  ...
  import t_particles, t_grid, t_emf
...
interface
  ...
  import t_particles, t_grid, t_node_conf, t_vdf_msg
...
!-----------------------------------------------------------------------------------------
! perform delayed injection for each species
!-----------------------------------------------------------------------------------------
subroutine delayed_injection_part( this, jay, emf, grid, no_co, bnd, t )
  ...
  do
    if (.not. associated(species)) exit
    call delayed_injection( species, grid, emf, jay, no_co, bnd%send_vdf, bnd%recv_vdf, &
                            bnd%bnd_cross, bnd%node_cross, bnd%send_spec, bnd%recv_spec, t )
    species => species % next
  enddo

  ! not currently supported for neutrals / cathode

end subroutine delayed_injection_part
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
subroutine copy_zpulse_to_spec_part( this, zpulse_list )

  class( t_particles ), intent(inout) :: this
  class( t_zpulse_list ), intent(in), target :: zpulse_list
  ...
  do
    ...
    if ( spec%udist%use_zpulse_equilibrium ) then
      spec%udist%zplist => zpulse_list
    endif
    spec => spec % next
  enddo

end subroutine copy_zpulse_to_spec_part
!-----------------------------------------------------------------------------------------
...
subroutine inject_cathode_part( this, jay, t, dt, no_co, coordinates )
  ...
  do i=1, this%num_cathode
  call this % cathode(i) % inject_lowroundoff( jay%pf(1), this%jay_tmp, &
  ...
  do i=1, this%num_cathode
  call this % cathode(i) % inject( jay%pf(1), t, dt, no_co, coordinates)
  enddo
