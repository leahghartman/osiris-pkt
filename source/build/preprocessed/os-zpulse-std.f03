# 1 "zpulse/os-zpulse-std.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "zpulse/os-zpulse-std.f03" 2
...
subroutine read_input_zpulse( this, input_file, g_space, bnd_con, periodic, grid, sim_options )
  ...
  namelist /nl_zpulse/ &
    if_launch, b_type, a0, omega0, k0, phase, pol_type, pol, propagation, direction, &
  ...
  if (disp_out(input_file)) then
    if (mpi_node()==0) print *," - Reading zpulse configuration..."
  endif
...
subroutine init_zpulse( this, restart, t, dt, b, g_space, nx_p_min, no_co, interpolation )
  ...
  if ( this%if_launch ) then
    ...
    case (p_bessel)
      if (p_x_dim /= 3) then
        write(err_buf__,*) 'Bessel beams only exist in 3D geometries.';call err__("zpulse/os-zpulse-std.f03",915)
...
subroutine launch_zpulse_1d( this, b, e, t, nx_p_min, g_space )
  ...
  do i1 = b%lbound(2), b%ubound(2)
    ...
    b%f1(2,i1) = b%f1(2,i1) - real( lenv_2 * cos(k_z_2*(z_2 - z_center)-this%prop_sign*this%omega0*t+this%phase0) * &
      sin_pol * this%prop_sign * k_z_2/this%omega0, p_k_fld )
    b%f1(3,i1) = b%f1(3,i1) + real( lenv_2 * cos(k_z_2*(z_2 - z_center)-this%prop_sign*this%omega0*t+this%phase0) * &
      cos_pol * this%prop_sign * k_z_2/this%omega0, p_k_fld )

    ! e%f1(1,i1) = e%f1(1,i1) + 0.0_p_double
    e%f1(2,i1) = e%f1(2,i1) + real( lenv * cos(k_z *(z - z_center) -this%prop_sign*this%omega0*t+ this%phase0) * &
      cos_pol, p_k_fld )
    e%f1(3,i1) = e%f1(3,i1) + real( lenv * cos(k_z *(z - z_center) -this%prop_sign*this%omega0*t+ this%phase0) * &
      sin_pol, p_k_fld )
...
subroutine launch_zpulse_2d( this, b, e, t, nx_p_min, g_space )
  ...
  real(p_double) :: cos_pol, sin_pol, k
  ...
  if ( .not. this%if_lon_tilt ) then
    ...
    do i1 = b%lbound(2), b%ubound(2)
      ...
      do i2 = b%lbound(3), b%ubound(3)
        ...
        b%f2(2,i1,i2) = real( - lenv_2 * this % per_envelope_2d(t,z_2,r ) * sin_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )
        b%f2(3,i1,i2) = real( + lenv_2 * this % per_envelope_2d(t,z_2,r_2) * cos_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )

        e%f2(1,i1,i2) = 0.0_p_k_fld
        e%f2(2,i1,i2) = real( lenv * this % per_envelope_2d(t,z,r_2) * cos_pol, p_k_fld )
        e%f2(3,i1,i2) = real( lenv * this % per_envelope_2d(t,z,r ) * sin_pol, p_k_fld )
    ...
    do i1 = b%lbound(2), b%ubound(2)
      ...
      do i2 = b%lbound(3), b%ubound(3)
        ...
        b%f2(2,i1,i2) = real( - amp * this % lon_envelope( z_2 + this%lon_tilt(1) * ( r - r_center ), t ) * &
          this % per_envelope_2d(t,z_2,r ) * sin_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )
        b%f2(3,i1,i2) = real( + amp * this % lon_envelope( z_2 + this%lon_tilt(1) * (r_2 - r_center), t ) * &
          this % per_envelope_2d(t,z_2,r_2) * cos_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )

        e%f2(1,i1,i2) = 0.0_p_k_fld
        e%f2(2,i1,i2) = real( + amp * this % lon_envelope( z + this%lon_tilt(1) * ( r_2 - r_center ), t ) * &
          this % per_envelope_2d(t,z,r_2) * cos_pol, p_k_fld )
        e%f2(3,i1,i2) = real( + amp * this % lon_envelope( z + this%lon_tilt(1) * ( r - r_center ), t ) * &
          this % per_envelope_2d(t,z,r ) * sin_pol, p_k_fld )
  ...
  if (this%if_boost) then
    ...
    if (mpi_node()==0) print *,'Boosting zpulse initialization...'
    ...
    do i1 = b%lbound(2), b%ubound(2)
      ...
      do i2 = b%lbound(3), b%ubound(3)
        ...
        do i3 = b%lbound(4), b%ubound(4)
          ...
          b%f3(2,i1,i2,i3) = b%f3(2,i1,i2,i3) - real( lenv_2 * this % per_envelope_3d(t,z_2,r1 ,r2_2) * &
            sin_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )
          b%f3(3,i1,i2,i3) = b%f3(3,i1,i2,i3) + real( lenv_2 * this % per_envelope_3d(t,z_2,r1_2,r2 ) * &
            cos_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )

          !e%f3(1,i1,i2,i3) = 0.0_p_double
          e%f3(2,i1,i2,i3) = e%f3(2,i1,i2,i3) + real( lenv * this % per_envelope_3d(t,z,r1_2,r2 ) * cos_pol, p_k_fld )
          e%f3(3,i1,i2,i3) = e%f3(3,i1,i2,i3) + real( lenv * this % per_envelope_3d(t,z,r1 ,r2_2) * sin_pol, p_k_fld )
      ...
      do i2 = b%lbound(3), b%ubound(3)
        ...
        do i3 = b%lbound(4), b%ubound(4)
          ...
          b%f3(2,i1,i2,i3) = real( - amp * this % lon_envelope( z_2 + &
            ...
            this % per_envelope_3d(t,z_2,r1 ,r2_2) * sin_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )
          b%f3(3,i1,i2,i3) = real( + amp * this % lon_envelope( z_2 + &
            this%lon_tilt(1) * ( r1_2 - r1_center ) + &
            this%lon_tilt(2) * ( r2 - r2_center ), t ) * &
            this % per_envelope_3d(t,z_2,r1_2,r2 ) * cos_pol * this%prop_sign * this%k0/this%omega0, p_k_fld )

          !e%f3(1,i1,i2,i3) = 0.0_p_double
          e%f3(2,i1,i2,i3) = real( + amp * this % lon_envelope( z + &
            this%lon_tilt(1) * ( r1_2 - r1_center ) + &
            this%lon_tilt(2) * ( r2 - r2_center ), t ) * &
            this % per_envelope_3d(t,z ,r1_2,r2 ) * cos_pol, p_k_fld )
          e%f3(3,i1,i2,i3) = real( + amp * this % lon_envelope( z + &
            this%lon_tilt(1) * ( r1 - r1_center ) + &
            this%lon_tilt(2) * ( r2_2 - r2_center ), t ) * &
            this % per_envelope_3d(t,z ,r1 ,r2_2) * sin_pol, p_k_fld )
  ...
  do i2 = 1, b%nx_(idx2)
    ...
    do i1 = g_nx(1) - nx_p_min(1) + 1, b%nx_(1) + 1, -1
      ...
      if (lenv > 0.0_p_double) then
        e2p = lenv * this % per_envelope_2d( t, x1min + i1*dx1, x2min + (i2-0.5)*dx2 ) * cos_pol
        e2m = lenv * this % per_envelope_2d( t, x1min + i1*dx1, x2min + (i2-1.5)*dx2 ) * cos_pol
      ...
      if (lenv > 0.0_p_double) then
        b2p = - lenv * this % per_envelope_2d( t, x1min + (i1-0.5)*dx1, x2min + (i2)*dx2 ) * sin_pol * prop_sign
        b2m = - lenv * this % per_envelope_2d( t, x1min + (i1-0.5)*dx1, x2min + (i2-1)*dx2 ) * sin_pol * prop_sign
  ...
  do i3 = 1, b%nx_(idx3)
    do i2 = 1, b%nx_(idx2)
      ...
      do i1 = g_nx(1) - nx_p_min(1) + 1, b%nx_(1) + 1, -1
        ...
        lenv = amp * this % lon_envelope( x1min + i1*dx1, t )
        if (lenv > 0.0_p_double) then
          e2p = lenv * this % per_envelope_3d( t, x1min + i1 *dx1, &
                                                x2min + (i2-0.5)*dx2, &
                                                x3min + (i3-1 )*dx3 ) * cos_pol
          e2m = lenv * this % per_envelope_3d( t, x1min + i1 *dx1, &
                                                x2min + (i2-1.5)*dx2, &
                                                x3min + (i3-1 )*dx3 ) * cos_pol

          e3p = lenv * this % per_envelope_3d( t, x1min + i1 *dx1, &
                                                x2min + (i2-1 )*dx2, &
                                                x3min + (i3-0.5)*dx3 ) * sin_pol
          e3m = lenv * this % per_envelope_3d( t, x1min + i1 *dx1, &
                                                x2min + (i2-1 )*dx2, &
                                                x3min + (i3-1.5)*dx3 ) * sin_pol
        ...
        lenv = amp * this % lon_envelope( x1min + (i1-0.5)*dx1, t )
        if (lenv > 0.0_p_double) then
          b2p = - lenv * this % per_envelope_3d( t, x1min + (i1-0.5)*dx1, &
                                                 x2min + (i2 )*dx2, &
                                                 x3min + (i3-0.5)*dx3 ) * sin_pol * prop_sign
          b2m = - lenv * this % per_envelope_3d( t, x1min + (i1-0.5)*dx1, &
                                                 x2min + (i2-1 )*dx2, &
                                                 x3min + (i3-0.5)*dx3 ) * sin_pol * prop_sign

          b3p = lenv * this % per_envelope_3d( t, x1min + (i1-0.5)*dx1, &
                                                 x2min + (i2-0.5)*dx2, &
                                                 x3min + (i3 )*dx3 ) * cos_pol * prop_sign
          b3m = lenv * this % per_envelope_3d( t, x1min + (i1-0.5)*dx1, &
                                                 x2min + (i2-0.5)*dx2, &
                                                 x3min + (i3-1 )*dx3 ) * cos_pol * prop_sign
...
!-----------------------------------------------------------------------------------------
  ...
  call alloc(lg_x_range, (/2,b_pulse%x_dim_/),"zpulse/os-zpulse-std.f03",2231)
...
write(err_buf__,*) 'Divergence correction of zpulses using interpolated B-Field';call err__("zpulse/os-zpulse-std.f03",2267)
write(err_buf__,*) 'is not yet implemented';call err__("zpulse/os-zpulse-std.f03",2268)
  ...
  call freemem(lg_x_range,"zpulse/os-zpulse-std.f03",2275)
  endif
...
! envelope at the requested position
!-----------------------------------------------------------------------------------------
function per_envelope_2d_zpulse( this, tin_sim, x1_sim, x2 ) result( per_envelope_2d )
...
! envelope at the requested position
!-----------------------------------------------------------------------------------------
function per_envelope_3d_zpulse( this, tin_sim, x1_sim, x2, x3 ) result( per_envelope_3d )
