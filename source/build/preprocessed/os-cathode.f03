...
# 10 "os-cathode.f03" 2
  ...
  type :: t_cathode
    ...
    integer :: temp_type ! type of temporal density profile
    ...
    character(len = p_max_expr_len) :: temp_func_expr = " "
    type(t_fparser) :: temp_func
  ...
  contains
    ...
    procedure :: restart_read => restart_read_cathode
...
subroutine read_input_cathode( this, input_file, pspecies, spname, periodic, if_move, &
  ...
  character(20) :: temp_type ! temporal density profile type
  ...
  namelist /nl_cathode/ dir, wall, temp_type, t_start, t_rise, t_fall, t_flat, &
            temp_func_expr, mov_inj, prof_type, density, center, gauss_width, gauss_w0, &
            tr_u_expression, channel_bottom, channel_r0, channel_depth, &
...
! executable statements
  ...
  temp_type = "polynomial" ! defaults to a polynomial temporal density (using t_rise, etc)
  ...
  temp_func_expr = "NO_FUNCTION_SUPPLIED!"
  ...
  select case ( trim( prof_type ) )
    ...
    case ( "polynomial" )
      this%prof_type = p_polynomial
      this%gauss_width = gauss_width ! Full width of flat portion
      this%gauss_w0 = gauss_w0 ! Length of taper from 1 to 0 on edges
      ...
      if (this%gauss_w0 <= 0.0) then
        write(0,*) "(*error*) invalid transverse profile for cathode."
        write(0,*) "(*error*) 'gauss_w0' must be specified and > 0.0 when using ", &
             "the polynomial profile."
        write(0,*) "(*error*) bailing out..."
        stop
      endif
  ...
  case ( "polynomial" )
    this % temp_type = p_polynomial

  case ( "math func" )
    this % temp_type = p_func
    this % temp_func_expr = trim(temp_func_expr)

    ! Check math function
    call setup(this%temp_func, trim(this%temp_func_expr), (/'t'/), ierr)

    if (ierr /= 0) then
      if ( mpi_node() == 0 ) then
        write(0,*) "(*error*) Invalid function supplied : '", &
        trim(this%temp_func_expr), "'"
      endif
      stop
    endif

  case default
    write(0,*) "(*error*) invalid temporal profile for cathode."
    write(0,*) "(*error*) bailing out..."
    call abort_program()

  end select

  if (this%temp_type == p_polynomial) then
  ...
  if( tr_u_expression(i) .ne. "off" ) then
    ...
    if (ierr .ne. 0) then
    write(err_buf__,*) 'Error seting up transverse momentum function.';call err__("os-cathode.f03",463)
  ...
  if (disp_out(input_file)) then
    if (mpi_node()==0) print *,"    - reading associated species configuration..."
  endif
...
!---------------------------------------------------
   ...
   case (2)
     ...
     call alloc(this%ppos_cell, (/ 1, lnum_par_x(iperp(1)) /),"os-cathode.f03",549)
   ...
   case (3)
     ...
     call alloc(this%ppos_cell, (/ 2, lnum_par_x(iperp(1))*lnum_par_x(iperp(2)) /),"os-cathode.f03",558)
   ...
   if ( restart ) then
    ...
    if (.not. associated(this%species)) then
        write(err_buf__,*) 'species is not associated in setup_cathode';call err__("os-cathode.f03",582)
    call abort_program(p_err_invalid)
    endif
...
subroutine cleanup_cathode( this )
  ...
  call freemem(this%ppos_cell,"os-cathode.f03",671)
...
! executable statements
  ...
  call restart_io_write("p_cath_rst_id", p_cath_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",700)

  call restart_io_write("this%cathode_id", this%cathode_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",703)

  call restart_io_write("this%dir", this%dir, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",706)

  call restart_io_write("this%wall", this%wall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",709)

  call restart_io_write("this%deposit_current", this%deposit_current, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",712)

  call restart_io_write("this%inppos", this%inppos, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",715)

  call restart_io_write("this%nppos", this%nppos, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",718)

  call restart_io_write("this%vel", this%vel, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",721)

  call restart_io_write("this%rgamma", this%rgamma, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",724)

  ! Omitted for now to keep file format the same (everything is read in anyway...)
  ! call restart_io_write("this%temp_type", this%temp_type, restart_handle, ierr)
  ! call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",728)

  call restart_io_write("this%t_start", this%t_start, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",731)

  call restart_io_write("this%t_rise", this%t_rise, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",734)

  call restart_io_write("this%t_fall", this%t_fall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",737)

  call restart_io_write("this%t_flat", this%t_flat, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",740)

  call restart_io_write("this%t_total", this%t_total, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",743)

  call restart_io_write("this%prof_type", this%prof_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",746)

  call restart_io_write("this%density", this%density, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",749)

  call restart_io_write("this%center", this%center, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",752)

  call restart_io_write("this%gauss_width", this%gauss_width, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",755)

  call restart_io_write("this%gauss_w0", this%gauss_w0, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",758)

  call restart_io_write("this%sp_id", this%sp_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",761)

  call restart_io_write("this%channel_bottom", this%channel_bottom, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",764)

  call restart_io_write("this%channel_r0", this%channel_r0, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",767)

  call restart_io_write("this%channel_depth", this%channel_depth, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",770)

  call restart_io_write("this%channel_size", this%channel_size, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",773)

  call restart_io_write("this%channel_wall", this%channel_wall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",776)
...
! executable statements
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",809)

  ! check if restart file is compatible
  if ( rst_id /= p_cath_rst_id) then
  write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("os-cathode.f03",813)
  write(err_buf__,*) 'from incompatible binary (cathode)';call err__("os-cathode.f03",814)
  call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%cathode_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",819)

  call restart_io_read(this%dir, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",822)

  call restart_io_read(this%wall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",825)

  call restart_io_read(this%deposit_current, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",828)

  call restart_io_read(this%inppos, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",831)

  call restart_io_read(this%nppos, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",834)

  call restart_io_read(this%vel, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",837)

  call restart_io_read(this%rgamma, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",840)

  ! Omitted for now to keep file format the same (everything is read in anyway...)
  ! call restart_io_read(this%temp_type, restart_handle, ierr)
  ! call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",844)

  call restart_io_read(this%t_start, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",847)

  call restart_io_read(this%t_rise, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",850)

  call restart_io_read(this%t_fall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",853)

  call restart_io_read(this%t_flat, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",856)

  call restart_io_read(this%t_total, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",859)

  call restart_io_read(this%prof_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",862)

  call restart_io_read(this%density, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",865)

  call restart_io_read(this%center, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",868)

  call restart_io_read(this%gauss_width, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",871)

  call restart_io_read(this%gauss_w0, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",874)

  call restart_io_read(this%sp_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",877)

  call restart_io_read(this%channel_bottom, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",880)

  call restart_io_read(this%channel_r0, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",883)

  call restart_io_read(this%channel_depth, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",886)

  call restart_io_read(this%channel_size, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",889)

  call restart_io_read(this%channel_wall, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"os-cathode.f03",892)
...
subroutine create_particles_cathode( this, t, dt, coords )
  ...
  ! Set momentum of injected particles
   ...
   select case (tr_parser)
     ...
     do k=ptrcur, ptrcur + npar - 1
      ...
      call this % species % get_position( k, xnewpart )
      do i=1, p_x_dim-1
  ...
  class( t_cathode ), intent(in) :: cathode
...
! x = r
  ...
  ! gaussian like polynomial for time envelope and transverse profile
