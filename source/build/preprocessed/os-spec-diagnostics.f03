# 1 "spec/diagnostics/os-spec-diagnostics.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/diagnostics/os-spec-diagnostics.f03" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 7 "spec/diagnostics/os-spec-diagnostics.f03" 2
...
# 8 "spec/diagnostics/os-spec-diagnostics.f03" 2
...
module m_species_diagnostics
  ...
  integer, public :: diag_part_boost_ev, diag_part_boost_extract_ev
  ...
  character(len=12), dimension(4), parameter, public :: &
     p_boosted_report_quants = (/ 'charge_boost', &
                                  'j1_boost    ', &
                                  'j2_boost    ', &
                                  'j3_boost    ' /)
  ...
  character(len=6), dimension(24), parameter, public :: &
     p_rep_udist = (/ 'ufl1  ', &
                          'ufl2  ', &
                          'ufl3  ', &
                          'uth1  ', &
  ...
  integer, parameter, public :: p_vfl1 = 13
  integer, parameter, public :: p_vfl2 = 14
  integer, parameter, public :: p_vfl3 = 15

  integer, parameter, public :: p_P11 = 16
  integer, parameter, public :: p_P22 = 17
  integer, parameter, public :: p_P33 = 18

  integer, parameter, public :: p_P12 = 19
  integer, parameter, public :: p_P13 = 20
  integer, parameter, public :: p_P23 = 21

  integer, parameter, public :: p_nufl1 = 22
  integer, parameter, public :: p_nufl2 = 23
  integer, parameter, public :: p_nufl3 = 24



  ! special quantity for normalizing local deposited values:
  ! abs( 1 / charge ) if charge /= 0, 1 otherwise
  integer, parameter, public :: p_norm = 88
...
!-----------------------------------------------------------------------------------------
  ...
  call this % boosted_diag % cleanup()
  deallocate( this % boosted_diag )

  deallocate( this % boosted_report_quants )
...
!-----------------------------------------------------------------------------------------
subroutine get_current_4vector( spec, grid, no_co, current, send_msg, recv_msg )
  ...
  integer, dimension (2, p_x_dim) :: gc_num
  integer :: quant, n
  integer, dimension(4), parameter :: quants = (/ p_charge, p_j1, p_j2, p_j3 /)
  type(t_vdf) :: tmp_vdf
  integer :: component
  ...
  do n = 1, spec%diag%boosted_diag%num_diag_components_to_buffer
    ...
    call deposit_quant( spec, grid, no_co, tmp_vdf, quant, send_msg, recv_msg )

    call current % copy( tmp_vdf, 1, component )

    call tmp_vdf % cleanup()

  enddo

end subroutine get_current_4vector
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
subroutine report_heat_flux_species( spec, no_co, tstep, t, tmin )
!-----------------------------------------------------------------------------------------
...
!---------------------------------------------------
subroutine report_temperature_species( spec, no_co, tstep, t, tmin )
!---------------------------------------------------
...
subroutine init_report_quants_species( this )
  ...
  use m_species_define, only : t_diag_species
  use m_species_diagnostics, only : p_report_quants, p_boosted_report_quants
...
# 1265 "spec/diagnostics/os-spec-diagnostics.f03"
  ! process phasespaces
...
!-----------------------------------------------------------------------------------------
  ...
  subroutine setup_report_list( list, spec_name, path, rep_type, interpolation, label_prefix )
    ...
    case default
      interp_offset = 0.0_p_double
      write(err_buf__,*) 'Interpolation value not supported';call err__("spec/diagnostics/os-spec-diagnostics.f03",1449)
...
! Species diagnostics
!-----------------------------------------------------------------------------------------
subroutine report_species( this, emf, g_space, grid, no_co, tstep, t, tmin, send_msg, &
                           recv_msg )
  ...
  if( this%diag%if_use_boosted_diag ) then
    ...
    call get_current_4vector( this, grid, no_co, current_4vector, send_msg, recv_msg )

    call this%diag%boosted_diag%extract( tstep, t, grid, g_space, current_4vector )
  ...
  if( this%diag%if_use_boosted_raw ) then
    ...
    call extract_boosted_raw( this, this%diag%boosted_diag, tstep, t, grid, g_space )
    ...
    if ( test_if_report( tstep, this%diag%boosted_diag%ndump_fac_raw ) ) then
      ...
      call write_boosted_raw( this, this%diag%boosted_diag, no_co, grid, n(tstep) )
...
!-----------------------------------------------------------------------------------------
  ...
  if (test_if_report( tstep, this%diag%ndump_fac_ene )) then
      ...
      if ( root(no_co) ) then
        ...
        if ( t == tmin ) then
            call mkdir( path, ierr )
            call check_error(ierr,p_err_msg_spec,p_err_diagfile,"spec/diagnostics/os-spec-diagnostics.f03",2086)
