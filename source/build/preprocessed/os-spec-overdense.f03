# 1 "overdense/os-spec-overdense.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "overdense/os-spec-overdense.f03" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 6 "overdense/os-spec-overdense.f03" 2
...
# 7 "overdense/os-spec-overdense.f03" 2

module m_spec_overdense
...
# 19 "./memory/memory.h"
use memory
# 11 "overdense/os-spec-overdense.f03" 2

use m_system
use m_parameters
use m_species_define, only : t_species, t_part_idx, t_spec_msg, p_cell_low, p_cell_near
...
type, extends( t_species ) :: t_species_overdense
...
contains
  ...
  procedure :: read_input => read_input_spec_overdense
  procedure :: init => init_spec_overdense
  procedure :: push => push_spec_overdense
  procedure :: cleanup => cleanup_spec_overdense
  procedure :: list_algorithm => list_algorithm_spec_overdense
  procedure :: reshape => reshape_spec_overdense
...
end type t_species_overdense

public :: t_species_overdense
...
!-----------------------------------------------------------------------------------------
subroutine read_input_spec_overdense( this, input_file, def_name, periodic, if_move, &
                                      grid, dt, read_prof, sim_options )

  implicit none

  class( t_species_overdense ), intent(inout) :: this
...
end subroutine read_input_spec_overdense
!-----------------------------------------------------------------------------------------

!-----------------------------------------------------------------------------------------
! sets up this data structure from the given information
!-----------------------------------------------------------------------------------------
subroutine init_spec_overdense( this, sp_id, interpolation, grid_center, grid, g_space, emf, &
...
end subroutine init_spec_overdense
!-----------------------------------------------------------------------------------------

!-----------------------------------------------------------------------------------------
! Damp particles, then call superclass push
!-----------------------------------------------------------------------------------------
subroutine push_spec_overdense( this, emf, current, t, tstep, tid, n_threads, options )
...
end subroutine push_spec_overdense
!-----------------------------------------------------------------------------------------

!-----------------------------------------------------------------------------------------
! Call superclass cleanup, then cleanup extra members
!-----------------------------------------------------------------------------------------
subroutine cleanup_spec_overdense( this )
...
end subroutine cleanup_spec_overdense
!-----------------------------------------------------------------------------------------

!-----------------------------------------------------------------------------------------
! List damping algorithm
!-----------------------------------------------------------------------------------------
subroutine list_algorithm_spec_overdense( this )

  implicit none

  class( t_species_overdense ), intent(in) :: this

  ! Call superclass list_algorithm
  call this % t_species % list_algorithm()
...
end subroutine list_algorithm_spec_overdense
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
subroutine reshape_spec_overdense( this, old_grid, new_grid, msg_patt, no_co, send_msg, recv_msg )
!-----------------------------------------------------------------------------------------
  ...
  if ( this%splitter%if_recombine ) then
    ...
    do
      ...
      if ( recombine%if_recombine_local ) then
        call freemem(recombine%cells,"overdense/os-spec-overdense.f03",395)
      endif
      ...
      if ( recombine%if_recombine_local ) then
        ...
        call alloc(recombine%cells, (/ cell_count /),"overdense/os-spec-overdense.f03",409)
...
end subroutine reshape_spec_overdense
!-----------------------------------------------------------------------------------------
...
subroutine damp_species( this, damper, dt, i0, i1 )
  ...
  integer :: i, j
  real(p_k_part), dimension(p_x_dim) :: x_vector
...
subroutine push_particles_apart( this, i, j )
  ...
  case ( 2 )
    ...
    if ( this%p(2,i) == 0 ) then ! i think this is unnecessary
      u = (/zero, a*mag_i, zero /)
    else if ( this%p(1,i) == 0) then
      u = (/a*mag_i, zero, zero /)
    else
      u(2) = a*mag_i/sqrt(1 + this%p(2,i)**2/this%p(1,i)**2)
      u(1) = u(2)*this%p(2,i)/this%p(1,i)
      u(3) = zero
    endif
...
subroutine split_species( this, n )
  ...
  class( t_species_overdense ), intent(inout), target :: this
  integer, intent(in) :: n
  ...
  real(p_k_part), dimension(p_x_dim) :: position, x_copy
...
subroutine recombine_species( species, recombine, n, t )
  ...
  class( t_species_overdense ), intent(inout), target :: species
  ...
  real( p_k_part ), dimension( p_x_dim ) :: x ! Position vector for particle #idx
...
# 1086 "overdense/os-spec-overdense.f03"
    ...
    write(err_buf__,*) "Something went wrong in particle recombinations.";call err__("overdense/os-spec-overdense.f03",1144)
    write(err_buf__,*) "Hopefully Josh was still debugging them.";call err__("overdense/os-spec-overdense.f03",1145)
    call abort_program( p_err_invalid )
