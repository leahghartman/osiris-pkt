# 1 "emf/boundary/os-emf-lindman.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "emf/boundary/os-emf-lindman.f90" 2
...
# 23 "emf/boundary/os-emf-lindman.f90" 2
...
# 24 "emf/boundary/os-emf-lindman.f90" 2
...
!---------------------------------------------------
   ...
   call restart_io_write("p_lindman_rst_id", p_lindman_rst_id, restart_handle, ierr)
   if ( ierr/=0 ) then
     write(err_buf__,*) 'error writing restart data for lindman object.';call err__("emf/boundary/os-emf-lindman.f90",147)
...
!---------------------------------------------------
   ...
   if ( ierr/=0 ) then
      write(err_buf__,*) 'error reading restart data for lindman object.';call err__("emf/boundary/os-emf-lindman.f90",177)
      call abort_program(p_err_rstrd)
   endif
   if ( rst_id /= p_lindman_rst_id) then
      write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("emf/boundary/os-emf-lindman.f90",181)
      write(err_buf__,*) 'from incompatible binary (lindman)';call err__("emf/boundary/os-emf-lindman.f90",182)
      call abort_program(p_err_rstrd)
   endif
...
!---------------------------------------------------
 ...
 call asrt__(p_f_dim == 3,"p_f_dim == 3","emf/boundary/os-emf-lindman.f90",218)
 ...
 select case(x_dim)
  ...
  case (1)
    if (i_dim == 1 ) then
        ...
        write(err_buf__,*) 'LINDMAN(NEW): Error in i_dir';call err__("emf/boundary/os-emf-lindman.f90",254)
  ...
  case (2)
    select case(i_dim)
      ...
      case default
        write(err_buf__,*) 'LINDMAN(NEW): Error in i_dir';call err__("emf/boundary/os-emf-lindman.f90",272)
         call abort_program( p_err_invalid )
   end select
 ...
 case (3)
    ...
    select case(i_dim)
      ...
      case default
        write(err_buf__,*) 'LINDMAN(NEW): Error in i_dir';call err__("emf/boundary/os-emf-lindman.f90",301)
        call abort_program( p_err_invalid )
    end select
 ...
 select case (orientation)
   ...
   case default
     write(err_buf__,*) 'LINDMAN(NEW): Error in orientation';call err__("emf/boundary/os-emf-lindman.f90",321)
     call abort_program( p_err_invalid )
 end select
 this%range = range
...
if(this%idir < 1) then
write(err_buf__,*) 'LINDMAN(UPDATE): The Lindman Object is not active';call err__("emf/boundary/os-emf-lindman.f90",619)
call abort_program(p_err_invalid)
end if
...
if( this%idir < 1) then
write(err_buf__,*) 'LINDMAN(UPDATE_B): The Lindman Object is not active';call err__("emf/boundary/os-emf-lindman.f90",770)
call abort_program(p_err_invalid)
end if
...
if ( dim /= this%idir ) then
  write(err_buf__,*) "invalid dimension specified";call err__("emf/boundary/os-emf-lindman.f90",890)
  call abort_program( p_err_invalid )
endif
