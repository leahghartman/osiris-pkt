# 1 "vdf/os-wall-comm.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-wall-comm.f90" 2
!#define __DEBUG__ 1
...
subroutine move_window_wall( this, space )
  ...
  if ( x_dim(space) /= this%x_dim) then
    write(err_buf__,*) "The dimensions of the space object, ", x_dim(space) ," do not";call err__("vdf/os-wall-comm.f90",76)
    write(err_buf__,*) "match the dimensions of the wall object, ", this%x_dim;call err__("vdf/os-wall-comm.f90",77)
  ...
  do i_dim = 1, this%x_dim
    if(lnx_move(i_dim) < 0) then
      write(err_buf__,*) "Illegal move, only positive moves allowed";call err__("vdf/os-wall-comm.f90",88)
      call abort_program( p_err_invalid )
    endif
  enddo
  ...
  do i_dim = 1, this%x_dim
    if ((lnx_move(i_dim) > this%gc_num(1,i_dim)) .or. &
       ...
       write(err_buf__,*) "Illegal move, too many cells along direction ",i_dim;call err__("vdf/os-wall-comm.f90",96)
       write(err_buf__,*) "gc_num(:,",i_dim,") = ",this%gc_num(:,i_dim);call err__("vdf/os-wall-comm.f90",97)
       write(err_buf__,*) "lnx_move(",i_dim,")  = ",lnx_move(i_dim);call err__("vdf/os-wall-comm.f90",98)
       call abort_program(p_err_invalid)
    endif
  enddo
...
subroutine isend_wall( wall, update_type, dim, bnd, no_co, nx_move, send_msg )
  ...
  if ( send_msg(bnd)%handle /= MPI_REQUEST_NULL ) then
    write(err_buf__,*) 'isend_msg_wall called when message still waiting to complete';call err__("vdf/os-wall-comm.f90",432)
    call abort_program( p_err_invalid )
  endif

    if ( neighbor( no_co, bnd, dim ) < 0 ) then
        write(0,*) 'isend_msg_wall called without a valid neighbor, dim, bnd = ',dim, bnd, &
        trim("vdf/os-wall-comm.f90"), ':', 438
        call abort_program( p_err_invalid )
    endif
  ...
  if ( msg_size > bsize ) then
        if ( bsize > 0 ) call freemem(send_msg(bnd)%buffer,"vdf/os-wall-comm.f90",488)
    call alloc(send_msg(bnd)%buffer, (/msg_size/),"vdf/os-wall-comm.f90",489)
  endif
...
subroutine irecv_wall( wall, update_type, dim, bnd, no_co, nx_move, recv_msg )
  ...
  if ( recv_msg( bnd )%handle /= MPI_REQUEST_NULL ) then
    write(err_buf__,*) 'irecv_wall called when message still waiting to complete';call err__("vdf/os-wall-comm.f90",534)
    call abort_program( p_err_invalid )
  endif
  ...
  ! (*debug*)
  if ( msg_size == 0 ) then
    write(err_buf__,*) 'Invalid message size, aborting.';call err__("vdf/os-wall-comm.f90",580)
    call abort_program( p_err_invalid )
  endif
  ...
  if ( msg_size > bsize ) then
        if ( bsize > 0 ) call freemem(recv_msg(bnd)%buffer,"vdf/os-wall-comm.f90",606)
    call alloc(recv_msg(bnd)%buffer, (/msg_size/),"vdf/os-wall-comm.f90",607)
  endif
...
subroutine reshape_wall_nocopy( this, new_grid, dbg )
  ...
  if (this%x_dim > 0) then
     ...
     select case (x_dim)
        case(1)
          call freemem(this%f1,"vdf/os-wall-comm.f90",824)
          call alloc(this%f1, lower, upper,"vdf/os-wall-comm.f90",825)
        ...
        case(2)
          call freemem(this%f2,"vdf/os-wall-comm.f90",832)
          call alloc(this%f2, lower, upper,"vdf/os-wall-comm.f90",833)
        ...
        case(3)
          call freemem(this%f3,"vdf/os-wall-comm.f90",840)
          call alloc(this%f3, lower, upper,"vdf/os-wall-comm.f90",841)
...
subroutine init_msg_buffers_wall( this, msg_patt, send, recv, my_node )
  ...
  do i = 1, msg_patt%n_send
    ...
    call alloc(send(i)%buffer, (/bsize/),"vdf/os-wall-comm.f90",889)
    ...
    if (send(i)%node == my_node) then
      ...
      if ( self_send_id >= 1 ) then
        write(err_buf__,*) 'More than 1 message to be sent to self';call err__("vdf/os-wall-comm.f90",898)
        call abort_program( p_err_invalid )
      endif
  ...
  do i = 1, msg_patt%n_recv
    ...
    if ( recv(i)%node == my_node ) then
      ...
      ! sanity check
      if ( self_send_id < 1 ) then
        write(err_buf__,*) 'No message is being sent to self';call err__("vdf/os-wall-comm.f90",933)
        call abort_program( p_err_invalid )
      endif
      ...
      call alloc(recv(i)%buffer, (/bsize/),"vdf/os-wall-comm.f90",941)
    endif
...
subroutine reshape_wall_copy( this, old_lb, new_grid, no_co, send_msg, recv_msg )
  ...
  if ( this%x_dim > 1 ) then
    ...
    if ( msg_patt%n_send > 0 ) call alloc(send, (/msg_patt%n_send/),"vdf/os-wall-comm.f90",998)
    if ( msg_patt%n_recv > 0 ) call alloc(recv, (/msg_patt%n_recv/),"vdf/os-wall-comm.f90",999)
    ...
    if ( msg_patt%n_recv > 0) then
      call cleanup( recv )
      call freemem(recv,"vdf/os-wall-comm.f90",1040)
    endif
    ...
    if ( msg_patt%n_send > 0) then
       ...
       ! clear send buffers
       call cleanup(send)
       call freemem(send,"vdf/os-wall-comm.f90",1049)
    endif
...
subroutine shift_range_wall( this, shift )
  ...
  select case ( this%x_dim )
    case(1)
       ...
       call alloc(old_f1, lb, ub,"vdf/os-wall-comm.f90",1116)

       ! copy values to temp buffer
       old_f1 = this%f1

       ! reallocate wall buffer
       call freemem(this%f1,"vdf/os-wall-comm.f90",1122)

       lb(1 + this%idir) = this%range(1)
       ub(1 + this%idir) = this%range(2)

       call alloc(this%f1, lb, ub,"vdf/os-wall-comm.f90",1127)

       ! copy values back to wall buffer
       this%f1 = old_f1

       ! cleanup temp buffer
       call freemem(old_f1,"vdf/os-wall-comm.f90",1133)
    ...
    case(2)
       ...
       call alloc(old_f2, lb, ub,"vdf/os-wall-comm.f90",1140)

       ! copy values to temp buffer
       old_f2 = this%f2

       ! reallocate wall buffer
       call freemem(this%f2,"vdf/os-wall-comm.f90",1146)

       lb(1 + this%idir) = this%range(1)
       ub(1 + this%idir) = this%range(2)

       call alloc(this%f2, lb, ub,"vdf/os-wall-comm.f90",1151)

       ! copy values back to wall buffer
       this%f2 = old_f2

       ! cleanup temp buffer
       call freemem(old_f2,"vdf/os-wall-comm.f90",1157)
    ...
    case(3)
       ...
       call alloc(old_f3, lb, ub,"vdf/os-wall-comm.f90",1164)

       ! copy values to temp buffer
       old_f3 = this%f3

       ! reallocate wall buffer
       call freemem(this%f3,"vdf/os-wall-comm.f90",1170)

       lb(1 + this%idir) = this%range(1)
       ub(1 + this%idir) = this%range(2)

       call alloc(this%f3, lb, ub,"vdf/os-wall-comm.f90",1175)

       ! copy values back to wall buffer
       this%f3 = old_f3

       ! cleanup temp buffer
       call freemem(old_f3,"vdf/os-wall-comm.f90",1181)
