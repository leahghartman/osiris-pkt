# 1 "qed-cyl/os-photons-qed-cyl.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "qed-cyl/os-photons-qed-cyl.f03" 2
# 1 "./os-config.h" 1
...
# 2 "qed-cyl/os-photons-qed-cyl.f03" 2
...
# 3 "qed-cyl/os-photons-qed-cyl.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "qed-cyl/os-photons-qed-cyl.f03" 2
...
!---------------------------------------------------------------------------------------------------
    ...
    if ( local_idx%buf_size > 0 ) call freemem(local_idx%idx,"qed-cyl/os-photons-qed-cyl.f03",305)
    local_idx%start = 1
    local_idx%nidx = 0
    ...
    subroutine add_particle_index( list, i )
        ...
        if ( list%nidx + 1 > list%buf_size ) then
            ...
            call alloc(tmp_idx, (/ list%buf_size /),"qed-cyl/os-photons-qed-cyl.f03",327)
            if ( list%nidx > 0 ) call memcpy( tmp_idx, list%idx, list%nidx )
            call freemem(list%idx,"qed-cyl/os-photons-qed-cyl.f03",329)
            list%idx => tmp_idx
        endif
...
subroutine create_pairs_betheheitler_qedcyl( this, electrons, positrons, dt, i0, i1)
    !---------------------------------------------------------------------------------------------------
        ...
        ! cleanup the local_idx buffer for deleted photons
        if ( local_idx%buf_size > 0 ) call freemem(local_idx%idx,"qed-cyl/os-photons-qed-cyl.f03",463)
        local_idx%start = 1
        local_idx%nidx = 0
        local_idx%buf_size = 0
        ...
        subroutine add_particle_index( list, i )
            ...
            if ( list%nidx + 1 > list%buf_size ) then
                list%buf_size = list%buf_size + p_spec_buf_block
                call alloc(tmp_idx, (/ list%buf_size /),"qed-cyl/os-photons-qed-cyl.f03",485)
                if ( list%nidx > 0 ) call memcpy( tmp_idx, list%idx, list%nidx )
                call freemem(list%idx,"qed-cyl/os-photons-qed-cyl.f03",487)
                list%idx => tmp_idx
            endif
...
subroutine advance_photons_qedcyl( this, electrons, positrons, emf, gdt, tid, n_threads)
    ...
    select case ( this%coordinates )
        case default
            write(err_buf__,*) 'Please use cylindrical coordinates';call err__("qed-cyl/os-photons-qed-cyl.f03",560)
