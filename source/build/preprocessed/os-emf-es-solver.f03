# 1 "emf/os-emf-es-solver.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "emf/os-emf-es-solver.f03" 2
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
  ...
  call alloc(lnx_p, (/ 2, rho%x_dim_, no_num(no_co) /),"emf/os-emf-es-solver.f03",96)
  ...
  select case (rho%x_dim_)
    case(1)
       write(err_buf__,*) '1D ES field solver not implemented yet';call err__("emf/os-emf-es-solver.f03",105)
       call abort_program( p_err_notimplemented )
    case(2)
       call es_solver_2d( rho, ef, lnx_p, no_co )

    case(3)
       write(err_buf__,*) '3D ES field solver not implemented yet';call err__("emf/os-emf-es-solver.f03",111)
       call abort_program( p_err_notimplemented )

  end select

  call freemem(lnx_p,"emf/os-emf-es-solver.f03",116)
...
!-----------------------------------------------------------------------------------------
  ...
  do i = 1, no_num(no_co)
    ...
    call alloc(charge, lb, ub,"emf/os-emf-es-solver.f03",170)
    ...
    ! free temp memory
    call freemem(charge,"emf/os-emf-es-solver.f03",215)
...
!-----------------------------------------------------------------------------------------
  ...
  ! get size of the grid on each node
  call alloc(lnx_p, (/ 2, rho%x_dim_, no_num(no_co) /),"emf/os-emf-es-solver.f03",258)
  ...
  select case (rho%x_dim_)
    case(1)
       write(err_buf__,*) '1D ES field solver (beam) not implemented yet';call err__("emf/os-emf-es-solver.f03",267)
       call abort_program( p_err_notimplemented )
    case(2)
  ...
  call freemem(lnx_p,"emf/os-emf-es-solver.f03",282)
...
!-----------------------------------------------------------------------------------------
  ...
  do i = 1, no_num(no_co)
    ...
    call alloc(charge, lb, ub,"emf/os-emf-es-solver.f03",349)
    ...
    ! free temp memory
    call freemem(charge,"emf/os-emf-es-solver.f03",399)
  ...
  if ( no_num(no_co) == 1 ) then
    ...
    enddo
     ...
     do i = 1, newsize
       ...
       ! copy local charge of node being processed to all nodes
       call alloc(charge, lb, ub,"emf/os-emf-es-solver.f03",550)
       if ( myrank + 1 == i ) then
         do ri2 = 1, rho%nx_(2)
           do ri1 = 1, rho%nx_(1)
             charge( lnx0(1) + ri1, lnx0(2) + ri2 ) = rho%f2(1,ri1,ri2)
       ...
       ! free temp memory
       call freemem(charge,"emf/os-emf-es-solver.f03",577)
  ...
  if ( no_num(no_co) == 1 ) then
    ...
    ! Create new communicators will all nodes that lie in the same x1
     ...
     do i = 1, newsize
       ...
       ! copy local charge of node being processed to all nodes
       call alloc(charge, lb, ub,"emf/os-emf-es-solver.f03",714)
       if ( myrank + 1 == i ) then
         do ri2 = 1, rho%nx_(2)
           do ri1 = 1, rho%nx_(1)
             charge( lnx0(1) + ri1, lnx0(2) + ri2 ) = rho%f2(1,ri1,ri2)
       ...
       ! free temp memory
       call freemem(charge,"emf/os-emf-es-solver.f03",744)
...
subroutine es_solver_3d_beam( rho, ef, bf, u1, nx_p, no_co, send_msg, recv_msg )
  ...
  if ( no_num(no_co) == 1 ) then
    ...
    do i = 1, newsize
      ...
      call alloc(charge, lb, ub,"emf/os-emf-es-solver.f03",910)
      ...
      ! free temp memory
      call freemem(charge,"emf/os-emf-es-solver.f03",959)
    enddo
