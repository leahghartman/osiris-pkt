# 1 "pkt/os-photons-define-pkt.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "pkt/os-photons-define-pkt.f03" 2
...
# 2 "pkt/os-photons-define-pkt.f03" 2
# 1 "./os-config.h" 1
...
# 3 "pkt/os-photons-define-pkt.f03" 2

module m_photons_define_pkt
...
# 19 "./memory/memory.h"
use memory
# 7 "pkt/os-photons-define-pkt.f03" 2
...
!-----------------------------------------------------------------------------------------
type, extends( t_species_pkt ) :: t_photons_pkt
...
contains
  ...
  procedure :: advance => advance_photons_pkt
...
interface
  subroutine advance_photons_pkt( this, den_ph, grad_ph, den_grid, grad_grid, gdt, tid, n_threads )
    ...
    use m_vdf_define
    import t_photons_pkt, t_species_pkt, t_emf, p_double, p_k_part, p_k_fld
    ...
    do i = 1, p_x_dim
        do j = 1, 2
            if (this%bnd_con%type(j,i) == p_bc_thermal) then
                write(err_buf__,*) "Invalid boundary conditions for photon species.";call err__("pkt/os-photons-define-pkt.f03",141)
...
subroutine list_algorithm_photons( this )
  ...
  implicit none
  class( t_photons_pkt ), intent(in) :: this

  if (mpi_node()==0) print *,"Species : ", trim(this%name)
  print *, '   - Photon pkt pusher'
...
subroutine inject_photons( this, ig_xbnd_inj, jay, no_co, bnd_cross, node_cross, send_msg, recv_msg )
  ...
  ! Photons injection is not yet implemented, they are only created from pkt processes
...
!-----------------------------------------------------------------------------------------
  ...
  subroutine bad_photon( k, self, ilb, iub, msg )
    ...
    class( t_photons_pkt ), intent(in) :: self
  ...
  !-----------------------------------------------------------------------------------------
# 490 "pkt/os-photons-define-pkt.f03"
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
  subroutine read_checkpoint_photons( this, restart_handle )
    ...
    call check_error(ierr,err_msg,p_err_rstrd,"pkt/os-photons-define-pkt.f03",505)

    ! check if restart file is compatible
    if ( rst_id /= p_photons_rst_id) then
      write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("pkt/os-photons-define-pkt.f03",509)
      write(err_buf__,*) 'from incompatible binary (photons)';call err__("pkt/os-photons-define-pkt.f03",510)
      call abort_program(p_err_rstrd)
    endif

    ! read checkpoint data
    call restart_io_read(this%num_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"pkt/os-photons-define-pkt.f03",516)

    call restart_io_read(this%weight_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"pkt/os-photons-define-pkt.f03",519)

    call restart_io_read(this%energy_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"pkt/os-photons-define-pkt.f03",522)
  ...
  subroutine write_checkpoint_photons( this, restart_handle )
    ...
    call check_error(ierr,err_msg,p_err_rstwrt,"pkt/os-photons-define-pkt.f03",545)

    ! write checkpoint data
    call restart_io_write("this%num_pairs", this%num_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"pkt/os-photons-define-pkt.f03",549)

    call restart_io_write("this%weight_pairs", this%weight_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"pkt/os-photons-define-pkt.f03",552)

    call restart_io_write("this%energy_pairs", this%energy_pairs, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"pkt/os-photons-define-pkt.f03",555)
...
end module m_photons_define_pkt
