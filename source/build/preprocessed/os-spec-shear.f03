# 1 "shear/os-spec-shear.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "shear/os-spec-shear.f03" 2
...
subroutine advance_deposit_emf_shear( this, emf_shear, jay, t, tstep, tid, n_threads )
  ...
  select case ( this%coordinates )
    ...
    case default
      ...
      case default
        write(err_buf__,*) 'Not implemented for x_dim = ',p_x_dim;call err__("shear/os-spec-shear.f03",180)
        call abort_program(p_err_invalid)
...
!-----------------------------------------------------------------------------------------
  ...
  if ( this%num_pistons > 0 ) then
    write(err_buf__,*) 'Pistons are currently offline';call err__("shear/os-spec-shear.f03",239)
    call abort_program( p_err_invalid )
  endif
...
!-----------------------------------------------------------------------------------------
  ...
  dt = real( gdt, p_k_part )
# 459 "shear/os-spec-shear.f03"
  if ( this%num_pistons > 0 ) then
    write(err_buf__,*) 'Pistons are currently offline';call err__("shear/os-spec-shear.f03",460)
    call abort_program( p_err_invalid )
  endif
...
!---------------------------------------------------------------------------------------------------
  ...
  dt = real( gdt, p_k_part )
# 683 "shear/os-spec-shear.f03"
  if ( this%num_pistons > 0 ) then
    write(err_buf__,*) 'Pistons are currently offline';call err__("shear/os-spec-shear.f03",684)
    call abort_program( p_err_invalid )
  endif
