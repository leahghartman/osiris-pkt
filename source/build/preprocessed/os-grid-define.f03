# 1 "grid/os-grid-define.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "grid/os-grid-define.f03" 2
!-------------------------------------------------------------------------------
...
# 17 "grid/os-grid-define.f03" 2
  ...
  ! exported symbols
  public :: t_grid, t_grid_arr, t_grid_io, t_msg, t_msg_patt
...
subroutine cleanup_msg_patt( patt )
  ...
  if ( patt%n_send>0 ) then
    do i = 1, patt%n_send
      call freemem(patt%send(i)%tils,"grid/os-grid-define.f03",303)
      call freemem(patt%send(i)%sub_type,"grid/os-grid-define.f03",304)
      call freemem(patt%send(i)%buffer,"grid/os-grid-define.f03",305)
    enddo

    call freemem(patt%send,"grid/os-grid-define.f03",308)
  endif

  if ( patt%n_recv>0 ) then
    do i = 1, patt%n_recv
      call freemem(patt%recv(i)%tils,"grid/os-grid-define.f03",313)
      call freemem(patt%recv(i)%sub_type,"grid/os-grid-define.f03",314)
      call freemem(patt%recv(i)%buffer,"grid/os-grid-define.f03",315)
    enddo

    call freemem(patt%recv,"grid/os-grid-define.f03",318)
  endif
...
subroutine init_grid( this, no_co, gc_num, g_box, restart, restart_handle )
  ...
  if ( .not. restart ) then
    ...
    call alloc(this%nx_node, (/ 3, nx_node_size /),"grid/os-grid-define.f03",438)
...
subroutine read_input_grid( this, input_file, partition )
  ...
  case ( "cartesian" )
    if (mpi_node()==0) print *,'cartesian coordinates'
...
subroutine write_checkpoint_grid( this, restart_handle )
  ...
  call restart_io_write("p_grid_rst_id", p_grid_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",795)

  call restart_io_write("this%x_dim", this%x_dim, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",798)

  call restart_io_write("this%g_nx", this%g_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",801)

  call restart_io_write("this%coordinates", this%coordinates, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",804)

  call restart_io_write("this%nnodes", this%nnodes, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",807)

  call restart_io_write("this%g_nx", this%g_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",810)

  call restart_io_write("this%min_nx", this%min_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",813)

  call restart_io_write("this%max_nx", this%max_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",816)

  call restart_io_write("this%nx_node", this%nx_node, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",819)

  call restart_io_write("this%my_nx", this%my_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",822)


  ! - Dynamic load balance parameters
  call restart_io_write("this%lb_type", this%lb_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",827)

  call restart_io_write("this%lb_gather_max", this%lb_gather_max, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",830)

  call restart_io_write("this%load_balance", this%load_balance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",833)

  call restart_io_write("this%n_dynamic", this%n_dynamic, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",836)

  call restart_io_write("this%start_load_balance", this%start_load_balance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",839)

  call restart_io_write("this%balance_on_start", this%balance_on_start, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",842)

  call restart_io_write("this%max_imbalance", this%max_imbalance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",845)

  call restart_io_write("this%cell_weight", this%cell_weight, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",848)

  ! - Global box (phsysical) dimensions
  call restart_io_write("this%g_box", this%g_box, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",852)
...
subroutine read_checkpoint_grid( this, restart_handle )
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"grid/os-grid-define.f03",878)
  ...
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",887)

  call restart_io_read(this%g_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",890)

  call restart_io_read(this%coordinates, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",893)

  call restart_io_read(this%nnodes, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",896)

  call restart_io_read(this%g_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",899)

  call restart_io_read(this%min_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",902)

  call restart_io_read(this%max_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",905)
  ...
  call alloc(this%nx_node, (/ 3, nx_node_size /),"grid/os-grid-define.f03",912)
  call restart_io_read(this%nx_node, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"grid/os-grid-define.f03",914)

  call restart_io_read(this%my_nx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",917)


  ! - Dynamic load balance parameters
  call restart_io_read(this%lb_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",922)

  call restart_io_read(this%lb_gather_max, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",925)

  call restart_io_read(this%load_balance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",928)

  call restart_io_read(this%n_dynamic, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",931)

  call restart_io_read(this%start_load_balance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",934)

  call restart_io_read(this%balance_on_start, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",937)

  call restart_io_read(this%max_imbalance, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",940)

  call restart_io_read(this%cell_weight, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",943)

  ! - Global box (phsysical) dimensions
  call restart_io_read(this%g_box, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"grid/os-grid-define.f03",947)
...
subroutine cleanup_grid(this)
  ...
  call freemem(this%nx_node,"grid/os-grid-define.f03",962); this % nx_node => null()
  call freemem(this%int_load,"grid/os-grid-define.f03",963); this % int_load => null()
...
subroutine cleanup_grid_io( this )
  ...
  call freemem(this % tiles,"grid/os-grid-define.f03",982)
  this % tiles => null()
...
subroutine init_grid_io( this, grid, no_co )
  ...
  if ( this % merge_type /= p_none .and. no_co % comm_size() > 1) then
    ...
    if ( this % group_size /= j ) then
      print *, "[",mpi_node(),"] ",'(*error*) Invalid group size '
    ...
    if ( this % group_rank == 0 ) then
      ! Sanity check
      if ( global_rank /= group_leader_rank ) then
        print *, "[",mpi_node(),"] ",'(*error*) Group rank 0 is not group leader '
        call abort_program( p_err_invalid )
      endif

      call alloc(this % tiles, [ 2, grid % x_dim, this % group_size ],"grid/os-grid-define.f03",1093)
    else
      this%tiles => null()
    endif
...
subroutine copy_grid(this, from)
    ...
    call alloc(this%nx_node, (/ 3, size( from%nx_node, 2 ) /),"grid/os-grid-define.f03",1318)
    ...
    if ( associated( from%int_load) ) then
      call alloc(this%int_load, [ size(from%int_load) ],"grid/os-grid-define.f03",1333)
...
subroutine alloc_msg( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_msg ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_msg" , &
                        bsize, &
                        file, line )

end subroutine alloc_msg



subroutine alloc_1d_msg( p, n, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_msg ), dimension(:), pointer :: p
...
# 143 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof( p )
  call log_allocation( addr, "t_msg" // '('// trim(strrange(1,n)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_1d_msg
# 270 "./memory/mem-template.h"
subroutine alloc_bound_1d_msg( p, lb, ub, file, line, stat )
...
# 329 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof(p)
  call log_allocation( addr, "t_msg" // '('// trim(strrange(1,lb,ub)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_bound_1d_msg



subroutine freemem_msg( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   call log_deallocation( addr, "t_msg", &
                        bsize, file, line )
...
end subroutine freemem_msg



subroutine freemem_1d_msg( p, file, line, stat )
...
end subroutine freemem_1d_msg
# 1353 "grid/os-grid-define.f03" 2
