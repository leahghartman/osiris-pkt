# 1 "grid/os-grid-parallel.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "grid/os-grid-parallel.f90" 2
!#define __DEBUG__ 1
...
# 19 "./memory/memory.h"
use memory
# 10 "grid/os-grid-parallel.f90" 2
...
subroutine random_dist( nx, nnodes, g_nx, min_nx )
    ...
    call alloc(old_seed, (/ seed_size /),"grid/os-grid-parallel.f90",107)
    call random_seed( get = old_seed )

    seed = seed + 1
    call alloc(new_seed, (/ seed_size /),"grid/os-grid-parallel.f90",111)
    do i = 1, seed_size
        new_seed(i) = 2*seed+i
    enddo
    ...
    call alloc(rnx, (/ nnodes /),"grid/os-grid-parallel.f90",122)
    ...
    call freemem(rnx,"grid/os-grid-parallel.f90",148)

    ! Restore random number generator to previous state
    call random_seed( put = old_seed )
    call freemem(new_seed,"grid/os-grid-parallel.f90",152)
    call freemem(old_seed,"grid/os-grid-parallel.f90",153)
...
recursive subroutine rec_tree_dist( nx, int_load, lnode, unode, min_nx )
    ...
    ! (* debug *)
    if ( range < 0 ) then
        print *, "[",mpi_node(),"] ",'Invalid range for calculating node distribution'
        call abort_program()
    endif
    ...
    ! print *, "[",mpi_node(),"] ",'Invalid split point, beyond max_split'
    ! call abort_program()
    ! endif
...
subroutine init_int_load( this )
    ...
    call freemem(this%int_load,"grid/os-grid-parallel.f90",411)
    ...
    call alloc(this%int_load, (/ size /),"grid/os-grid-parallel.f90",418)
...
subroutine clear_int_load( this )
    ...
    call freemem(this%int_load,"grid/os-grid-parallel.f90",437)
    this%int_load => null()
...
subroutine conv_load_2_int_load( this )
    ...
    do dim = 1, this%x_dim
        do i = idx+1, idx+this%g_nx(dim)
            if ( this%int_load(i) < 0 ) then
                print *, "[",mpi_node(),"] ",'Invalid value for int_load (A), dim = ', 1, ', pos = ', i-idx
    ...
    do dim = 1, this%x_dim
        do i = idx+1, idx+this%g_nx(dim)
            if ( this%int_load(i) < 0 ) then
                print *, "[",mpi_node(),"] ",'Invalid value for int_load (B), dim = ', 1, ', pos = ', i-idx
                call abort_program( p_err_invalid )
            endif
        enddo
        idx = idx + this%g_nx(dim)
...
subroutine add_expression_load( this )
    ...
    if (mpi_node()==0) print *,"   - Adding expression defined load"
...
subroutine check_consistency( grid, no_co )
    ...
    comm(no_co), ierr)
    if ( ierr /= 0 ) then
        write(err_buf__,*) "MPI Error";call err__("grid/os-grid-parallel.f90",694)
    ...
    call alloc(nx_node, (/ 3, tnodes /),"grid/os-grid-parallel.f90",709)
    call mpi_reduce(grid%nx_node, nx_node, 3*tnodes, MPI_INTEGER, MPI_MAX, 0, &
    comm(no_co), ierr)
    if (ierr/=0) then
        write(err_buf__,*) "MPI error";call err__("grid/os-grid-parallel.f90",713)
        call abort_program( p_err_mpi )
    endif
    if ( root( no_co ) ) then
        do j = 1, tnodes
    ...
    ! cleanup used memory
    call freemem(nx_node,"grid/os-grid-parallel.f90",730)
...
subroutine get_dyn_msg( n_msg, msg, from_grid, to_grid, no_co, &
    ...
    do dim = 2, from_grid%x_dim
        ...
        call alloc(msg_nodes, (/ no_num(no_co) /),"grid/os-grid-parallel.f90",849)
        ...
        ! Sanity check
        if ( n_msg == 0 ) then
            write(err_buf__,*) 'Number of messages is 0';call err__("grid/os-grid-parallel.f90",883)
            call abort_program()
        endif

        ! allocate message pattern (n_msg will always be > 0)
        call alloc(msg, (/ n_msg /),"grid/os-grid-parallel.f90",888)
        ...
        call freemem(msg_nodes,"grid/os-grid-parallel.f90",948)
    ...
    do i = 1, this%x_dim
        ...
        do j = 1, this%nnodes(i)
            if ( this%nx_node(3,j+idx_node) < this%min_nx(i) ) then
                write(err_buf__,*) 'Partition too small';call err__("grid/os-grid-parallel.f90",1062)
                call abort_program()
            endif
        enddo
        ...
        do j = 2, this%nnodes(i)
            if ( this%nx_node(1,j+idx_node) /= this%nx_node(2,j+idx_node-1) + 1 ) then
                write(err_buf__,*) 'Partition overlap';call err__("grid/os-grid-parallel.f90",1070)
                call abort_program()
            endif
        enddo
