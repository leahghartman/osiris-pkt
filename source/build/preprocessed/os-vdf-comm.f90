# 1 "vdf/os-vdf-comm.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-vdf-comm.f90" 2
!#define __DEBUG__ 1
...
subroutine alloc_vdf_msg( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_vdf_msg ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_vdf_msg" , &
                        bsize, &
                        file, line )

end subroutine alloc_vdf_msg



subroutine alloc_1d_vdf_msg( p, n, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_vdf_msg ), dimension(:), pointer :: p
...
# 143 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof( p )
  call log_allocation( addr, "t_vdf_msg" // '('// trim(strrange(1,n)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_1d_vdf_msg
# 270 "./memory/mem-template.h"
subroutine alloc_bound_1d_vdf_msg( p, lb, ub, file, line, stat )
...
# 329 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof(p)
  call log_allocation( addr, "t_vdf_msg" // '('// trim(strrange(1,lb,ub)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_bound_1d_vdf_msg



subroutine freemem_vdf_msg( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   call log_deallocation( addr, "t_vdf_msg", &
                        bsize, file, line )
...
end subroutine freemem_vdf_msg



subroutine freemem_1d_vdf_msg( p, file, line, stat )
...
end subroutine freemem_1d_vdf_msg
# 135 "vdf/os-vdf-comm.f90" 2
...
subroutine unpack_vdf_msg( msg, vdf, k )
    ...
    case default
        write(err_buf__,*) 'unpack_vdf_msg called with an invalid msg%type';call err__("vdf/os-vdf-comm.f90",468)
        call abort_program()
...
recursive subroutine cleanup_vdf_msg_1( msg )
    ...
    call freemem(msg%buffer,"vdf/os-vdf-comm.f90",490)
...
subroutine cleanup_vdf_msg_array( vdf_msg )
    ...
    do i=1, size(vdf_msg)
        ...
        call freemem(vdf_msg(i)%buffer,"vdf/os-vdf-comm.f90",513)
...
subroutine irecv_vdf_msg( recv, no_co )
    ...
    if ( recv%node /= my_node ) then
        ...
        if (ierr/=0) then
            write(err_buf__,*) "MPI error";call err__("vdf/os-vdf-comm.f90",542)
...
subroutine isend_vdf_msg( send, no_co )
    ...
    if ( send%node /= my_node ) then
        ...
        send%handle, ierr )
        if (ierr/=0) then
            write(err_buf__,*) "MPI error";call err__("vdf/os-vdf-comm.f90",574)
            call abort_program( p_err_mpi )
        endif
...
subroutine wait_vdf_msg_array( vdf_msg )
    ...
    call alloc(status, (/ MPI_STATUS_SIZE, count /),"vdf/os-vdf-comm.f90",603)
    call alloc(request, (/ count /),"vdf/os-vdf-comm.f90",604)
    ...
    if (j > 0) then
        ...
        if (ierr/=0) then
            write(err_buf__,*) "MPI error";call err__("vdf/os-vdf-comm.f90",618)
            call abort_program( p_err_mpi )
        endif
    endif
    ...
    call freemem(request,"vdf/os-vdf-comm.f90",628)
    call freemem(status,"vdf/os-vdf-comm.f90",629)
...
subroutine wait_vdf_msg( vdf_msg )
    ...
    if ( vdf_msg%handle /= MPI_REQUEST_NULL ) then
        ...
        if (ierr/=0) then
            write(err_buf__,*) "MPI error";call err__("vdf/os-vdf-comm.f90",651)
            call abort_program( p_err_mpi )
        endif
...
subroutine isend_vdf( vdf, update_type, dim, bnd, no_co, nx_move, send_msg, vdf_b )
    ...
    if ( send_msg(bnd)%handle /= MPI_REQUEST_NULL ) then
        write(err_buf__,*) 'isend_msg_vdf called when message still waiting to complete';call err__("vdf/os-vdf-comm.f90",746)
        call abort_program( p_err_invalid )
    endif

    if ( neighbor( no_co, bnd, dim ) < 0 ) then
        write(0,*) 'isend_msg_vdf called without a valid neighbor, dim, bnd = ',dim, bnd, &
        trim("vdf/os-vdf-comm.f90"), ':', 752
        call abort_program( p_err_invalid )
    endif
    ...
    if ( msg_size > bsize ) then
        if ( bsize > 0 ) call freemem(send_msg(bnd)%buffer,"vdf/os-vdf-comm.f90",803)
        call alloc(send_msg(bnd)%buffer, (/msg_size/),"vdf/os-vdf-comm.f90",804)
    endif
...
subroutine irecv_vdf( vdf, update_type, dim, bnd, no_co, nx_move, recv_msg, vdf_b )
    ...
    if ( recv_msg( bnd )%handle /= MPI_REQUEST_NULL ) then
        write(err_buf__,*) 'irecv_vdf called when message still waiting to complete';call err__("vdf/os-vdf-comm.f90",855)
        call abort_program( p_err_invalid )
    endif
    ...
    ! (* debug *)
    if ( msg_size == 0 ) then
        write(err_buf__,*) 'Invalid message size, aborting.';call err__("vdf/os-vdf-comm.f90",898)
        call abort_program( p_err_invalid )
    endif
    ...
    if ( msg_size > bsize ) then
        if ( bsize > 0 ) call freemem(recv_msg(bnd)%buffer,"vdf/os-vdf-comm.f90",927)
        call alloc(recv_msg(bnd)%buffer, (/msg_size/),"vdf/os-vdf-comm.f90",928)
    endif
...
subroutine reshape_vdf_nocopy( this, new_grid, dbg )
    ...
    if (this%x_dim_ > 0) then
        ...
        select case (x_dim)
        case(1)
            call freemem(this%f1,"vdf/os-vdf-comm.f90",1017)
            call alloc(this%f1, lower, upper,"vdf/os-vdf-comm.f90",1018)
        ...
        case(2)
            ...
            call freemem(this%f2,"vdf/os-vdf-comm.f90",1027)
            call alloc(this%f2, lower, upper,"vdf/os-vdf-comm.f90",1028)
        ...
        case(3)
            ...
            call freemem(this%f3,"vdf/os-vdf-comm.f90",1037)
            call alloc(this%f3, lower, upper,"vdf/os-vdf-comm.f90",1038)
...
subroutine init_msg_buffers( this, msg_patt, send, recv, my_node )
    ...
    do i = 1, msg_patt%n_send
        ...
        call alloc(send(i)%buffer, (/bsize/),"vdf/os-vdf-comm.f90",1094)
        ...
        if (send(i)%node == my_node) then
            ...
            if ( self_send_id >= 1 ) then
                write(err_buf__,*) 'More than 1 message to be sent to self';call err__("vdf/os-vdf-comm.f90",1107)
                call abort_program( p_err_invalid )
            endif
    ...
    do i = 1, msg_patt%n_recv
        ...
        if ( recv(i)%node == my_node ) then
            ...
            ! sanity check
            if ( self_send_id < 1 ) then
                write(err_buf__,*) 'No message is being sent to self';call err__("vdf/os-vdf-comm.f90",1139)
                call abort_program( p_err_invalid )
            endif
            ...
            call alloc(recv(i)%buffer, (/bsize/),"vdf/os-vdf-comm.f90",1147)
        endif
...
subroutine reshape_vdf_copy( this, old_lb, new_grid, no_co, send_msg, recv_msg )
    ...
    if ( msg_patt%n_send > 0 ) call alloc(send, (/msg_patt%n_send/),"vdf/os-vdf-comm.f90",1183)
    if ( msg_patt%n_recv > 0 ) call alloc(recv, (/msg_patt%n_recv/),"vdf/os-vdf-comm.f90",1184)
    ...
    if ( msg_patt%n_recv > 0) then
        ...
        call freemem(recv,"vdf/os-vdf-comm.f90",1227)
    endif
    ...
    if ( msg_patt%n_send > 0) then
        ...
        call freemem(send,"vdf/os-vdf-comm.f90",1237)
    endif
