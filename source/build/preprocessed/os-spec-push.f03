# 1 "spec/os-spec-push.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/os-spec-push.f03" 2
!-----------------------------------------------------------------------------------------
! Particle pusher
!-----------------------------------------------------------------------------------------
...
interface dudt_boris_gravity
  module procedure dudt_boris_gravity
end interface
# 64 "spec/os-spec-push.f03"
interface advance_deposit_1d
  module procedure advance_deposit_1d
end interface

interface advance_deposit_2d
  module procedure advance_deposit_2d
end interface

interface advance_deposit_3d
  module procedure advance_deposit_3d
end interface
...
public :: advance_deposit_1d, advance_deposit_2d, advance_deposit_3d, advance_deposit

public :: dudt_boris, dudt_vay, dudt_fullrot, dudt_euler
public :: dudt_cond_vay, dudt_cary, dudt_boris_gravity
...
!-----------------------------------------------------------------------------------------
subroutine dudt_boris_gravity( this, emf, dt, i0, i1, energy, t )
!-----------------------------------------------------------------------------------------
! Advance velocities and calculate time-centered total energy
! process particles in the range [i0,i1]
! This routine is meant to be used for testing simd code and is written to use the same numerics
!-----------------------------------------------------------------------------------------
  ...
  real(p_double) :: g1dt, g2dt, g3dt
  real(p_double) :: g1dt2, g2dt2, g3dt2, dt2

  real(p_k_part), dimension(p_p_dim) :: fp, vtmp
  real(p_k_part) :: f2, gam, v_dot_e, fac, cor

  ! push gravity and return
  if ( this%free_stream ) then
    g1dt = this%gravity(1) * dt
    g2dt = this%gravity(2) * dt
    g3dt = this%gravity(3) * dt
    ...
    do ptrcur = i0, i1, p_cache_size
      ...
      pp = ptrcur
      do i=1,np
        this%p(1,pp) = this%p(1,pp) + g1dt
        this%p(2,pp) = this%p(2,pp) + g2dt
        this%p(3,pp) = this%p(3,pp) + g3dt
        pp = pp + 1
      enddo
    ...
    ! specify coefficients for use later
    dt2 = real( 0.5_p_double * dt, p_k_part ) ! for gravity push
    g1dt2 = this%gravity(1) * dt2
    g2dt2 = this%gravity(2) * dt2
    g3dt2 = this%gravity(3) * dt2
  endif
...
# 1422 "spec/os-spec-push.f03"
    ...
    pp = ptrcur
    do i=1,np
      utemp(1,i) = this%p(1,pp) + ep(1,i) + g1dt2
      utemp(2,i) = this%p(2,pp) + ep(2,i) + g2dt2
      utemp(3,i) = this%p(3,pp) + ep(3,i) + g3dt2

      ! Get time centered gamma
      u2 = (utemp(1,i)**2 + utemp(2,i)**2) + utemp(3,i)**2
# 1451 "spec/os-spec-push.f03"
      gamma = sqrt( u2 + 1 )
    ...
    pp = ptrcur
    do i=1,np
      this%p(1,pp) = utemp(1,i) + ep(1,i) + g1dt2
      this%p(2,pp) = utemp(2,i) + ep(2,i) + g2dt2
      this%p(3,pp) = utemp(3,i) + ep(3,i) + g3dt2
      pp = pp + 1
    end do
...
end subroutine dudt_boris_gravity
!-----------------------------------------------------------------------------------------
# 1665 "spec/os-spec-push.f03"
!-----------------------------------------------------------------------------------------
function ntrim(x)
!-----------------------------------------------------------------------------------------
...
subroutine advance_deposit( this, emf, current, t, tstep, tid, n_threads )
  ...
  select case ( this%coordinates )
    ...
    case default
      ...
      case default
        write(err_buf__,*) 'Not implemented for x_dim = ',p_x_dim;call err__("spec/os-spec-push.f03",2132)
