# 1 "emf/boundary/os-emf-boundary.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "emf/boundary/os-emf-boundary.f90" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 6 "emf/boundary/os-emf-boundary.f90" 2
...
# 7 "emf/boundary/os-emf-boundary.f90" 2
  ...
  if ( restart ) then
     ...
     if ( grid % coordinates == p_cylindrical_b ) then
        ...
        if ((this%type(p_lower, p_r_dim) == p_bc_periodic) .or. &
           ...
           write(err_buf__,*) 'no periodic b.c. for radial direction';call err__("emf/boundary/os-emf-boundary.f90",455)
    ...
    do i_bnd = p_lower, p_upper
      if ( this%type( i_bnd, i_x_dim ) == p_bc_lindman ) then
        ...
        if ( previous > 0 .and. previous /= i_x_dim ) then
           write(err_buf__,*) "Lindman boundaries are only allowed in 1 direction";call err__("emf/boundary/os-emf-boundary.f90",496)
           call abort_program( p_err_invalid )
        endif
...
!---------------------------------------------------
  ...
  call restart_io_write("p_bcemf_rst_id", p_bcemf_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/boundary/os-emf-boundary.f90",581)

  call restart_io_write("this%type", this%type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/boundary/os-emf-boundary.f90",584)

  call restart_io_write("this%global_type", this%global_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/boundary/os-emf-boundary.f90",587)
...
!---------------------------------------------------
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"emf/boundary/os-emf-boundary.f90",631)

  if ( rst_id /= p_bcemf_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("emf/boundary/os-emf-boundary.f90",634)
    write(err_buf__,*) 'from incompatible binary (emf_bound)';call err__("emf/boundary/os-emf-boundary.f90",635)
    write(err_buf__,*) 'rst_id = "', rst_id, '"';call err__("emf/boundary/os-emf-boundary.f90",636)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/boundary/os-emf-boundary.f90",641)

  call restart_io_read(this%global_type, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/boundary/os-emf-boundary.f90",644)
...
subroutine reshape_emf_bound( this, old_lb, new_lb, no_co, send_msg, recv_msg )
  ...
  use m_parameters, only : p_lower, p_upper
  ...
  use m_lindman, only : is_active, reshape_obj
  use m_vpml, only : is_active, reshape_obj
