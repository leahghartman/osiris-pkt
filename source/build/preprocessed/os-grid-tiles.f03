# 1 "tiles/os-grid-tiles.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "tiles/os-grid-tiles.f03" 2
...
subroutine init_grid_tiles( this, no_co, gc_num, g_box, restart, restart_handle )
  ...
  if ( .not. restart ) then
    ...
    class default
      write(err_buf__,*) 'Should only have t_node_conf_tiles type here';call err__("tiles/os-grid-tiles.f03",477)
      call abort_program()
    end select
...
subroutine cleanup_grid_tiles(this)
  ...
  ! call freemem(this%nx_node,"tiles/os-grid-tiles.f03",510)
  call freemem(this%int_load,"tiles/os-grid-tiles.f03",511)
...
subroutine parallel_partition_tiles( this, no_co, nx_til )
  ...
  if (associated(this%nx_node)) then
    call freemem(this%nx_node,"tiles/os-grid-tiles.f03",699)
  endif
  ...
  do i = 1, this%x_dim
    ...
    do j = 1, this%nnodes(i)
      if ( this%nx_node(3,j+idx_node) < this%min_nx(i) ) then
        write(err_buf__,*) 'Partition too small';call err__("tiles/os-grid-tiles.f03",727)
        call abort_program()
      endif
    enddo
    ...
    do j = 2, this%nnodes(i)
      if ( this%nx_node(1,j+idx_node) /= this%nx_node(2,j+idx_node-1) + 1 ) then
        write(err_buf__,*) 'Partition overlap';call err__("tiles/os-grid-tiles.f03",735)
        call abort_program()
      endif
    enddo
...
subroutine cleanup_grid_group(this)
  ...
  call freemem(this%nx_til,"tiles/os-grid-tiles.f03",776)
  call freemem(this%til_load_2,"tiles/os-grid-tiles.f03",777)
  call freemem(this%til_load_3,"tiles/os-grid-tiles.f03",778)
...
subroutine parallel_partition_group( this, no_co, n )
  ...
  select type(no_co); class is(t_node_conf_tiles)
    ...
    case( p_topology_hilbert, p_topology_snake )
      ...
      call alloc(g_til_aid_mm, (/ 3, no_num(no_co) /),"tiles/os-grid-tiles.f03",848)
      ...
      do j = 1, no_num(no_co)
        if ( g_til_aid_mm(3,j) < min_til ) then
          write(err_buf__,*) 'Partition too small';call err__("tiles/os-grid-tiles.f03",862)
          call abort_program()
        endif
      enddo
      ...
      do j = 2, no_num(no_co)
        if ( g_til_aid_mm(1,j) /= g_til_aid_mm(2,j-1) + 1 ) then
          write(err_buf__,*) 'Partition overlap';call err__("tiles/os-grid-tiles.f03",870)
          call abort_program()
        endif
      enddo
      ...
      call freemem(g_til_aid_mm,"tiles/os-grid-tiles.f03",877)
    ...
    case( p_topology_viktor )
      ...
      if ( this%x_dim == 1 ) then
        write(err_buf__,*) 'For 1d simulations, use topology="snake" or topology="hilbert"';call err__("tiles/os-grid-tiles.f03",886)
        call abort_program()
      endif
    ...
    case default
      ...
      write(err_buf__,*) 'Load balance not implemented for tile topology ',no_co%ti_co%topology_type,',';call err__("tiles/os-grid-tiles.f03",902)
      write(err_buf__,*) 'Try tile_topology = hilbert, snake or viktor.';call err__("tiles/os-grid-tiles.f03",903)
...
subroutine parallel_partition_io(this, no_co)
  ...
  select type(no_co); class is(t_node_conf_tiles)
    ...
    call alloc(this%nx_til, (/3, sum(no_co%ti_co%g_til)/),"tiles/os-grid-tiles.f03",940)

    ! allocate local pointer
    call alloc(nt_node, (/3, sum(this%nnodes(1:p_x_dim))/),"tiles/os-grid-tiles.f03",943)
    ...
    call freemem(nt_node,"tiles/os-grid-tiles.f03",984)
...
subroutine new_dlb_patt_( patt, no_co, g_til_aid_mm_old, g_til_aid_mm_new, diff )
  ...
  call alloc(msg_nodes, (/ num_nodes-1 /),"tiles/os-grid-tiles.f03",1014)
  ...
  ! Cleanup
  call freemem(msg_nodes,"tiles/os-grid-tiles.f03",1045)
...
subroutine get_dlb_msg( msg, n_msg, diff, msg_nodes, my_node, lb, ub, const )
  ...
  if ( n_msg>0 ) then
    call alloc(msg, (/ n_msg /),"tiles/os-grid-tiles.f03",1082)
...
subroutine new_msg( msg_n, node, til_ub, til_lb )
  ...
  ! set tile range
  call alloc(msg_n%tils, (/ til_ub - til_lb + 1 /),"tiles/os-grid-tiles.f03",1145)
  do i = til_lb, til_ub
...
subroutine get_dlb_msg_2d_viktor( no_co, msg, n_msg, diff, lb, ub, my_node, const )
  ...
  n_msg = size(unique,1)
  call alloc(msg, (/ n_msg /),"tiles/os-grid-tiles.f03",1274)
  ...
  do n = 1, n_msg
    ...
    call alloc(msg(n)%tils, (/unique(n,2)/),"tiles/os-grid-tiles.f03",1284)
  ...
  call freemem(unique,"tiles/os-grid-tiles.f03",1300)
...
subroutine get_dlb_msg_3d_viktor( no_co, msg, n_msg, diff, lb, ub, my_node, const )
  ...
  n_msg = size(unique,1)
  call alloc(msg, (/ n_msg /),"tiles/os-grid-tiles.f03",1333)
  ...
  do n = 1, n_msg
    ...
    msg(n)%node = my_node + const*val
    ! number of times that value will appear (number of tiles going to that node)
    call alloc(msg(n)%tils, (/unique(n,2)/),"tiles/os-grid-tiles.f03",1343)
    ! find g_til_aid's and pack them into the msg
    count = 0
    do i = lb(1), ub(1)
      do j = lb(2), ub(2)
  ...
  call freemem(unique,"tiles/os-grid-tiles.f03",1362)
...
subroutine init_tile_load( this, no_co )
  ...
  case( p_topology_viktor )
    ...
    select case( p_x_dim )
    case(2)
      call freemem(this%til_load_2,"tiles/os-grid-tiles.f03",1388)
      call alloc(this%til_load_2, no_co%ti_co%g_til,"tiles/os-grid-tiles.f03",1389)
      this%til_load_2 = 0
    case(3)
      call freemem(this%til_load_3,"tiles/os-grid-tiles.f03",1392)
      call alloc(this%til_load_3, no_co%ti_co%g_til,"tiles/os-grid-tiles.f03",1393)
      this%til_load_3 = 0
    end select

  case default

    call freemem(this%int_load,"tiles/os-grid-tiles.f03",1399)

    size = product(no_co%ti_co%g_til)
    call alloc(this%int_load, (/ size /),"tiles/os-grid-tiles.f03",1402)
    do i = 1, size
      this%int_load(i) = 0
    enddo
...
subroutine add_grid_load_tiles( this, no_co, n, int_load, til_load_2, til_load_3 )
  ...
  if ( this % lb_type == p_expr_load_balance ) then
    ...
    write(err_buf__,*) 'Expression load not yet implemented for tiles';call err__("tiles/os-grid-tiles.f03",1560)
    call abort_program(p_err_notimplemented)
...
subroutine conv_load_2_int_load_tiles( int_load )
  ...
  do i = 1, size(int_load)
    if ( int_load(i) < 0 ) then
      print *, "[",mpi_node(),"] ",'Invalid value for int_load (A), tile = ', i
  ...
  do i = 1, size(int_load)
    if ( int_load(i) < 0 ) then
      print *, "[",mpi_node(),"] ",'Invalid value for int_load (B), tile = ', i
      call abort_program( p_err_invalid )
    endif
  enddo
...
subroutine load_dist_viktor( this, no_co, min_til, cart_til_mm )
  ...
  case(2)
    ...
    call alloc(int_load, (/no_co%ti_co%g_til(1)/),"tiles/os-grid-tiles.f03",1777)
    ...
    ! cleanup objs
    call freemem(int_load,"tiles/os-grid-tiles.f03",1786)

    ! Fill in cart_til_mm(2)%ii
    do i = 1, nx(no_co,1)
      call alloc(int_load, (/no_co%ti_co%g_til(2)/),"tiles/os-grid-tiles.f03",1790)
      ...
      ! cleanup objs
      call freemem(int_load,"tiles/os-grid-tiles.f03",1801)
    enddo
  case(3)
    ! Fill in cart_til_mm(1)%ii
    call alloc(int_load, (/no_co%ti_co%g_til(1)/),"tiles/os-grid-tiles.f03",1805)
    ! project to first dimension
    ...
    call load_dist( this%lb_type, cart_til_mm(1)%ii(:,:,1,1), nx(no_co,1), &
      no_co%ti_co%g_til(1), int_load, min_til )
    ! cleanup objs
    call freemem(int_load,"tiles/os-grid-tiles.f03",1814)

    ! Fill in cart_til_mm(2)%ii
    do i = 1, nx(no_co,1)
      call alloc(int_load, (/no_co%ti_co%g_til(2)/),"tiles/os-grid-tiles.f03",1818)
      ! project node's range of tiles in dim one to second dim
      ...
      call load_dist( this%lb_type, cart_til_mm(2)%ii(:,i,:,1), nx(no_co,2), &
        no_co%ti_co%g_til(2), int_load, min_til )
      ! cleanup objs
      call freemem(int_load,"tiles/os-grid-tiles.f03",1829)

      ! Fill in cart_til_mm(3)%ii
      do k = 1, nx(no_co,2)
        call alloc(int_load, (/no_co%ti_co%g_til(3)/),"tiles/os-grid-tiles.f03",1833)
        ...
        ! cleanup objs
        call freemem(int_load,"tiles/os-grid-tiles.f03",1845)
      enddo
    enddo
  end select
...
subroutine unique_count_1d( list, unique )
  ...
  call alloc(tmp_uniq, (/size(list),2/),"tiles/os-grid-tiles.f03",1916)
  ...
  if ( associated(unique) ) then
    write(err_buf__,*) 'pointer "unique" should not be initialized';call err__("tiles/os-grid-tiles.f03",1932)
    call abort_program(p_err_invalid)
  endif

  if (i>0) then
    call alloc(unique, (/i,2/),"tiles/os-grid-tiles.f03",1937)
    unique = tmp_uniq(1:i,:)
  endif

  call freemem(tmp_uniq,"tiles/os-grid-tiles.f03",1941)
...
subroutine unique_count_2d( list, unique )
  ...
  call alloc(tmp_uniq, (/size(list),2/),"tiles/os-grid-tiles.f03",1961)
  ...
  if ( associated(unique) ) then
    write(err_buf__,*) 'pointer "unique" should not be initialized';call err__("tiles/os-grid-tiles.f03",1977)
    call abort_program(p_err_invalid)
  endif

  if (i>0) then
    call alloc(unique, (/i,2/),"tiles/os-grid-tiles.f03",1982)
    unique = tmp_uniq(1:i,:)
  endif

  call freemem(tmp_uniq,"tiles/os-grid-tiles.f03",1986)
...
subroutine unique_count_3d( list, unique )
  ...
  call alloc(tmp_uniq, (/size(list),2/),"tiles/os-grid-tiles.f03",2006)
  ...
  if ( associated(unique) ) then
    write(err_buf__,*) 'pointer "unique" should not be initialized';call err__("tiles/os-grid-tiles.f03",2022)
    call abort_program(p_err_invalid)
  endif

  if ( i>0 ) then
    call alloc(unique, (/i,2/),"tiles/os-grid-tiles.f03",2027)
    unique = tmp_uniq(1:i,:)
  endif

  call freemem(tmp_uniq,"tiles/os-grid-tiles.f03",2031)
