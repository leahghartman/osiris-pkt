# 1 "spec/diagnostics/os-spec-phasespace.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/diagnostics/os-spec-phasespace.f03" 2
...
# 2 "spec/diagnostics/os-spec-phasespace.f03" 2
# 1 "./os-config.h" 1
...
# 3 "spec/diagnostics/os-spec-phasespace.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "spec/diagnostics/os-spec-phasespace.f03" 2
...
!-----------------------------------------------------------------------------------------
  ...
  do
    ...
    call freemem(list%head,"spec/diagnostics/os-spec-phasespace.f03",2423)
...
!-----------------------------------------------------------------------------------------
  ...
  case default
    write(err_buf__,*) "Invalid phasespace type, or ";call err__("spec/diagnostics/os-spec-phasespace.f03",2720)
    write(err_buf__,*) "phasespace type not implemented";call err__("spec/diagnostics/os-spec-phasespace.f03",2721)
...
subroutine alloc_phasespace( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_phasespace ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_phasespace" , &
                        bsize, &
                        file, line )

end subroutine alloc_phasespace



subroutine alloc_1d_phasespace( p, n, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_phasespace ), dimension(:), pointer :: p
...
# 143 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof( p )
  call log_allocation( addr, "t_phasespace" // '('// trim(strrange(1,n)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_1d_phasespace
# 270 "./memory/mem-template.h"
subroutine alloc_bound_1d_phasespace( p, lb, ub, file, line, stat )
...
# 329 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof(p)
  call log_allocation( addr, "t_phasespace" // '('// trim(strrange(1,lb,ub)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_bound_1d_phasespace



subroutine freemem_phasespace( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   call log_deallocation( addr, "t_phasespace", &
                        bsize, file, line )
...
end subroutine freemem_phasespace



subroutine freemem_1d_phasespace( p, file, line, stat )
...
end subroutine freemem_1d_phasespace
# 2772 "spec/diagnostics/os-spec-phasespace.f03" 2
...
!-----------------------------------------------------------------------------------------
  ...
  case default
    if ( mpi_node() == 0 ) then
      write(0,*) "(*error*) Unsupported phasespace type ", "spec/diagnostics/os-spec-phasespace.f03", ":", 3125
...
!-----------------------------------------------------------------------------------------
  ...
  call asrt__(ndims>0,"ndims>0","spec/diagnostics/os-spec-phasespace.f03",3266)

  ! create the phasespace

  call alloc(phasespace,"spec/diagnostics/os-spec-phasespace.f03",3270)
  ...
  case default
    write(err_buf__,*) "Invalid phasespace type, or ";call err__("spec/diagnostics/os-spec-phasespace.f03",3334)
    write(err_buf__,*) "phasespace type not implemented";call err__("spec/diagnostics/os-spec-phasespace.f03",3335)
    call abort_program(p_err_invalid)
