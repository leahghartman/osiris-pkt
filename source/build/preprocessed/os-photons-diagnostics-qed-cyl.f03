# 1 "qed-cyl/os-photons-diagnostics-qed-cyl.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "qed-cyl/os-photons-diagnostics-qed-cyl.f03" 2
# 1 "./os-config.h" 1
...
# 2 "qed-cyl/os-photons-diagnostics-qed-cyl.f03" 2
...
# 3 "qed-cyl/os-photons-diagnostics-qed-cyl.f03" 2

module m_photons_diagnostics_qedcyl
...
# 19 "./memory/memory.h"
use memory
# 7 "qed-cyl/os-photons-diagnostics-qed-cyl.f03" 2
...
end module m_photons_diagnostics_qedcyl
...
!-----------------------------------------------------------------------------------------
    ...
    ! need for the t_species_cyl_modes routine
    ...
    if (this%betheheitler_info%if_betheheitler ) then
        ...
        call cleanup( this % betheheitler_info % rho_ion_cyl_m )
        call deposit_quant_cyl_modes( this%betheheitler_info%ion_cyl_m, grid, no_co, this % betheheitler_info % rho_ion_cyl_m, this % betheheitler_info % rho_ion, p_charge_cyl_m, send_msg, recv_msg )
    end if

end subroutine report_photons_qedcyl
!-----------------------------------------------------------------------------------------
    ...
    if (test_if_report( tstep, this%diag%ndump_fac_ene )) then
        ...
        if ( root(no_co) ) then
          ...
          if ( t == tmin ) then
              ...
              call check_error(ierr,"Unable to create directory for particle energy diagnostic",p_err_diagfile,"qed-cyl/os-photons-diagnostics-qed-cyl.f03",329)
...
end subroutine report_energy_photons_qedcyl
!-----------------------------------------------------------------------------------------
...
end subroutine report_pairs_photons_qedcyl
!---------------------------------------------------------------------------------------------------
