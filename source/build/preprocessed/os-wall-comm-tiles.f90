# 1 "tiles/os-wall-comm-tiles.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "tiles/os-wall-comm-tiles.f90" 2
# 1 "./os-config.h" 1
...
# 2 "tiles/os-wall-comm-tiles.f90" 2
...
# 3 "tiles/os-wall-comm-tiles.f90" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "tiles/os-wall-comm-tiles.f90" 2
...
subroutine loc_send_wall( wall, update_type, dim, bnd, nx_move, send_msg )
  ...
  if ( msg_size > bsize ) then
    call freemem(send_msg(bnd)%buffer,"tiles/os-wall-comm-tiles.f90",81)
    call alloc(send_msg(bnd)%buffer, (/msg_size/),"tiles/os-wall-comm-tiles.f90",82)
  endif
...
subroutine isend_wall_tiles( wall, update_type, dim, bnd, no_co, nx_move, send_msg )
  ...
  if ( send_msg(bnd)%handle /= MPI_REQUEST_NULL ) then
    write(err_buf__,*) 'isend_wall_tiles called when message still waiting to complete';call err__("tiles/os-wall-comm-tiles.f90",110)
  ...
  if ( msg_size > bsize ) then
    call freemem(send_msg(bnd)%buffer,"tiles/os-wall-comm-tiles.f90",162)
    call alloc(send_msg(bnd)%buffer, (/msg_size/),"tiles/os-wall-comm-tiles.f90",163)
  endif
...
subroutine loc_recv_wall( wall, update_type, dim, bnd, nx_move, recv_msg )
  ...
  ! (*debug*)
  if ( msg_size == 0 ) then
    write(err_buf__,*) 'Invalid message size, aborting.';call err__("tiles/os-wall-comm-tiles.f90",223)
    call abort_program( p_err_invalid )
  endif
...
subroutine irecv_wall_tiles( wall, update_type, dim, bnd, no_co, nx_move, recv_msg )
  ...
  if ( recv_msg( bnd )%handle /= MPI_REQUEST_NULL ) then
    write(err_buf__,*) 'irecv_wall_tiles called when message still waiting to complete';call err__("tiles/os-wall-comm-tiles.f90",255)
    call abort_program( p_err_invalid )
  endif
  ...
  ! (*debug*)
  if ( msg_size == 0 ) then
    write(err_buf__,*) 'Invalid message size, aborting.';call err__("tiles/os-wall-comm-tiles.f90",292)
    call abort_program( p_err_invalid )
  endif
  ...
  if ( msg_size > bsize ) then
    call freemem(recv_msg(bnd)%buffer,"tiles/os-wall-comm-tiles.f90",313)
    call alloc(recv_msg(bnd)%buffer, (/msg_size/),"tiles/os-wall-comm-tiles.f90",314)
  endif
