# 1 "qed/os-photons.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "qed/os-photons.f03" 2
# 1 "./os-config.h" 1
...
!---------------------------------------------------------------------------------------------------
    ...
    if ( local_idx%buf_size > 0 ) call freemem(local_idx%idx,"qed/os-photons.f03",546)
    local_idx%start = 1
    local_idx%nidx = 0
    ...
    subroutine add_particle_index( list, i )
        ...
        if ( list%nidx + 1 > list%buf_size ) then
            ...
            call alloc(tmp_idx, (/ list%buf_size /),"qed/os-photons.f03",568)
            if ( list%nidx > 0 ) call memcpy( tmp_idx, list%idx, list%nidx )
            call freemem(list%idx,"qed/os-photons.f03",570)
            list%idx => tmp_idx
        endif
...
!---------------------------------------------------------------------------------------------------
  ...
  subroutine create_pairs_betheheitler( this, electrons, positrons, dt, i0, i1)
    ...
    ! cleanup the local_idx buffer for deleted photons
    if ( local_idx%buf_size > 0 ) call freemem(local_idx%idx,"qed/os-photons.f03",705)
    local_idx%start = 1
    local_idx%nidx = 0
    local_idx%buf_size = 0
    ...
    subroutine add_particle_index( list, i )
        ...
        if ( list%nidx + 1 > list%buf_size ) then
            list%buf_size = list%buf_size + p_spec_buf_block
            call alloc(tmp_idx, (/ list%buf_size /),"qed/os-photons.f03",727)
            if ( list%nidx > 0 ) call memcpy( tmp_idx, list%idx, list%nidx )
            call freemem(list%idx,"qed/os-photons.f03",729)
            list%idx => tmp_idx
        endif
    ...
    select case ( this%coordinates )
        case default
            select case ( p_x_dim )
                ...
                case default
                    write(err_buf__,*) 'Not implemented for x_dim = ',p_x_dim;call err__("qed/os-photons.f03",814)
