# 1 "vdf/os-vdf-smooth.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-vdf-smooth.f90" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!-----------------------------------------------------------------------------------------
  ...
  call freemem(this%scoef,"vdf/os-vdf-smooth.f90",106)
...
!-----------------------------------------------------------------------------------------
  ...
  if (ierr == 0) then
    ...
    if (disp_out(input_file)) then
      if (mpi_node()==0) print *," - no smoothing specified"
    endif
  endif
  ...
  if ( this%max_order > 0 ) then
    call alloc(this%scoef, (/ 2*this%max_order + 1, p_x_dim /),"vdf/os-vdf-smooth.f90",356)
    this%scoef = 0.
...
!-----------------------------------------------------------------------------------------
  ...
  if ( associated(this%scoef) ) then
    select case ( pf%x_dim_ )
     ...
     case default
       write(err_buf__,*) "invalid dimensions on pf vdf, xdim = ", pf % x_dim_;call err__("vdf/os-vdf-smooth.f90",535)
