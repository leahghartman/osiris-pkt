# 1 "os-dynloadbalance.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "os-dynloadbalance.f03" 2
!#define __DEBUG__ 1
...
subroutine test_reshape_vdf( vdf_ref, old_lb, new_lb, no_co )
    ...
    if ( vdf%x_dim_ /= 3 ) then
        write(err_buf__,*) 'test_reshape_vdf is only implemented for 3D';call err__("os-dynloadbalance.f03",45)
        call abort_program()
    endif
            ...
            do i1 = lbound( vdf%f3, 2 ), ubound( vdf%f3, 2 )
                ...
                do k = 1, vdf%f_dim()
                    if ( vdf%f3( k, i1, i2, i3 ) /= v1 ) then
                        print *, "[",mpi_node(),"] ",'reshape_vdf_copy failed, bad value at position ', i1, ', ', i2, ', ', i3, ' fc: ', k
                        print *, "[",mpi_node(),"] ",'expected : ', v1, ' got : ', vdf%f3( k, i1, i2, i3 )
...
# 246 "os-dynloadbalance.f03"
                ! move particles to proper process
                call sim%part%reshape( sim%grid, new_lb, sim%no_co, sim%bnd%send_vdf, sim%bnd%recv_vdf )
                ...
                call sim%emf%reshape( sim%grid, new_lb, sim%no_co, sim%bnd%send_vdf, sim%bnd%recv_vdf )
