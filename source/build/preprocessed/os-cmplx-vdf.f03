# 1 "vdf/os-cmplx-vdf.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-cmplx-vdf.f03" 2
# 1 "./os-config.h" 1
...
subroutine init_cmplx_pointers( this )
  ...
  if ( 2 * f_dim /= this % f_dim_ ) then
    write(err_buf__,*) "(*error*) Superclass f_dim is not compatible with complex data";call err__("vdf/os-cmplx-vdf.f03",113)
...
!---------------------------------------------------------------------------------------------------
  ...
  write(err_buf__,*) "(*error*) check_nan_vdf is not implemented for complex valued VDFs";call err__("vdf/os-cmplx-vdf.f03",622)
  call abort_program( p_err_invalid )
