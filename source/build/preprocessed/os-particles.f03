# 1 "particles/os-particles.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "particles/os-particles.f03" 2
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! particle class
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# 8 "particles/os-particles.f03" 2
...
# 9 "particles/os-particles.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 13 "particles/os-particles.f03" 2
...
# 221 "particles/os-particles.f03"
...
subroutine report_node_load_part( this, n, ndump, t, no_co )
    ...
    ! gather data
    nnodes = no_num( no_co )
    call alloc(node_load, (/nnodes/),"particles/os-particles.f03",451)
    ...
    if ( nnodes > 1 ) then
        ...
        if ( ierr /= MPI_SUCCESS ) then
            write(err_buf__,*) 'MPI Error';call err__("particles/os-particles.f03",458)
    ...
    if ( root( no_co ) ) then
        ...
        select case (p_x_dim)
        case(1)
            call alloc(f1, lnx,"particles/os-particles.f03",500)
            f1 = -1.0
            do i = 1, nnodes
                f1( no_co%ngp( i, 1 ) ) = node_load(i)
            enddo
            call diagFile % add_dataset( "load", f1 )
            call freemem(f1,"particles/os-particles.f03",506)

        case(2)
            call alloc(f2, lnx,"particles/os-particles.f03",509)
            f2 = -1.0
            do i = 1, nnodes
                f2( no_co%ngp( i, 1 ), no_co%ngp( i, 2 ) ) = node_load(i)
            enddo
            call diagFile % add_dataset( "load", f2 )
            call freemem(f2,"particles/os-particles.f03",515)

        case(3)
            call alloc(f3, lnx,"particles/os-particles.f03",518)
            f3 = -1.0
            do i = 1, nnodes
                f3( no_co%ngp( i, 1 ), no_co%ngp( i, 2 ), no_co%ngp( i, 3 ) ) = node_load(i)
            enddo
            call diagFile % add_dataset( "load", f3 )
            call freemem(f3,"particles/os-particles.f03",524)
        end select
    ...
    ! free load array
    call freemem(node_load,"particles/os-particles.f03",533)
...
# 918 "particles/os-particles.f03"
...
!-----------------------------------------------------------------------------------------
subroutine init_particles( this, g_space, jay, emf, grid, no_co, bnd, zpulse_list, &
...
!-----------------------------------------------------------------------------------------
    ...
    logical :: keep_previous_charge, use_boost
    ...
    case default
        interp_offset = 0.0_p_double
        write(err_buf__,*) 'Interpolation value not supported';call err__("particles/os-particles.f03",1003)
    ...
    if (pushev==0) then
        ...
        species => this % species; use_boost = .false.
        do
            if (.not. associated(species)) exit
            if (species%diag%if_use_boosted_diag .or. &
                species%diag%if_use_boosted_raw) use_boost = .true.
            species => species % next
        enddo
        if (use_boost) then
            diag_part_boost_extract_ev = create_event('boosted particle diag extract portion')
            diag_part_boost_ev = create_event('boosted particle diag write portion')
        endif
...
subroutine advance_deposit_particles( this, emf, jay, tstep, t, no_co, options )
    ...
    ! executable statements
# 1266 "particles/os-particles.f03"
    nt = n_threads( no_co )
    if ( nt > 1 ) then
...
!---------------------------------------------------------------------------------------------------
# 1410 "particles/os-particles.f03"
!-----------------------------------------------------------------------------------------
! report particle data
!-----------------------------------------------------------------------------------------
subroutine report_particles( this, emf, g_space, grid, no_co, tstep, t, tmin, send_msg, recv_msg )
    ...
    do
        ...
        if (.not. associated(species)) exit
        call species % report( emf, g_space, grid, no_co, tstep, t, tmin, send_msg, recv_msg )
        species => species % next
    enddo
...
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
subroutine report_energy_particles( this, no_co, tstep, t, tmin )
    ...
    if ( test_if_report( tstep, this%ndump_fac_ene ) ) then
        ...
        call alloc(ene_all, (/ this%num_species + 1 /),"particles/os-particles.f03",1563)
        ...
        if ( root(no_co) ) then
            ...
            if ( t == tmin ) then
                ...
                call mkdir( path, ierr )
                call check_error(ierr,p_err_msg,p_err_diagfile,"particles/os-particles.f03",1594)
        ...
        call freemem(ene_all,"particles/os-particles.f03",1634)
    ...
    do
        if (.not. associated(species)) exit
        call species % report_energy( no_co, tstep, t, tmin )
        species => species % next
    enddo
...
!---------------------------------------------------------------------------------------------------
subroutine add_load_particles( this, grid, emf, n )
    ...
    ! add approximate load from neutrals if appropriate
    if ( n > 0 ) then
        do i = 1, this%num_neutral
            call this % neutral(i) % add_particle_load( grid )
        enddo
    endif
...
subroutine cleanup_particles( this )
    ...
    if ( this%num_cathode > 0 ) then
        ...
        ! free cathode buffer
        ! call freemem(this%cathode,"particles/os-particles.f03",1859)
    ...
    if ( this%num_neutral > 0 ) then
        ...
        ! free species buffer
        ! call freemem(this%neutral,"particles/os-particles.f03",1873)
...
subroutine write_checkpoint_particles( this, restart_handle )
    ...
    call restart_io_write("p_part_rst_id", p_part_rst_id, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1922)

    ! write local data to file
    call restart_io_write("this%interpolation", this%interpolation, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1926)

    call restart_io_write("this%num_species", this%num_species, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1929)

    call restart_io_write("this%num_cathode", this%num_cathode, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1932)

    call restart_io_write("this%n_current", this%n_current, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1935)


    call restart_io_write("this%num_neutral", this%num_neutral, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles.f03",1939)
...
subroutine restart_read_particles( this, restart_handle )
    ...
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2001)

    ! check if restart file is compatible
    if ( rst_id /= p_part_rst_id) then
        write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("particles/os-particles.f03",2005)
        write(err_buf__,*) 'from incompatible binary (part)';call err__("particles/os-particles.f03",2006)
        write(err_buf__,*) 'rst_id = ', rst_id;call err__("particles/os-particles.f03",2007)
        call abort_program(p_err_rstrd)
    endif

    call restart_io_read(interpolation, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2012)

    call restart_io_read(num_species, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2015)

    call restart_io_read(num_cathode, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2018)

    call restart_io_read(this%n_current, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2021)



    call restart_io_read(num_neutral, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles.f03",2026)

    if (this%num_neutral /= num_neutral) then
    write(err_buf__,*) 'The number of neutrals specified in the input deck is different';call err__("particles/os-particles.f03",2029)
    write(err_buf__,*) 'input : ', this%num_neutral, ' rst: ', num_neutral;call err__("particles/os-particles.f03",2030)
    write(err_buf__,*) 'from the number of neutrals in the restart file';call err__("particles/os-particles.f03",2031)
    call abort_program(p_err_invalid)
    endif
    ...
    if ( this%interpolation /= interpolation ) then
        write(err_buf__,*) 'The interpolation level in the input deck is different';call err__("particles/os-particles.f03",2038)
        write(err_buf__,*) 'from the interpolation level in the restart file';call err__("particles/os-particles.f03",2039)
        call abort_program(p_err_invalid)
    endif

    if (this%num_species /= num_species) then
        write(err_buf__,*) 'The number of species specified in the input deck is different';call err__("particles/os-particles.f03",2044)
        write(err_buf__,*) 'from the number of species in the restart file';call err__("particles/os-particles.f03",2045)
        call abort_program(p_err_invalid)
    endif

    if (this%num_cathode /= num_cathode) then
        write(err_buf__,*) 'The number of cathodes specified in the input deck is different';call err__("particles/os-particles.f03",2050)
        write(err_buf__,*) 'from the number of cathodes in the restart file';call err__("particles/os-particles.f03",2051)
        call abort_program(p_err_invalid)
    endif
