# 1 "pkt/os-particles-pkt.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "pkt/os-particles-pkt.f03" 2
!
...
!-----------------------------------------------------------------------------------------
      ...
      if (mpi_node()==0) print *,"Reading pkt species group configuration..."
      ...
      if (ierr == 0) then
         ...
         if (mpi_node()==0) print *,"   - pkt species group parameters missing, using default settings"
      end if
...
!-----------------------------------------------------------------------------------------
      ...
      if (ierr == 0) then
         ...
         if (mpi_node()==0) print *,"   - pkt species group diagnostics parameters missing, using default settings"
      end if
...
!-----------------------------------------------------------------------------------------
      ...
      if (this%num_pkt > 0) then
         ...
         call alloc(seed, (/size_seed/),"pkt/os-particles-pkt.f03",708)
         do i = 1, size_seed
         ...
         call freemem(seed,"pkt/os-particles-pkt.f03",715)
      end if
...
!-----------------------------------------------------------------------------------------
   ...
   subroutine advance_deposit_pkt(this, emf, jay, tstep, t, no_co, options)
      ...
      do i = 1, this%num_pkt
         ...
         case default
            ...
            case (3)
               ...
               call this%pkt_group(i)%photons%advance(den_ph, grad_ph, density, &
                                                      grad_grid, dt(tstep), 0, 1)
            case default
               write(err_buf__,*) 'Not implemented for x_dim = ', p_x_dim;call err__("pkt/os-particles-pkt.f03",1070)
               call abort_program(p_err_invalid)
            end select
         case (p_cylindrical_b)
            call deposit_density_2d(density, ix_e, x_e, q_e, p_e, np_e, interp)
            ! call deposit_density_2d(photon_density, ix_ph, x_ph, &
            ! this%pkt_group(i)%photons%q(1:np_ph), &
            ! p_ph_zero, np_ph, interp)
            ! this%pkt_group(i)%electrons%photon_density_grid => photon_density

            call update_boundary(density, p_vdf_add, no_co, send_msg, recv_msg)
            call cleanup(send_msg)
            call cleanup(recv_msg)

            ! call update_boundary(photon_density, p_vdf_add, no_co, send_msg, recv_msg)
            ! call cleanup(send_msg)
            ! call cleanup(recv_msg)

            ! TODO: add filtering for various densities here
            ...
            ! TODO: add 2D filtering for grad_grid_x1 and grad_grid_r here
         ...
         ! case (p_cylindrical_b)
         ! if (present(grad_grid_x1) .and. present(grad_grid_r)) then
         ! call advance_photons_cyl_2d(this, den_ph, grad_ph, den_grid, &
         ! grad_grid_x1, grad_grid_r, gdt, ip0, ip1)
         ! else
         ! write(err_buf__,*) 'grad_grid_x1 and grad_grid_r required for cylindrical coordinates';call err__("pkt/os-particles-pkt.f03",1117)
         ! call abort_program(p_err_invalid)
         ! end if
         ! case default
         ! select case (p_x_dim)
...
!---------------------------------------------------------------------------------------------------
   subroutine report_node_load_part_pkt(this, n, ndump, t, no_co)
      ...
      ! gather data
      nnodes = no_num(no_co)
      call alloc(node_load, (/nnodes/),"pkt/os-particles-pkt.f03",1427)
      ...
      if (nnodes > 1) then
         ...
         if (ierr /= MPI_SUCCESS) then
            write(err_buf__,*) 'MPI Error';call err__("pkt/os-particles-pkt.f03",1434)
      ...
      if (root(no_co)) then
         ...
         select case (p_x_dim)
         case (1)
            call alloc(f1, lnx,"pkt/os-particles-pkt.f03",1475)
            f1 = -1.0
            do i = 1, nnodes
               f1(no_co%ngp(i, 1)) = node_load(i)
            end do
            call diagFile%add_dataset("load", f1)
            call freemem(f1,"pkt/os-particles-pkt.f03",1481)

         case (2)
            call alloc(f2, lnx,"pkt/os-particles-pkt.f03",1484)
            f2 = -1.0
            do i = 1, nnodes
               f2(no_co%ngp(i, 1), no_co%ngp(i, 2)) = node_load(i)
            end do
            call diagFile%add_dataset("load", f2)
            call freemem(f2,"pkt/os-particles-pkt.f03",1490)

         case (3)
            call alloc(f3, lnx,"pkt/os-particles-pkt.f03",1493)
            f3 = -1.0
            do i = 1, nnodes
               f3(no_co%ngp(i, 1), no_co%ngp(i, 2), no_co%ngp(i, 3)) = node_load(i)
            end do
            call diagFile%add_dataset("load", f3)
            call freemem(f3,"pkt/os-particles-pkt.f03",1499)
         end select
      ...
      ! free load array
      call freemem(node_load,"pkt/os-particles-pkt.f03",1508)
