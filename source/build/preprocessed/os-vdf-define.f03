# 1 "vdf/os-vdf-define.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "vdf/os-vdf-define.f03" 2
!#define DEBUG_FILE 1
!-------------------------------------------------------------------------------
...
subroutine cleanup_vdf(this)
  ...
  select case (this%x_dim_)
    case(1)
      call freemem(this%f1,"vdf/os-vdf-define.f03",267)
    case(2)
      call freemem(this%f2,"vdf/os-vdf-define.f03",269)
    case(3)
      call freemem(this%f3,"vdf/os-vdf-define.f03",271)
    case default
      continue
  end select
...
subroutine new_vdf( this, x_dim, f_dim, nx, gc_num, dx, zero )
  ...
  if ( (x_dim < 1) .or. (x_dim > p_max_dim)) then
    write(err_buf__,*) "Invalid value for x_dim, ", x_dim, " must be between 1 and ", p_max_dim;call err__("vdf/os-vdf-define.f03",315)
    call abort_program( p_err_invalid )
  endif

  if (f_dim < 1) then
    write(err_buf__,*) "Invalid value for f_dim, ", f_dim, " must be >= 1";call err__("vdf/os-vdf-define.f03",320)
    call abort_program( p_err_invalid )
  endif

  if ( size(nx) < x_dim ) then
    write(err_buf__,*) "The size of nx, ",size(nx),"is smaller that x_dim, ",x_dim;call err__("vdf/os-vdf-define.f03",325)
    call abort_program( p_err_invalid )
  endif

  if (( size(gc_num,1) /= 2 ) .or. ( size(gc_num,2) < x_dim )) then
    write(err_buf__,*) "The size of gc_num is invalid, must be gc_num(2, >= x_dim)";call err__("vdf/os-vdf-define.f03",330)
    call abort_program( p_err_invalid )
  endif
  ...
  select case (x_dim)
  case(1)
    call alloc(this%f1, lb, ub,"vdf/os-vdf-define.f03",356)
    tmp = c_loc( this % f1( lb(1), lb(2) ) )

  case(2)
    call alloc(this%f2, lb, ub,"vdf/os-vdf-define.f03",360)
    tmp = c_loc( this % f2( lb(1), lb(2), lb(3) ) )

  case(3)
    call alloc(this%f3, lb, ub,"vdf/os-vdf-define.f03",364)
...
subroutine new_copy_vdf( this, source, f_dim, zero, copy, fc )
  ...
  if (present(f_dim)) then
    if (f_dim < 1) then
      write(err_buf__,*) "Field components (f_dim) must be >= 1";call err__("vdf/os-vdf-define.f03",415)
      call abort_program(p_err_invalid)
    endif
    f_dim_ = f_dim
  else
...
subroutine write_checkpoint_vdf( this, restart_handle )
  ...
  ! Save version id
  call restart_io_write("p_vdf_rst_id", p_vdf_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",509)

  ! save number of dimensions and grid parameters
  call restart_io_write("this%x_dim_", this%x_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",513)

  call restart_io_write("this%f_dim_", this%f_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",516)

  call restart_io_write("this%nx_", this%nx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",519)

  call restart_io_write("this%gc_num_", this%gc_num_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",522)

  call restart_io_write("this%dx_", this%dx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",525)
  ...
  if (this%x_dim_ > 0) then
    ...
    call check_error(ierr,err_msg,p_err_rstwrt,"vdf/os-vdf-define.f03",536)
  endif
...
subroutine read_checkpoint_vdf( this, restart_handle )
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",571)

  if ( rst_id /= p_vdf_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("vdf/os-vdf-define.f03",574)
    write(err_buf__,*) 'from incompatible binary (vdf)';call err__("vdf/os-vdf-define.f03",575)
    call abort_program(p_err_rstrd)
  endif

  ! read number of dimensions and grid parameters

  call restart_io_read(this%x_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",582)


  call restart_io_read(this%f_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",586)


  call restart_io_read(this%nx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",590)


  call restart_io_read(this%gc_num_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",594)


  call restart_io_read(this%dx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",598)
  ...
  if (this%x_dim_ > 0) then
    ...
    select case ( this%x_dim_ )
    case(1)
      call alloc(this%f1, lb, ub,"vdf/os-vdf-define.f03",615)
      tmp = c_loc( this % f1( lb(1), lb(2) ) )
      call restart_io_read(this%f1, restart_handle, ierr)

    case(2)
      call alloc(this%f2, lb, ub,"vdf/os-vdf-define.f03",620)
      tmp = c_loc( this % f2( lb(1), lb(2), lb(3) ) )
      call restart_io_read(this%f2, restart_handle, ierr)

    case(3)
      call alloc(this%f3, lb, ub,"vdf/os-vdf-define.f03",625)
      tmp = c_loc( this % f3( lb(1), lb(2), lb(3), lb(4) ) )
    ...
    call check_error(ierr,err_msg,p_err_rstrd,"vdf/os-vdf-define.f03",634)
