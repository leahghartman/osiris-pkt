# 1 "spec/os-spec-boundary.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/os-spec-boundary.f03" 2
!#define __DEBUG__ 1
       ...
       do j = 1, 2
         ...
         elseif (this%type(j,i) == p_bc_periodic) then
           if ( .not. if_photons ) then
            ...
            if (mpi_node()==0) print *,"   Warning, the code is running with a specific BC for photons "
            if (mpi_node()==0) print *,"   It is only meant to be used with the qed_cyl simulation mode"
            if (mpi_node()==0) print *,"   The BC is periodical in the radial direction, only for photons"
           endif
         endif
       enddo
    endif
  ...
  if ( restart ) then
   ...
   if ( coordinates == p_cylindrical_b ) then
     ...
     if ((this%type(p_lower, p_r_dim) == p_bc_periodic) .or. &
       ...
       if (.not. if_photons) then
        write(err_buf__,*) 'no periodic b.c. for radial direction';call err__("spec/os-spec-boundary.f03",352)
...
subroutine restart_write_spe_bound( this, restart_handle )
  ...
  ! executable statements
  call restart_io_write("p_spbnd_rst_id", p_spbnd_rst_id, restart_handle, ierr)
  if ( ierr/=0 ) then
    write(err_buf__,*) 'error writing restart data for species boundary object.';call err__("spec/os-spec-boundary.f03",400)
  call abort_program(p_err_rstwrt)
  endif

  call restart_io_write("this%type", this%type, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error writing restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",406)
    call abort_program(p_err_rstwrt)
  endif
  call restart_io_write("this%uth_bnd", this%uth_bnd, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error writing restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",411)
    call abort_program(p_err_rstwrt)
  endif
  call restart_io_write("this%ufl_bnd", this%ufl_bnd, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error writing restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",416)
    call abort_program(p_err_rstwrt)
  endif
...
subroutine restart_read_spe_bound( this, restart_handle )
  ...
  if ( ierr/=0 ) then
  write(err_buf__,*) 'error reading restart data for species boundary object.';call err__("spec/os-spec-boundary.f03",444)
  call abort_program(p_err_rstrd)
  endif

  ! check if restart file is compatible
  if ( rst_id /= p_spbnd_rst_id) then
  write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("spec/os-spec-boundary.f03",450)
  write(err_buf__,*) 'from incompatible binary (species boundary)';call err__("spec/os-spec-boundary.f03",451)
  call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%type, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error reading restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",457)
    call abort_program(p_err_rstrd)
  endif
  call restart_io_read(this%uth_bnd, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error reading restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",462)
    call abort_program(p_err_rstrd)
  endif
  call restart_io_read(this%ufl_bnd, restart_handle, ierr)
  if ( ierr/=0 ) then
     write(err_buf__,*) 'error reading restart data for spe_bound object.';call err__("spec/os-spec-boundary.f03",467)
    call abort_program(p_err_rstrd)
  endif
...
subroutine check_boundary( species, min_npar, i_dim, bnd_cross, node_cross )
  ...
  if ( par_idx_size < ncross ) then
    ...
    call freemem(bnd_cross( p_lower ) % idx,"spec/os-spec-boundary.f03",567)
    call freemem(bnd_cross( p_upper ) % idx,"spec/os-spec-boundary.f03",568)

    ! give it a little head room
    call alloc(bnd_cross( p_lower ) % idx, (/ ncross + p_idx_block /),"spec/os-spec-boundary.f03",571)
    call alloc(bnd_cross( p_upper ) % idx, (/ ncross + p_idx_block /),"spec/os-spec-boundary.f03",572)
  endif
...
subroutine check_node_cross( this, n_threads, node_cross )
  ...
  if ( this%num_par > cross_bsize ) then
    call freemem(node_cross % idx,"spec/os-spec-boundary.f03",731)
    call alloc(node_cross % idx, (/ this%num_par /),"spec/os-spec-boundary.f03",732)
  endif
...
subroutine phys_boundary_species( this, current, dt, i_dim, bnd_idx, par_idx, npar )
  ...
  case default
    ...
    write(err_buf__,*) "invalid particle bc";call err__("spec/os-spec-boundary.f03",1384)
...
# 1440 "spec/os-spec-boundary.f03"
