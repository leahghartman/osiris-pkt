...
!-----------------------------------------------------------------------------------------
  ...
  logical :: init_move_window, if_move_any
  ...
  init_py_func = "NO_PYTHON_FUNCTION_SUPPLIED!"
  ...
  ext_py_func = "NO_PYTHON_FUNCTION_SUPPLIED!"
   ...
   do i=1, p_f_dim
    select case(trim(type_init_b(i)))
      ...
      case( "python" )
        ...
        if_move_any = if_move(1)
        do j = 2, p_x_dim
          if_move_any = if_move_any .or. if_move(j)
        enddo
        if (init_move_window .and. if_move_any) then
          if ( mpi_node() == 0 ) then
      ...
      case default
        ...
        print *, "Available initial B-field types are 'uniform', 'math func', 'dipole'"
        print *, "and 'python'"
        print *, "aborting..."
        stop
    end select
    ...
    select case(trim(type_init_e(i)))
      ...
      case default
        ...
        print *, "Available initial E-field types are 'uniform', 'math func', 'dipole'"
        print *, "and 'python'"
        print *, "aborting..."
        stop
    end select
  ...
  do i=1, p_f_dim
    select case(trim(type_ext_b(i)))
      ...
      case default
        ...
        print *, "Available external B-field types are 'none', 'uniform', 'math func',"
        print *, "'dipole' and 'python'"
        print *, "aborting..."
        stop
    end select
    ...
    select case(trim(type_ext_e(i)))
      ...
      case default
        ...
        print *, "Available external E-field types are 'none', 'uniform', 'math func',"
        print *, "'dipole' and 'python'"
        print *, "aborting..."
        stop
    end select
...
!-----------------------------------------------------------------------------------------
  ...
  use m_boosted_diag, only : p_boosted_fields
  ...
  if ( this%ext_fld /= p_extfld_none ) then
    ...
    if (fsolverev==0) then
      if (mpi_node()==0) print *,' - setting up external fields...'
    endif
  ...
  if ( ( this%smooth_type == p_emfsmooth_stand ) .or. &
    ...
    call alloc(this%b_part,"emf/os-emf.f03",822)
    call alloc(this%e_part,"emf/os-emf.f03",823)
  ...
  if( this % diag % if_use_boosted_diag) then
    ...
    boost_basepath = trim(path_mass) // 'FLD_BOOST' // p_dir_sep 

    call this % diag % boosted_diag % init( grid, g_space, dt(tstep), tmin, tmax, &
                  ndump(tstep), trim( boost_basepath ), this%e, this%e%gc_num_, restart, restart_handle )
...
!-----------------------------------------------------------------------------------------
  ...
  if ( this%part_fld_alloc ) then
    ...
    call freemem(this%b_part,"emf/os-emf.f03",881)
    call freemem(this%e_part,"emf/os-emf.f03",882)
  endif
...
!-----------------------------------------------------------------------------------------
  ...
  call restart_io_write("p_emf_rst_id", p_emf_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/os-emf.f03",945)

  call restart_io_write("this%n_current_emf", this%n_current_emf, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/os-emf.f03",948)

  call restart_io_write("this%part_grid_center", this%part_grid_center, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/os-emf.f03",951)

  call this%b%write_checkpoint( restart_handle )
  call this%e%write_checkpoint( restart_handle )

  call restart_io_write("this%ext_fld", this%ext_fld, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"emf/os-emf.f03",957)
...
!-----------------------------------------------------------------------------------------
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"emf/os-emf.f03",1001)

  ! check if restart file is compatible
  if ( rst_id /= p_emf_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file from incompatible binary (emf)';call err__("emf/os-emf.f03",1005)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%n_current_emf, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/os-emf.f03",1010)


  call restart_io_read(this%part_grid_center, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/os-emf.f03",1014)
  ...
  call restart_io_read(this%ext_fld, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"emf/os-emf.f03",1023)
...
!-----------------------------------------------------------------------------------------
  ...
  use m_system
  use m_parameters
  use m_emf_define, only : t_emf, p_extfld_none, p_extfld_static
  use m_grid_define, only : t_grid
  use m_node_conf, only : t_node_conf
  use m_vdf_comm, only : t_vdf_msg, reshape_copy, reshape_nocopy
  ...
  if ( this%ext_fld /= p_extfld_none ) then
    ...
    if ( this%ext_fld /= p_extfld_static ) then
      ...
      write(err_buf__,*) 'Not implemented yet';call err__("emf/os-emf.f03",1244)
  ...
  call this%bnd_con%reshape( old_lb, new_lb, no_co, send_msg, recv_msg )

  ! store new position on global grid
  this%gix_pos(1:p_x_dim) = new_lb%my_nx( p_lower, 1:p_x_dim )

  if ( this%coordinates == p_cylindrical_b ) this % solver % gir_pos = this%gix_pos(2)
...
subroutine fill_data_emf(this, tstep)
  ...
  if( n(tstep) /= this%n_current_emf) then
    write(err_buf__,*) "Error in fill_data_emf.. see 'fill_data_emf' is os-emf.f03";call err__("emf/os-emf.f03",1307)
    call abort_program(p_err_rstrd)
  endif
