# 1 "qed-cyl/os-particles-qed-cyl.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "qed-cyl/os-particles-qed-cyl.f03" 2
!
...
# 6 "qed-cyl/os-particles-qed-cyl.f03" 2
...
# 7 "qed-cyl/os-particles-qed-cyl.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 11 "qed-cyl/os-particles-qed-cyl.f03" 2
...
!-----------------------------------------------------------------------------------------
    ...
    if ( num_qed > 0 ) then
        ...
        ! if (mpi_node()==0) print *,"   - reading associated collisions configuration..."
...
!-----------------------------------------------------------------------------------------
  ...
  if ( this%num_qed > 0 ) then
     ...
     call alloc(seed, (/ size_seed /),"qed-cyl/os-particles-qed-cyl.f03",764)
     do i=1, size_seed
     ...
     call freemem(seed,"qed-cyl/os-particles-qed-cyl.f03",771)
  endif
...
subroutine report_node_load_part_qedcyl( this, n, ndump, t, no_co )
  ...
  ! gather data
  nnodes = no_num( no_co )
  call alloc(node_load, (/nnodes/),"qed-cyl/os-particles-qed-cyl.f03",1288)
  ...
  if ( nnodes > 1 ) then
      ...
      if ( ierr /= MPI_SUCCESS ) then
          write(err_buf__,*) 'MPI Error';call err__("qed-cyl/os-particles-qed-cyl.f03",1295)
  ...
  if ( root( no_co ) ) then
      ...
      select case (p_x_dim)
      case(1)
          call alloc(f1, lnx,"qed-cyl/os-particles-qed-cyl.f03",1337)
          f1 = -1.0
          do i = 1, nnodes
              f1( no_co%ngp( i, 1 ) ) = node_load(i)
          enddo
          call diagFile % add_dataset( "load", f1 )
          call freemem(f1,"qed-cyl/os-particles-qed-cyl.f03",1343)

      case(2)
          call alloc(f2, lnx,"qed-cyl/os-particles-qed-cyl.f03",1346)
          f2 = -1.0
          do i = 1, nnodes
              f2( no_co%ngp( i, 1 ), no_co%ngp( i, 2 ) ) = node_load(i)
          enddo
          call diagFile % add_dataset( "load", f2 )
          call freemem(f2,"qed-cyl/os-particles-qed-cyl.f03",1352)

      case(3)
          call alloc(f3, lnx,"qed-cyl/os-particles-qed-cyl.f03",1355)
          f3 = -1.0
          do i = 1, nnodes
              f3( no_co%ngp( i, 1 ), no_co%ngp( i, 2 ), no_co%ngp( i, 3 ) ) = node_load(i)
          enddo
          call diagFile % add_dataset( "load", f3 )
          call freemem(f3,"qed-cyl/os-particles-qed-cyl.f03",1361)
      end select
  ...
  ! free load array
  call freemem(node_load,"qed-cyl/os-particles-qed-cyl.f03",1370)
          ...
          if (.not. associated(sp2)) exit
            if (i1 .eq. int(2*num_qed+couples_bh(2,k))) then
              select type( sp2 )
                ...
                if (mpi_node()==0) print *,"Bethe Heitler activated between QED group ", k, " and ",trim(sp2%name)
                exit
              end select
            endif
            sp2 => sp2 % next; i1 = i1 + 1
