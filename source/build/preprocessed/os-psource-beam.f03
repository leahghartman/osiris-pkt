# 1 "spec/psource/os-psource-beam.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/psource/os-psource-beam.f03" 2
# 1 "./os-config.h" 1
...
# 2 "spec/psource/os-psource-beam.f03" 2
...
# 3 "spec/psource/os-psource-beam.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "spec/psource/os-psource-beam.f03" 2
...
integer, parameter :: p_num_x_max = 1000
...
subroutine read_input_beam( this, input_file, coordinates )
    ...
    namelist /nl_profile/ den_min, density, gauss_n_sigma, gauss_range, &
                          ...
                          uth, gamma, num_x, x, fx, profile_type
    ...
    if ((this%if_mult) .and. ( num_x > 0 )) then
        call alloc(this%x, (/ num_x, p_x_dim /),"spec/psource/os-psource-beam.f03",180)
        call alloc(this%fx, (/ num_x, p_x_dim /),"spec/psource/os-psource-beam.f03",181)
    endif


    ! longitudinal profile options
    do j= 1, 1
      if ( profile_type(j) == "default" ) then
        if (num_x > 0) then
           profile_type(j) = "piecewise-linear"
        else
          ! default is now gaussian in x1
          profile_type(j) = "gaussian"
        endif
      endif
