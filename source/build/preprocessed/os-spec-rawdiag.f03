# 1 "spec/diagnostics/os-spec-rawdiag.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/diagnostics/os-spec-rawdiag.f03" 2
# 1 "./os-config.h" 1
...
# 2 "spec/diagnostics/os-spec-rawdiag.f03" 2
...
# 3 "spec/diagnostics/os-spec-rawdiag.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "spec/diagnostics/os-spec-rawdiag.f03" 2
...
!-------------------------------------------------------------------------------
  ...
  diagFile%particles%offset_t(j) = -0.5_p_double
# 216 "spec/diagnostics/os-spec-rawdiag.f03"
...
subroutine write_raw_( spec, no_co, n, t, n_aux )
  ...
  if ( spec%num_par > 0 ) then
    call alloc(idx, [spec%num_par],"spec/diagnostics/os-spec-rawdiag.f03",333)
  ...
  if ( total_par > 0 ) then
    ...
    if ( num_par > 0 ) then
      ...
      call alloc(write_buffer, [ num_par ],"spec/diagnostics/os-spec-rawdiag.f03",377)
      chunk % start(1) = start
      ...
      call diagFile % write_par_cdset( datasets(k), chunk, start )
      k = k+1
# 428 "spec/diagnostics/os-spec-rawdiag.f03"
      call freemem(write_buffer,"spec/diagnostics/os-spec-rawdiag.f03",428)

      ! Tags
      if ( spec%add_tag ) then
        call alloc(write_tag_buffer, [2, num_par],"spec/diagnostics/os-spec-rawdiag.f03",432)
        chunk_tag % count(1) = 2
        ...
        call freemem(write_tag_buffer,"spec/diagnostics/os-spec-rawdiag.f03",448)
  ...
  if ( spec%num_par > 0 ) then
    call freemem(idx,"spec/diagnostics/os-spec-rawdiag.f03",471)
  endif
...
subroutine extract_boosted_raw_( spec, bd, tstep, t1, grid, g_space )
  ...
  if ( .not. associated(bd%idx) ) then
    call alloc(bd%idx, [spec%num_par_max],"spec/diagnostics/os-spec-rawdiag.f03",532)
  else if ( spec%num_par_max > size(bd%idx) ) then
    call freemem(bd%idx,"spec/diagnostics/os-spec-rawdiag.f03",534)
    call alloc(bd%idx, [spec%num_par_max],"spec/diagnostics/os-spec-rawdiag.f03",535)
  endif
  idx => bd%idx
...
subroutine write_boosted_raw_( spec, bd, no_co, grid, n )
  ...
  if ( num_par > 0 ) then
    call alloc(idx, (/ bd%num_par /),"spec/diagnostics/os-spec-rawdiag.f03",749)
    call alloc(idx_sorted, (/ bd%num_par /),"spec/diagnostics/os-spec-rawdiag.f03",750)
    call alloc(n_file, (/ nt2 /),"spec/diagnostics/os-spec-rawdiag.f03",751)
  ...
  do i2 = 1, nt2
    ...
    if ( total_par > 0 ) then
      ...
      if ( my_par > 0 ) then
        ...
        call alloc(write_buffer, [ my_par ],"spec/diagnostics/os-spec-rawdiag.f03",848)
        ...
        call freemem(write_buffer,"spec/diagnostics/os-spec-rawdiag.f03",898)

        ! Tags
        if ( spec%add_tag ) then
          call alloc(write_tag_buffer, [2, my_par],"spec/diagnostics/os-spec-rawdiag.f03",902)
          chunk_tag % count(1) = 2
          ...
          call freemem(write_tag_buffer,"spec/diagnostics/os-spec-rawdiag.f03",918)
  ...
  ! Free particle indexes
  if ( num_par > 0 ) then
    call freemem(idx,"spec/diagnostics/os-spec-rawdiag.f03",937)
    call freemem(idx_sorted,"spec/diagnostics/os-spec-rawdiag.f03",938)
    call freemem(n_file,"spec/diagnostics/os-spec-rawdiag.f03",939)
  endif
