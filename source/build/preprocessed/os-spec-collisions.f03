# 1 "spec/os-spec-collisions.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/os-spec-collisions.f03" 2
!-------------------------------------------------------------------------------
! Collision module
!-------------------------------------------------------------------------------
!
...
# 65 "spec/os-spec-collisions.f03" 2
...
# 66 "spec/os-spec-collisions.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 72 "spec/os-spec-collisions.f03" 2
...
!-------------------------------------------------------------------------------
  ...
  select case ( lowercase(trim(collision_model)) )
    case ("nanbu")
      write(err_buf__,*) "Nanbu model is poorly implemented at this point, so much so that I'm not going to let you use it";call err__("spec/os-spec-collisions.f03",411)
      write(err_buf__,*) "Perez model is recommended, otherwise please review the code and update the Nanbu model yourself";call err__("spec/os-spec-collisions.f03",412)
      write(err_buf__,*) "Shutting down";call err__("spec/os-spec-collisions.f03",413)
      call abort_program(p_err_notimplemented)
      self%CollisionModel = p_CollisionModel_Nanbu
    case ("sentoku")
      write(wrn_buf__,*) "Sentoku model currently seems to have a factor of 2 error";call wrn__("spec/os-spec-collisions.f03",417)
      write(wrn_buf__,*) "(Yes we could just fix it, but haven't actually found the error, it's just off by 2";call wrn__("spec/os-spec-collisions.f03",418)
      write(wrn_buf__,*) "(so, for now, it persists)";call wrn__("spec/os-spec-collisions.f03",419)
      self%CollisionModel = p_CollisionModel_Sentoku
    case ("nanbu_rel")
      write(err_buf__,*) "'Nanbu relativistic' model is a bad idea";call err__("spec/os-spec-collisions.f03",422)
      write(err_buf__,*) "Use Perez Model";call err__("spec/os-spec-collisions.f03",423)
      call abort_program(p_err_notimplemented)
    ...
    case ("")
      self%CollisionModel = p_CollisionModel_Perez
    case default
      write(err_buf__,*) "Error: Can't parse Collision Model in Input Deck";call err__("spec/os-spec-collisions.f03",435)
  ...
  select case ( lowercase(trim(cross_section_correction)) )
    ...
    case ("")
      self%CrossCorrection = p_RelativisticCrossSectionCorrection_None
    case default
      write(err_buf__,*) "Error: Can't parse Relativistic Cross Section Correction in Input Deck";call err__("spec/os-spec-collisions.f03",449)
      call abort_program(p_err_invalid)
  end select
  ...
  select case ( lowercase(trim(frame_correction)) )
    ...
    case ("")
      self%FrameCorrection = p_FrameCorrection_None
    case default
      write(err_buf__,*) "Error: Can't parse Relativistic Frame Correction in Input Deck";call err__("spec/os-spec-collisions.f03",465)
      call abort_program(p_err_invalid)
  end select
  ...
  select case ( lowercase(trim(root_finder)) )
    ...
    case ("")
      self%RootFinder = p_RootFinder_None
    case default
      write(err_buf__,*) "Error: Can't parse Root Finding Alogrithm in Input Deck";call err__("spec/os-spec-collisions.f03",481)
      call abort_program(p_err_invalid)
  end select
...
subroutine verify_input_deck( self, species_head )
  ...
  ! Executable statements
    ...
    do
      ...
      if ( MODULO(self%n_collide, sp1%n_sort) /= 0 ) then
        if (mpi_node()==0) print *,'(*warning*) n_collide is not a multiple of n_sort for species ', trim(sp1%name)
        if (mpi_node()==0) print *,'(*warning*) setting n_sort to n_collide for this species.'
  ...
  if ( .not. any_species_to_collide ) then
    write(err_buf__,*) 'n_collide set, but no species to be collided';call err__("spec/os-spec-collisions.f03",534)
    write(err_buf__,*) 'Abort !';call err__("spec/os-spec-collisions.f03",535)
    call abort_program()
  endif
  ...
  outer_loop: do
    ...
    if ( sp1%if_collide ) then
      ...
      write(err_buf__,*) 'Only one species to collide, and if_like_collide == .false.';call err__("spec/os-spec-collisions.f03",554)
      call abort_program()
    endif
    sp1 => sp1%next
  enddo outer_loop
  ...
  do
    ...
    if( sp1%if_collide ) then
      if( push_start_time /= sp1%push_start_time ) then
        write(err_buf__,*) "push_start_time of colliding species not compatible:";call err__("spec/os-spec-collisions.f03",578)
        write(err_buf__,*) "all colliding species must have same push_start_time!";call err__("spec/os-spec-collisions.f03",579)
        write(err_buf__,*) "ABORT!";call err__("spec/os-spec-collisions.f03",580)
        call abort_program(p_err_invalid)
      endif
    else
      if( push_start_time < sp1%push_start_time ) then
        write(err_buf__,*) "push_start_time of non colliding species not compatible:";call err__("spec/os-spec-collisions.f03",585)
        write(err_buf__,*) "non-colliding species must have push_start_time not greater than colliding species!";call err__("spec/os-spec-collisions.f03",586)
        write(err_buf__,*) "ABORT!";call err__("spec/os-spec-collisions.f03",587)
        call abort_program(p_err_invalid)
      endif
    endif
    sp1 => sp1%next
  ...
  if ( .not.(self%coulomb_logarithm_automatic) .and. &
    ...
    write(err_buf__,*) "It is not wise to set the Coulomb Logarithm to 0 if it is not calculated automatically.";call err__("spec/os-spec-collisions.f03",597)
    write(err_buf__,*) "ABORT!";call err__("spec/os-spec-collisions.f03",598)
    call abort_program(p_err_invalid)
  endif
  ...
  do
    ...
    if ( sp1%if_collide .and. ( sp1%q_real == 0 ) ) then
      write(err_buf__,*) 'No q_real was given for species ', sp1%name;call err__("spec/os-spec-collisions.f03",608)
      write(err_buf__,*) 'Charge (q_real) needed for collisions!';call err__("spec/os-spec-collisions.f03",609)
      call abort_program(p_err_invalid)
    endif
    sp1 => sp1%next
  enddo
  ...
  if( self%CollisionModel == p_CollisionModel_Nanbu &
    ...
    if( self%RootFinder == p_RootFinder_None ) then
      write(err_buf__,*) "Cumulative scattering model selected, but not root finding method specified";call err__("spec/os-spec-collisions.f03",619)
      call abort_program(p_err_invalid)
    endif
  endif
  ...
  if ( self%norm_charge_density <= 0.0_p_k_part ) then
    write(err_buf__,*) 'No norm number density given.';call err__("spec/os-spec-collisions.f03",626)
    write(err_buf__,*) 'Needed for collisions. ABORT!';call err__("spec/os-spec-collisions.f03",627)
    call abort_program()
  endif

  if ( self%CrossCorrection == p_RelativisticCrossSectionCorrection_None &
       .and. self%CrossCorrTime) then
    write(err_buf__,*) 'Timestep adjusted for Cross Section Correction, but no correction set.';call err__("spec/os-spec-collisions.f03",633)
    call abort_program()
  endif

  if ( self%PerezLowTempCorrection .and. (self%CollisionModel .ne. p_CollisionModel_Perez) ) then
    write(err_buf__,*) 'Low-temperature correction only implemented for Perez collision Model';call err__("spec/os-spec-collisions.f03",638)
    call abort_program(p_err_invalid)
  endif
...
!-------------------------------------------------------------------------------
  ...
  if ( self%n_collide > 0 ) then
    ...
    !call alloc(self%rng_array, (/self%number_of_collision_cells/),"spec/os-spec-collisions.f03",709)
...
subroutine reshape_collisions( self, l_nx )
  ...
  if ( self%n_collide > 0 ) then
    ...
    do dim=1, p_x_dim
      !check compatibility of local pic grid vs. collision grid
      if ( modulo( l_nx(dim), self%nx_collision_cells(dim) ) /= 0) then
        write(err_buf__,*) 'Local node grid (nx_p/node_num) is not a multiple of collision grid for dimension ', dim;call err__("spec/os-spec-collisions.f03",751)
        call abort_program( p_err_invalid )
      endif
    enddo
    ...
    !call freemem(self%rng_array,"spec/os-spec-collisions.f03",770)
    deallocate(self%cell_rngs)
    !call alloc(self%rng_array, (/self%number_of_collision_cells/),"spec/os-spec-collisions.f03",772)
...
!-------------------------------------------------------------------------------
  ...
  if (mpi_node()==0) print *,''
  if (mpi_node()==0) print *,'Collisions:'

  ! Report collision model
  select case ( self%CollisionModel )
     case(p_CollisionModel_Nanbu)
        if (mpi_node()==0) print *,"- Model: Nanbu, cumulative"
     case(p_CollisionModel_Sentoku)
        if (mpi_node()==0) print *,"- Model: Sentoku, relativistic"
     case(p_CollisionModel_Nanbu_Rel)
        if (mpi_node()==0) print *,"- Model: Nanbu, relativistic"
     case(p_CollisionModel_Isotropic)
        if (mpi_node()==0) print *,"- Model: Uniform scattering angle"
     case(p_CollisionModel_Takizuka)
        if (mpi_node()==0) print *,"- Model: Non-Relativistic Takizuka&Abe Model"
     case(p_CollisionModel_Perez)
        if (mpi_node()==0) print *,"- Model: Perez Relativistic Extension of Nanbu Model"
     case default
        if (mpi_node()==0) print *,"- Model: UNKNOWN - Please update os-spec-collisions%list_collision_algorithms"
  end select

  ! Report correction type for crosssection
  select case ( self%CrossCorrection )
     case(p_RelativisticCrossSectionCorrection_None)
        if (mpi_node()==0) print *,"- Type of crosssection correction: none"
     case(p_RelativisticCrossSectionCorrection_Rejection)
        if (mpi_node()==0) print *,"- Type of crosssection correction: rejection method"
     case(p_RelativisticCrossSectionCorrection_Scattering)
        if (mpi_node()==0) print *,"- Type of crosssection correction: scattering angle distribution"
     case default
        if (mpi_node()==0) print *,"- Type of crosssection correction: UNKNOWN - Please update os-spec-collisions%list_collision_algorithms"
  end select

  ! Report correction type for crossection time
  select case ( self%CrossCorrTime )
     case (.false.)
        if (mpi_node()==0) print *,"- Type of crosssection TIME correction: none"
     case (.true.)
        if (mpi_node()==0) print *,"- Type of crosssection TIME correction: dt = dt * 2"
     !case default
     ! if (mpi_node()==0) print *,"- Type of crosssection TIME correction: huh?"
  ...
  select case( self%FrameCorrection )
     case(p_FrameCorrection_None)
        if (mpi_node()==0) print *,"- Type of frame correction: none"
     case(p_FrameCorrection_dt)
        if (mpi_node()==0) print *,"- Type of frame correction: correction for delta T only"
     case(p_FrameCorrection_dt_n)
        if (mpi_node()==0) print *,"- Type of frame correction: correction for delta T and density"
     case default
        if (mpi_node()==0) print *,"- Type of frame correction: UNKNOWN - Please update os-spec-collisions%list_collision_algorithms"
  end select
  ...
  select case( self%RootFinder )
     ...
     case(p_RootFinder_Newton)
        if (mpi_node()==0) print *,"- Type of solver used for A(s): newton"
     case(p_RootFinder_Linear)
        if (mpi_node()==0) print *,"- Type of solver used for A(s): linear interpolation"
     case(p_RootFinder_Cubic)
        if (mpi_node()==0) print *,"- Type of solver used for A(s): cubic interpolation"
     case default
        if (mpi_node()==0) print *,"- Type of solver used for A(s): UNKNOWN - Please update os-spec-collisions%list_collision_algorithms"
  end select
...
! enddo
  ...
  call alloc(last_particle_list, (/ number_of_collision_cells +1, num_species /),"spec/os-spec-collisions.f03",1012)
  ...
  call alloc(coll_cell_num_par, (/ num_species, n_threads /),"spec/os-spec-collisions.f03",1034)
  call alloc(cell_fluid_states, (/ num_species, n_threads /),"spec/os-spec-collisions.f03",1035)
  call alloc(permutation_vector, (/ num_par_max_coll_cell + 1, num_species, n_threads /),"spec/os-spec-collisions.f03",1036)
  ...
  call freemem(last_particle_list,"spec/os-spec-collisions.f03",1173)
  call freemem(coll_cell_num_par,"spec/os-spec-collisions.f03",1174)
  call freemem(cell_fluid_states,"spec/os-spec-collisions.f03",1175)
  call freemem(permutation_vector,"spec/os-spec-collisions.f03",1176)
...
!---------------------------------------------------
  ...
  do particle_pair_loop_iterator = 1, last_pair, 2
    ...
    select case ( self%FrameCorrection )
      ...
      case default
        write(err_buf__,*) "Somehow frame correction was not set";call err__("spec/os-spec-collisions.f03",1474)
        write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",1475)
        call abort_program(p_err_invalid)
    end select
    ...
    select case (self%CollisionModel)
      ...
      case( p_CollisionModel_Nanbu, p_CollisionModel_Nanbu_Rel )
        ...
        elseif( s > p_useA_max ) then
          ...
          select case ( self%RootFinder )
            ...
            case default
              write(err_buf__,*) "Somehow no root finder was set";call err__("spec/os-spec-collisions.f03",1520)
              write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",1521)
              call abort_program(p_err_invalid)
      ...
      case default
        write(err_buf__,*) "Somehow no collision model was selected";call err__("spec/os-spec-collisions.f03",1571)
        write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",1572)
        call abort_program( p_err_invalid)
    end select
...
!---------------------------------------------------
  ...
  do i=1, num_par_a
    ...
    select case ( self%FrameCorrection )
      ...
      case default
        write(err_buf__,*) "Somehow frame correction was not set";call err__("spec/os-spec-collisions.f03",1951)
        write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",1952)
        call abort_program(p_err_invalid)
    ...
    select case (self%CollisionModel)
      ...
      case( p_CollisionModel_Nanbu, p_CollisionModel_Nanbu_Rel )
        ...
        elseif( s > p_useA_max ) then
          ...
          select case( self%RootFinder )
            ...
            case default
              write(err_buf__,*) "Somehow no root finder was set";call err__("spec/os-spec-collisions.f03",1995)
              write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",1996)
              call abort_program(p_err_invalid)
      ...
      case default
        write(err_buf__,*) "Somehow no collision model was selected";call err__("spec/os-spec-collisions.f03",2052)
        write(err_buf__,*) "This is a bug";call err__("spec/os-spec-collisions.f03",2053)
        call abort_program( p_err_invalid)
...
!---------------------------------------------------
  ...
  real(p_k_part) :: cphi, sphi, ctheta, stheta, phi
  ...
  do particle_pair_loop_iterator = 1, last_pair, 2
    ...
    ! Include correction (P. Alves) if s >= 0.5
    s = s_first / (au_rel**3)
    if (s .lt. 0.5) then
      ...
      ! s >= 0.5
      call local_rng%harvest_real2( random_unit_interval )
      ctheta = 2.0_p_k_part*random_unit_interval - 1.0_p_k_part
      stheta = sqrt( 1.0_p_k_part - ctheta**2)
    endif
...
!---------------------------------------------------------------------------------------------------
  ...
  if ( (species%n_sort > 0) .and. (t > species%push_start_time) .and. (species%num_par .gt. 0) ) then
    ...
    if ( mod( n, species%n_sort ) == 0 ) then
      ...
      call alloc(idx, (/ species%num_par /),"spec/os-spec-collisions.f03",4095)
      ...
      call freemem(idx,"spec/os-spec-collisions.f03",4116)
...
!---------------------------------------------------------------------------------------------------
  ...
  call alloc(npic, (/n_grid/),"spec/os-spec-collisions.f03",4167)
  npic = 0
  ...
  call freemem(npic,"spec/os-spec-collisions.f03",4206)
...
!---------------------------------------------------------------------------------------------------
  ...
  call alloc(npic, (/ n_grid /),"spec/os-spec-collisions.f03",4256)
  npic = 0
  ...
  ! call asrt__(isum == species%num_par,"isum == species%num_par","spec/os-spec-collisions.f03",4287)
  ...
  call freemem(npic,"spec/os-spec-collisions.f03",4300)
...
!---------------------------------------------------------------------------------------------------
  ...
  call alloc(npic, (/n_grid/),"spec/os-spec-collisions.f03",4356)
  npic = 0
  ...
  ! isum must be the same as the total number of particles
  ! call asrt__(isum == species%num_par,"isum == species%num_par","spec/os-spec-collisions.f03",4391)
  ...
  call freemem(npic,"spec/os-spec-collisions.f03",4404)
  ...
  if ( num_par > 0 .and. calculate_lambda_debye ) then
    ...
    if (w_tot > 0._p_k_part ) then
      ...
      if (num_par .gt. 0) then
        write(err_buf__,*) "Any (in fact all) particles in collision cell have zero charge, I don't see how that's right";call err__("spec/os-spec-collisions.f03",4516)
        call abort_program(-1)
      else
        write(err_buf__,*) "Honestly I'm just confused";call err__("spec/os-spec-collisions.f03",4519)
        call abort_program(-1)
      endif
    endif
...
!-------------------------------------------------------------------------------
  ...
  !call freemem(this%rng_array,"spec/os-spec-collisions.f03",4747)
...
subroutine restart_write_collisions(this, restart_handle)
  ...
  call restart_io_write("p_collisions_rst_id", p_collisions_rst_id, restart_handle, ierr)
  if ( ierr/=0 ) then
   write(err_buf__,*) 'error writing restart data for collisions.';call err__("spec/os-spec-collisions.f03",4767)
   call abort_program(p_err_rstwrt)
  endif

  call restart_io_write("this%number_of_collision_cells", this%number_of_collision_cells, restart_handle, ierr)
  if ( ierr/=0 ) then
   write(err_buf__,*) 'error writing restart data for collisions.';call err__("spec/os-spec-collisions.f03",4773)
   call abort_program(p_err_rstwrt)
  endif
...
subroutine restart_read_collisions(this, restart_handle)
  ...
  if ( ierr/=0 ) then
    write(err_buf__,*) 'error reading restart data for collisions.';call err__("spec/os-spec-collisions.f03",4800)
    call abort_program(p_err_rstwrt)
  endif

  ! check if restart file is compatible
  if ( rst_id /= p_collisions_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("spec/os-spec-collisions.f03",4806)
    write(err_buf__,*) 'from incompatible binary (collisions)';call err__("spec/os-spec-collisions.f03",4807)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(num_coll_cells, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"spec/os-spec-collisions.f03",4812)

  if( this%number_of_collision_cells .ne. num_coll_cells) then
    write(err_buf__,*) "Collision cells in restart data and input deck don't match";call err__("spec/os-spec-collisions.f03",4815)
    write(err_buf__,*) "Expected", this%number_of_collision_cells,"found",num_coll_cells;call err__("spec/os-spec-collisions.f03",4816)
    write(err_buf__,*) "This is non-recoverable";call err__("spec/os-spec-collisions.f03",4817)
    call abort_program(p_err_invalid)
  endif
...
!---------------------------------------------------
  ...
  ! allocate table
  call alloc(lookup_table_A, (/ p_lookup_size /),"spec/os-spec-collisions.f03",5040)
  ...
  write(err_buf__,*) "Linear lookup table not written.";call err__("spec/os-spec-collisions.f03",5047)
  write(err_buf__,*) "If you want to use that feature you'll have to write one.";call err__("spec/os-spec-collisions.f03",5048)
  call abort_program(p_err_notimplemented)
...
!---------------------------------------------------
  ...
  write(err_buf__,*) "Linear lookup table not written.";call err__("spec/os-spec-collisions.f03",5076)
  write(err_buf__,*) "If you want to use that feature you'll have to write one.";call err__("spec/os-spec-collisions.f03",5077)
  call abort_program(p_err_notimplemented)
...
!---------------------------------------------------
  ...
  ! Allocate table
  call alloc(lookup_table_A, (/p_lookup_size/),"spec/os-spec-collisions.f03",5103)
  call alloc(lookup_table_Ap, (/p_lookup_size/),"spec/os-spec-collisions.f03",5104)
...
!---------------------------------------------------
  ...
  ! Executable statements
  if (mpi_node()==0) print *,"Reading lookuptable: ", fname
  open(file_id_tem, file=fname, iostat = ierr, action = "read")
  if( ierr /= 0 ) then
    write(err_buf__,*) "Problem opening lookup table: ", fname;call err__("spec/os-spec-collisions.f03",5177)
    write(err_buf__,*) "Abort!";call err__("spec/os-spec-collisions.f03",5178)
    call abort_program()
  endif
  ...
  do i=1, size(tab)
    ...
    if( ierr /= 0 ) then
      write(err_buf__,*) "Problem reading value (", i, ") from lookup table: ", fname;call err__("spec/os-spec-collisions.f03",5185)
      write(err_buf__,*) "Abort!";call err__("spec/os-spec-collisions.f03",5186)
      call abort_program()
    endif
  enddo
  ...
  if( ierr /= 0 ) then
    write(err_buf__,*) "Problem closing lookup table: ", fname;call err__("spec/os-spec-collisions.f03",5193)
    write(err_buf__,*) "Abort!";call err__("spec/os-spec-collisions.f03",5194)
    call abort_program()
  endif
...
subroutine alloc_local_fluid_state( p, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_local_fluid_state ), pointer :: p
  ...
  addr = loc( p )
  bsize = sizeof( p )
  call log_allocation( addr, "t_local_fluid_state" , &
                        bsize, &
                        file, line )

end subroutine alloc_local_fluid_state



subroutine alloc_1d_local_fluid_state( p, n, file, line, stat )

  use iso_c_binding

  implicit none

  type( t_local_fluid_state ), dimension(:), pointer :: p
...
# 143 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof( p )
  call log_allocation( addr, "t_local_fluid_state" // '('// trim(strrange(1,n)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_1d_local_fluid_state




subroutine alloc_2d_local_fluid_state( p, n, file, line, stat )
...
# 236 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof( p )
  call log_allocation( addr, "t_local_fluid_state" // '('// trim(strrange(2,n)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_2d_local_fluid_state




subroutine alloc_bound_1d_local_fluid_state( p, lb, ub, file, line, stat )
...
# 329 "./memory/mem-template.h"
  ...
  addr = loc(p)
  bsize = sizeof(p)
  call log_allocation( addr, "t_local_fluid_state" // '('// trim(strrange(1,lb,ub)) // ')', &
                        bsize, &
                        file, line )

end subroutine alloc_bound_1d_local_fluid_state



subroutine freemem_local_fluid_state( p, file, line, stat )
  ...
  if ( associated(p) ) then
   ...
   call log_deallocation( addr, "t_local_fluid_state", &
                        bsize, file, line )
...
end subroutine freemem_local_fluid_state



subroutine freemem_1d_local_fluid_state( p, file, line, stat )
...
end subroutine freemem_1d_local_fluid_state




subroutine freemem_2d_local_fluid_state( p, file, line, stat )
...
end subroutine freemem_2d_local_fluid_state
# 5208 "spec/os-spec-collisions.f03" 2
