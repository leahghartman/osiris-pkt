# 1 "particles/os-particles-charge.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "particles/os-particles-charge.f90" 2
!-----------------------------------------------------------------------------------------
...
# 6 "particles/os-particles-charge.f90" 2
# 1 "./os-config.h" 1
...
# 7 "particles/os-particles-charge.f90" 2
...
subroutine get_charge_htc( particles, charge_htc )
  ...
  if ( particles%charge%n_prev_update /= particles%charge%n_last_update - 1 ) then
    write(err_buf__,*) 'Charge density for the previous iteration was not saved';call err__("particles/os-particles-charge.f90",116)
    call abort_program()
  endif
...
subroutine get_dcharge_dt( particles, dt, dcharge_dt )
  ...
  ! Ensure the previous charge density was properly calculated
  if ( particles%charge%n_prev_update /= particles%charge%n_last_update - 1 ) then
    write(err_buf__,*) 'Charge density for the previous iteration was not saved';call err__("particles/os-particles-charge.f90",165)
    call abort_program()
  endif
...
subroutine restart_write_charge_part( this, restart_handle )
  ...
  ! Write tag
  call restart_io_write("p_part_charge_rst_id", p_part_charge_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles-charge.f90",331)

  ! write local data to file
  call restart_io_write("this%n_last_update", this%n_last_update, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles-charge.f90",335)

  call restart_io_write("this%n_prev_update", this%n_prev_update, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles-charge.f90",338)
  ...
  call restart_io_write("curr_ptr", curr_ptr, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstwrt,"particles/os-particles-charge.f90",352)
...
subroutine setup_charge_part( this, keep_previous, restart, restart_handle )
  ...
  if ( restart ) then
   ...
   call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles-charge.f90",379)

   ! check if restart file is compatible
   if ( rst_id /= p_part_charge_rst_id) then
     write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("particles/os-particles-charge.f90",383)
     write(err_buf__,*) 'from incompatible binary (particles charge)';call err__("particles/os-particles-charge.f90",384)
     write(err_buf__,*) 'rst_id = ', rst_id;call err__("particles/os-particles-charge.f90",385)
     call abort_program(p_err_rstrd)
   endif

     call restart_io_read(this % n_last_update, restart_handle, ierr)
     call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles-charge.f90",390)

     call restart_io_read(this % n_prev_update, restart_handle, ierr)
     call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles-charge.f90",393)

     call this % charge0 % read_checkpoint( restart_handle )
     call this % charge1 % read_checkpoint( restart_handle )

     ! Associate pointers
     call restart_io_read(curr_ptr, restart_handle, ierr)
     call check_error(ierr,err_msg,p_err_rstrd,"particles/os-particles-charge.f90",400)
