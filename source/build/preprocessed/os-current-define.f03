# 1 "current/os-current-define.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "current/os-current-define.f03" 2
!#define __DEBUG__ 1
...
# 6 "current/os-current-define.f03" 2
...
# 7 "current/os-current-define.f03" 2
# 1 "./memory/memory.h" 1
...
# 19 "./memory/memory.h"
use memory
# 8 "current/os-current-define.f03" 2
...
type :: t_current
  ...
  contains
    ...
    procedure :: num_threads => num_threads_current
...
subroutine init_current( this, grid, gc_min, dx, no_co, if_move, &
  ...
  if ( restart ) then
    ...
    nt = this % num_threads( no_co )

    call alloc(this%pf, (/ nt /),"current/os-current-define.f03",212)
    ...
    do i = 1, nt
      !$omp critical
      call this%pf(i) % new( p_x_dim, p_f_dim, grid%my_nx(3,:), gc_num_new, dx, .true. )
      !$omp end critical
    enddo
    !$omp end parallel do
...
subroutine cleanup_current(this)
  ...
  if (associated(this%pf)) then
    ...
    enddo
    call freemem(this%pf,"current/os-current-define.f03",287)
  endif
...
subroutine normalize_cyl_current( this )
  ...
  ! call asrt__(this%coordinates == p_cylindrical_b,"this%coordinates == p_cylindrical_b","current/os-current-define.f03",365)
...
subroutine write_checkpoint_current( this, restart_handle )
  ...
  call restart_io_write("p_current_rst_id", p_current_rst_id, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",484)

  call restart_io_write("this%n_current_jay", this%n_current_jay, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",487)

  ! no need to write electric current data, only structure information
  call restart_io_write("this%pf(1)%x_dim_", this%pf(1)%x_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",491)

  call restart_io_write("this%pf(1)%f_dim_", this%pf(1)%f_dim_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",494)

  call restart_io_write("this%pf(1)%nx_", this%pf(1)%nx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",497)

  call restart_io_write("this%pf(1)%gc_num_", this%pf(1)%gc_num_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",500)

  call restart_io_write("this%pf(1)%dx_", this%pf(1)%dx_, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",503)
...
subroutine read_checkpoint_current( this, no_co, restart_handle )
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",533)

  ! check if restart file is compatible
  if ( rst_id /= p_current_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("current/os-current-define.f03",537)
    write(err_buf__,*) 'from incompatible binary (electric current)';call err__("current/os-current-define.f03",538)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(this%n_current_jay, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",543)

  ! read electric current parameters and create electric current structure
  call restart_io_read(lx_dim, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",547)

  call restart_io_read(f_dim, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",550)

  call restart_io_read(lnx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",553)

  call restart_io_read(lgc_num, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",556)

  call restart_io_read(ldx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"current/os-current-define.f03",559)

  ! setup of of the vdf object for the field
  nt = n_threads( no_co )
  call alloc(this%pf, (/ nt /),"current/os-current-define.f03",563)

  !$omp parallel do
  do i = 1, nt
    !$omp critical
    call this%pf(i) % new( lx_dim, f_dim, lnx, lgc_num, ldx, .true. )
    !$omp end critical
  enddo
  !$omp end parallel do
...
subroutine fill_data_current(this, tstep)
  ...
  if( n(tstep) /= this%n_current_jay) then
    write(err_buf__,*) "Error in fill_data_current.. see 'fill_data_current' is os-current-define.f03";call err__("current/os-current-define.f03",754)
    call abort_program(p_err_rstrd)
  endif
...
!---------------------------------------------------------------------------------------------------
! Get the number of threads to initialize current arrays (could be different if running on GPU)
!---------------------------------------------------------------------------------------------------
function num_threads_current( this, no_co )
  ...
  integer :: num_threads_current

  num_threads_current = n_threads( no_co )

end function num_threads_current
