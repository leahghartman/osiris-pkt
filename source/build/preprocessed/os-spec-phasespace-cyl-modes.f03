# 1 "cyl_modes/os-spec-phasespace-cyl-modes.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "cyl_modes/os-spec-phasespace-cyl-modes.f03" 2
...
# 2 "cyl_modes/os-spec-phasespace-cyl-modes.f03" 2
# 1 "./os-config.h" 1
...
# 3 "cyl_modes/os-spec-phasespace-cyl-modes.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "cyl_modes/os-spec-phasespace-cyl-modes.f03" 2
...
!-----------------------------------------------------------------------------------------
  ...
  do pos =1, ndims
    ...
    if ((x_or_p(pos) == p_phase_xc) .and. (xp_dim(pos) == 1)) then
      ...
      if (mpi_node()==0) print *,"   - NOTE: xc phasespace only available in dims 2 and 3,"
      if (mpi_node()==0) print *,"     xc1 defaulting to x1."
    endif
...
!-----------------------------------------------------------------------------------------
  ...
  case default
    if ( mpi_node() == 0 ) then
      write(0,*) "(*error*) Unsupported phasespace type ", "cyl_modes/os-spec-phasespace-cyl-modes.f03", ":", 613
...
!-----------------------------------------------------------------------------------------
  ...
  call asrt__(ndims>0,"ndims>0","cyl_modes/os-spec-phasespace-cyl-modes.f03",677)
  ...
  case default
    write(err_buf__,*) "Invalid phasespace type, or ";call err__("cyl_modes/os-spec-phasespace-cyl-modes.f03",738)
    write(err_buf__,*) "phasespace type not implemented";call err__("cyl_modes/os-spec-phasespace-cyl-modes.f03",739)
    call abort_program(p_err_invalid)
...
!-----------------------------------------------------------------------------------------
  ...
  case (p_phase_xc) ! x-cartesian
    ...
    case default
      write(err_buf__,*) "Should not get to this point, 1 not allowed";call err__("cyl_modes/os-spec-phasespace-cyl-modes.f03",778)
      call abort_program(p_err_invalid)
    end select
