...
subroutine read_input_sim( sim, input_file )
  ...
  if (mpi_node()==0) print *,'Reading zpulses ... '
  ...
  if (mpi_node()==0) print *,'Reading antenna configuration...'
...
subroutine init_sim( sim, grid_gc_min )
  ...
  call sim%part%init( sim%g_space, sim%jay, sim%emf, sim%grid, sim%no_co, sim%bnd, &
                      sim%zpulse_list, ndump(sim%tstep), restart, restart_handle, &
                      t(sim%time), sim%tstep, tmin(sim%time), tmax(sim%time), sim%options)
...
subroutine iter_sim( sim )
  ...
  call sim%part%delayed_injection( sim%jay, sim%emf, sim%grid, sim%no_co, sim%bnd, t(sim%time) )
  ...
  if ( sim % emf % if_charge_cons( sim%tstep ) ) then
    ...
    write(err_buf__,*) "Charge conservation diagnostic is temporarily disabled";call err__("os-simulation.f03",579)
    call abort_program()
  ...
  call sim % part % inject_cathode( sim%jay, t(sim%time), dt(sim%tstep), &
...
subroutine set_int_load_sim( sim, grid, n_ )
  ...
  if ( use_part_load( grid ) ) then
    call sim % part % add_load( grid, sim % emf, n_ )
  endif
