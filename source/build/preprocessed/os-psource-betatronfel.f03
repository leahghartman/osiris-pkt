# 1 "spec/psource/os-psource-betatronfel.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "spec/psource/os-psource-betatronfel.f03" 2
# 1 "./os-config.h" 1
...
# 2 "spec/psource/os-psource-betatronfel.f03" 2
...
# 3 "spec/psource/os-psource-betatronfel.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 7 "spec/psource/os-psource-betatronfel.f03" 2
...
!---------------------------------------------------------------------------------------------------
  ...
  select case ( p_x_dim )
     ...
     case (2) ! -----------------------------------------------------------
       ...
       if ( ierr/=0 ) then
          write(err_buf__,*) 'Allocation Failed!';call err__("spec/psource/os-psource-betatronfel.f03",197)
     ...
     case (3) ! -----------------------------------------------------------
       ...
       allocate( ppos(p_x_dim, ppcell), &
                 ...
                 pcharge(ppcell), stat = ierr)
       if ( ierr/=0 ) then
          write(err_buf__,*) 'Allocation Failed!';call err__("spec/psource/os-psource-betatronfel.f03",357)
          call abort_program( p_err_alloc )
       endif
  ...
  if ( ierr /= 0 ) then
    write(err_buf__,*) 'Deallocation failed';call err__("spec/psource/os-psource-betatronfel.f03",500)
