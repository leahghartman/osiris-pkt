# 1 "tiles/os-spec-comm-tiles.f90"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "tiles/os-spec-comm-tiles.f90" 2
!-----------------------------------------------------------------------------------------
...
# 6 "tiles/os-spec-comm-tiles.f90" 2
...
# 7 "tiles/os-spec-comm-tiles.f90" 2
...
# 19 "./memory/memory.h"
use memory
# 11 "tiles/os-spec-comm-tiles.f90" 2
...
subroutine loc_send_msg( msg, spec, par_idx, n_idx, shift )
  ...
  if ( max_n_part1 < 1 ) then
    write(err_buf__,*) "msg%max_buffer1_size is too small";call err__("tiles/os-spec-comm-tiles.f90",170)
    write(err_buf__,*) "please recompile with larger value";call err__("tiles/os-spec-comm-tiles.f90",171)
    call abort_program()
  endif
  ...
  if ( msg%size1 > msg%max_buffer1_size ) then
    write(err_buf__,*) " msg%size1 is too large, this should never happen";call err__("tiles/os-spec-comm-tiles.f90",188)
    call abort_program()
  endif
  ...
  if ( bsize < msg%size1 ) then
    call freemem(msg%buffer1,"tiles/os-spec-comm-tiles.f90",200)
    call alloc(msg%buffer1, (/ msg%size1 /),"tiles/os-spec-comm-tiles.f90",201)
  endif
  ...
  if ( msg%n_part2 > 0 ) then
    ...
    if ( bsize < msg%size2 ) then
      call freemem(msg%buffer2,"tiles/os-spec-comm-tiles.f90",227)
      call alloc(msg%buffer2, (/ msg%size2 /),"tiles/os-spec-comm-tiles.f90",228)
    endif
...
subroutine irecv_2_spec_group( recv_spec, garbage )
  ...
  if (ierr/=0) then
   write(err_buf__,*) "MPI error";call err__("tiles/os-spec-comm-tiles.f90",361)
  ...
  if ( recv_spec%size2 > 0 ) then
    ...
    if ( recv_spec%size2 > bsize ) then
      call freemem(recv_spec%buffer2,"tiles/os-spec-comm-tiles.f90",375)
      call alloc(recv_spec%buffer2, (/ recv_spec%size2 /),"tiles/os-spec-comm-tiles.f90",376)
    endif
    ...
    if (ierr/=0) then
       write(err_buf__,*) "MPI error";call err__("tiles/os-spec-comm-tiles.f90",383)
       call abort_program( p_err_mpi )
    endif
...
subroutine loc_recv_msg( msg )
  ...
  if (ierr/=0) then
   write(err_buf__,*) "MPI error";call err__("tiles/os-spec-comm-tiles.f90",441)
   call abort_program( p_err_mpi )
  endif
