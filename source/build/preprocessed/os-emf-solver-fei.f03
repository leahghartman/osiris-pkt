# 1 "emf/solver/os-emf-solver-fei.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "emf/solver/os-emf-solver-fei.f03" 2
!-----------------------------------------------------------------------------------------
...
# 6 "emf/solver/os-emf-solver-fei.f03" 2
...
# 7 "emf/solver/os-emf-solver-fei.f03" 2
...
# 19 "./memory/memory.h"
use memory
# 11 "emf/solver/os-emf-solver-fei.f03" 2
...
type, extends( t_emf_solver ), public :: t_emf_solver_fei
    ...
    real(p_double) :: ku, kl, dk, weight_w, dtdx1
...
subroutine cleanup_fei( this )
    ...
    if ( associated(this%f_buffer2) ) call freemem(this%f_buffer2,"emf/solver/os-emf-solver-fei.f03",1506)
    if ( associated(this%f_buffer3) ) call freemem(this%f_buffer3,"emf/solver/os-emf-solver-fei.f03",1507)
...
subroutine filter_correct_current_2d( this, jay )
    ...
    if ( .not. associated( this%f_buffer2 ) ) then
      !$omp critical
      call alloc(this%f_buffer2, (/ 3, fft_m1, m2 /),"emf/solver/os-emf-solver-fei.f03",1748)
      ...
      if ( bshape(2)/=fft_m1 .or. bshape(3)/=m2 ) then
        !$omp critical
        call freemem(this%f_buffer2,"emf/solver/os-emf-solver-fei.f03",1756)
        call alloc(this%f_buffer2, (/ 3, fft_m1, m2 /),"emf/solver/os-emf-solver-fei.f03",1757)
    ...
    call this % fft % fft( this%f_buffer2, m1, m2 )
    ...
    call this % fft % ifft( this%f_buffer2, m1, m2 )
...
subroutine filter_correct_current_3d( this, jay )
    ...
    if ( .not. associated( this%f_buffer3 ) ) then
      !$omp critical
      call alloc(this%f_buffer3, (/ 3, fft_m1, m2, m3 /),"emf/solver/os-emf-solver-fei.f03",1884)
      call this % fft % init( m1 )
      !$omp end critical
    else
      ! check if buffer should be resized (e.g., from dlb)
      ...
      if ( bshape(2)/=fft_m1 .or. bshape(3)/=m2 .or. bshape(4)/=m3 ) then
        !$omp critical
        call freemem(this%f_buffer3,"emf/solver/os-emf-solver-fei.f03",1892)
        call alloc(this%f_buffer3, (/ 3, fft_m1, m2, m3 /),"emf/solver/os-emf-solver-fei.f03",1893)
        ! re-init fft if necessary
        if ( bshape(2)/=fft_m1 ) then
          call this % fft % cleanup()
          call this % fft % init( m1 )
    ...
    call this % fft % fft( this%f_buffer3, m1, m2, m3 )
    ...
    call this % fft % ifft( this%f_buffer3, m1, m2, m3 )
...
subroutine min_gc_fei( this, min_gc )
    ...
    min_gc(p_lower,1) = max( 3*this%n_coef-2, this%n_damp_cell )
    min_gc(p_upper,1) = max( 3*this%n_coef-1, this%n_damp_cell )
...
subroutine init_fei( this, gix_pos, coords )
    ...
    integer :: i, j, n_taper
    integer :: taper_order_cell(this%taper_order/2), n_coef_cell(this%taper_order/2)
    ...
    if ( this%taper_bnd ) then
        ...
        ! Standard: solver_ord = 2*n_coef
        ! taper_ord = 2*n_coef
        ! taper_ord = 2*n_taper
        ! n_taper = n_coef
        ! solver_ord = taper_ord

        n_taper = this%taper_order/2
        allocate( this%taper_coef_b( n_taper, n_taper ) )
        allocate( this%taper_coef_e( n_taper, n_taper ) )

        ! set desired solver at the boundary to be 2nd-order Yee
        this%taper_coef_b(:,:) = 0.0
        this%taper_coef_e(:,:) = 0.0
        call std_coef( 2, this%taper_coef_b(:,1) )
        call std_coef( 2, this%taper_coef_e(:,1) )

        ! get the solver order at each cell, going from order 2 at cell 1 to order
        ! solver_ord at cell n_taper
        do i = 1, n_taper
            taper_order_cell(i) = 2*nint( ( (this%solver_ord-2)/(n_taper-1.0) * (i-1) + &
                                             2 ) / 2.0 )
            n_coef_cell(i) = nint( (this%n_coef-1)/(n_taper-1.0) * (i-1) + 1 )
        enddo

        select case ( this%type )
        case ( p_emf_fei_std )
            do i = 2, n_taper
                call std_coef( taper_order_cell(i), this%taper_coef_b(:,i) )
            enddo
        case ( p_emf_fei_bump )
            do i = 2, n_taper
                call bump_coef( taper_order_cell(i), n_coef_cell(i), this%kl, this%ku, &
                                this%dk, this%taper_coef_b(:,i) )
            enddo
        case ( p_emf_fei_xu )
            kernel_ptr => kernel_xu
            do i = 2, n_taper
                call gen_solver_coef( kernel_ptr, (/ this%dtdx1 /), taper_order_cell(i), &
                    n_coef_cell(i), this%taper_coef_b(:,i), this%weight_w, this%weight_n )
            enddo
        case ( p_emf_fei_dual )
            do i = 2, n_taper
                kernel_ptr => kernel_dual_e
                call gen_solver_coef( kernel_ptr, (/ this%dtdx1 /), taper_order_cell(i), &
                    n_coef_cell(i), this%taper_coef_e(:,i), this%weight_w, this%weight_n )
                kernel_ptr => kernel_dual_b
                call gen_solver_coef( kernel_ptr, (/ this%dtdx1 /), taper_order_cell(i), &
                    n_coef_cell(i), this%taper_coef_b(:,i), this%weight_w, this%weight_n )
            enddo
        case ( p_emf_fei_coef )
            ! solver coefficients linearly interpolate from specified solver to
            ! 2nd-order Yee
            do i = 2, n_taper
                this%taper_coef_b(1,i) = (this%coef_b(1)-this%taper_coef_b(1,1))*&
                                        (i-1)/(n_taper-1) + this%taper_coef_b(1,1)
                this%taper_coef_e(1,i) = (this%coef_e(1)-this%taper_coef_e(1,1))*&
                                        (i-1)/(n_taper-1) + this%taper_coef_e(1,1)
                do j = 2, i
                    this%taper_coef_b(j,i) = (this%coef_b(j)-this%taper_coef_b(j,1))*&
                                            (i-j+1)/(n_taper-j+1) + this%taper_coef_b(j,1)
                    this%taper_coef_e(j,i) = (this%coef_e(j)-this%taper_coef_e(j,1))*&
                                            (i-j+1)/(n_taper-j+1) + this%taper_coef_e(j,1)
                enddo
            enddo
        end select
        ...
        case ( p_emf_fei_std, p_emf_fei_bump, p_emf_fei_xu )
            this%taper_coef_e(:,:) = this%taper_coef_b(:,:)
        end select

        ! if ( mpi_node() == 0 ) then
        ! print *, "  Tapered solver stencil coefficients are:"
        ! select case ( this%type )
        ! case ( p_emf_fei_std, p_emf_fei_bump, p_emf_fei_xu )
        ! do i = 1, n_taper
        ! print *, "  C(", i, ") = ", this%taper_coef_e(:,i)
        ! enddo
        ! case ( p_emf_fei_dual, p_emf_fei_coef )
        ! do i = 1, n_taper
        ! print *, "  C_e(", i, ") = ", this%taper_coef_e(:,i)
        ! enddo
        ! do i = 1, n_taper
        ! print *, "  C_b(", i, ") = ", this%taper_coef_b(:,i)
        ! enddo
        ! end select
        ! endif
...
subroutine read_input_fei( this, input_file, dx, dt )
    ...
    case ( "standard" )
        ...
        if (disp_out(input_file)) then
            if (mpi_node()==0) print *,"- using Fei solver: standard type"
        endif
        n_coef = solver_ord/2
    ...
    case ( "bump" )
        ...
        if (disp_out(input_file)) then
            if (mpi_node()==0) print *,"- using Fei solver: bump type"
        endif
    ...
    case ( "xu" )
        ...
        if (disp_out(input_file)) then
            if (mpi_node()==0) print *,"- using Fei solver: Xu type"
        endif
        kernel_ptr => kernel_xu
    ...
    case ( "dual" )
        ...
        if (disp_out(input_file)) then
            if (mpi_node()==0) print *,"- using Fei solver: dual type"
        endif
    ...
    case ( "customized-coef" )
        ...
        if (disp_out(input_file)) then
            if (mpi_node()==0) print *,"- using Fei solver: customized-coefficient type"
        endif
    ...
    ! This used to be an input parameter, but is now determined automatically
    this%taper_order = 2*n_coef
