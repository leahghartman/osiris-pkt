# 1 "random/os-random.f03"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 467 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "random/os-random.f03" 2
# 1 "./os-config.h" 1
...
subroutine read_checkpoint_rnd( restart_handle, class_idx, enforce_rng_constancy )
  ...
  call check_error(ierr,err_msg,p_err_rstrd,"random/os-random.f03",233)

  ! check if restart file is compatible
  if ( rst_id /= p_rand_rst_id) then
    write(err_buf__,*) 'Corrupted restart file, or restart file ';call err__("random/os-random.f03",237)
    write(err_buf__,*) 'from incompatible binary (random number generator)';call err__("random/os-random.f03",238)
    call abort_program(p_err_rstrd)
  endif

  call restart_io_read(class_idx, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"random/os-random.f03",243)

  call restart_io_read(enforce_rng_constancy, restart_handle, ierr)
  call check_error(ierr,err_msg,p_err_rstrd,"random/os-random.f03",246)
...
subroutine write_checkpoint_rnd( this, restart_handle )
    ...
    call check_error(ierr,err_msg,p_err_rstwrt,"random/os-random.f03",268)

    ! write integer identifying the prng type
    class_idx = random_type( this )
    call restart_io_write("class_idx", class_idx, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"random/os-random.f03",273)

    call restart_io_write("this%enforce_random_constancy", this%enforce_random_constancy, restart_handle, ierr)
    call check_error(ierr,err_msg,p_err_rstwrt,"random/os-random.f03",276)
