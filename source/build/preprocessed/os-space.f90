...
! executable statements
          ...
          call restart_io_write("p_space_rst_id", p_space_rst_id, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",367)
          ...
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",374)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%x_bnd", this%x_bnd, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",379)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%x_bnd_old", this%x_bnd_old, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",384)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%move_num", this%move_num, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",389)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%xmoved", this%xmoved, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",394)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%if_move", this%if_move, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",399)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%move_vel", this%move_vel, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",404)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%x_bnd_initial", this%x_bnd_initial, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",409)
            call abort_program(p_err_rstwrt)
          endif
          call restart_io_write("this%total_i_moved", this%total_i_moved, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error writing restart data for space object.";call err__("os-space.f90",414)
            call abort_program(p_err_rstwrt)
          endif
...
! executable statements
    ...
    if ( ierr/=0 ) then
    write(err_buf__,*) 'error reading restart data for space object.';call err__("os-space.f90",447)
    call abort_program(p_err_rstrd)
    endif

    if ( rst_id /= p_space_rst_id) then
write(err_buf__,*) 'Corrupted restart file, or restart file';call err__("os-space.f90",452)
write(err_buf__,*) 'from incompatible binary (space)';call err__("os-space.f90",453)
   call abort_program(p_err_rstrd)
    endif

          call restart_io_read(this%x_dim, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",459)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%x_bnd, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",464)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%x_bnd_old, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",469)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%move_num, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",474)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%xmoved, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",479)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%if_move, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",484)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%move_vel, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",489)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%x_bnd_initial, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",494)
            call abort_program(p_err_rstrd)
          endif
          call restart_io_read(this%total_i_moved, restart_handle, ierr)
          if ( ierr/=0 ) then
            write(err_buf__,*) "error reading restart data for space object.";call err__("os-space.f90",499)
            call abort_program(p_err_rstrd)
          endif
