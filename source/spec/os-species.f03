...
subroutine read_input_species( this, input_file, def_name, periodic, if_move, grid, &
  ...
  select case ( trim( push_type ) )
    ...
    case('gravity')
      ...
      if (disp_out(input_file)) then
        SCR_ROOT("  ::::::::::::::::::::::::::")
        SCR_ROOT("  Using gravity")
        SCR_ROOT("  gravity = ", this%gravity)
        SCR_ROOT("  ::::::::::::::::::::::::::")
      endif
...
!-----------------------------------------------------------------------------------------
  ...
  restart_io_wr( this%if_use_delayed_injection, restart_handle, ierr )
  CHECK_ERROR( ierr, err_msg, p_err_rstwrt )

  restart_io_wr( this%delayed_injection_time, restart_handle, ierr )
  CHECK_ERROR( ierr, err_msg, p_err_rstwrt )
...
!-----------------------------------------------------------------------------------------
  ...
  restart_io_rd( this%if_use_delayed_injection, restart_handle, ierr )
  CHECK_ERROR( ierr, err_msg, p_err_rstwrt )

  restart_io_rd( this%delayed_injection_time, restart_handle, ierr )
  CHECK_ERROR( ierr, err_msg, p_err_rstwrt )
