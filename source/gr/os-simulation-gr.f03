...
subroutine allocate_objs_gr( sim )
  ...
  SCR_ROOT('Allocating gr geometry, emf and particle objects.')
...
subroutine init_gr( sim, grid_gc_min )
  ...
  select type( emf => sim%emf )
    ...
    class default
      ERROR('pointer to geometry must be set with a t_emf_gr object')
  ...
  select type( part => sim%part )
    ...
    class default
      ERROR('pointer to geometry must be set with a t_particles_gr object')
      call abort_program( p_err_invalid )
  end select
...
subroutine read_input_gr( sim, input_file )
  ...
  SCR_ROOT('Reading geometry configuration...')
