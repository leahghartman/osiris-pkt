...
!-----------------------------------------------------------------------------------------
  ...
  ! set parameters for initial spin dist
  select case ( trim( sdist_init ) )
  case ("bidirection")
  
    this%sdist_type = p_bidir
  
  case ("laser-phase")
    
    this%sdist_type = p_laser_phase
    
    this%omega0     = omega0
    this%per_center = per_center
    this%per_w0     = per_w0
    this%per_focus  = per_focus
    this%zr         = 0.5_p_double * omega0 * per_w0**2

  case default
    if ( mpi_node() == 0 ) then
      write(0,*) "   Error reading spin distribution parameters"
      write(0,*) "   invalid sdist_init, valid values are 'bidirection' and 'laser-phase'"
      write(0,*) "   aborting..."
    endif
    stop
  end select
  ...
  subroutine set_ion_parameters_tdse(rate_param, neut_param)
    ...
    call alloc( this%rate_param, (/3, tab_length/))
...
!-----------------------------------------------------------------------------------------
  ...
  case (1)
     ...
     ERROR('tdse_field_rates_1d not implemented yet')
...
!-----------------------------------------------------------------------------------------
  ...
  case ( p_tdse )
    ...
    case (1)
      ...
      ERROR('advance_density_1d not implemented yet')
      call abort_program(p_err_notimplemented)
    case (2)
...
!-----------------------------------------------------------------------------------------
  ...
  case ( p_tdse )
    ...
    case (1)
      ...
      ERROR( 'inject_particles_1d not implemented yet.' )
      call abort_program(p_err_notimplemented)
    case (2)
...
!-----------------------------------------------------------------------------------------
  ...
  #ifdef __HAS_SPIN__
  real(p_k_part), dimension(p_s_dim) :: snewpart
  #endif
  ...
  do j=1, this%multi_ion% nx_(2)
    ...
    if( coordinates == p_cylindrical_b ) then
      ...
      ERROR( 'TDSE ionization for cylindrical coordinates not implemented yet' )
      call abort_program(p_err_notimplemented)
        ...
        if ( this%multi_ion%f2(ion_idx,i,j) > this%multi_min )  then
          ...
          if (den_center > this%den_min) then
            ...
            do ix = 0, n_p_add-1
              ...
              ! calculate the spin sign
              sign = spin_sign( this%w%f2(1,i,j), this%w%f2(2,i,j), ixnewpart )

              select case ( this%sdist_type )

              case ( p_bidir )
                
                snewpart(1) = sign
                snewpart(2) = 0.0_p_double
                snewpart(3) = 0.0_p_double

              case ( p_laser_phase )

                ! ppos is the relative position to focal point
                ppos(1) = g_xmin(1) + dx(1) * ( real((ixnewpart(1) + my_nx_p(1) - 2), p_double) + xnewpart(1) )
                ppos(2) = g_xmin(2) + dx(2) * ( real((ixnewpart(2) + my_nx_p(2) - 2), p_double) + xnewpart(2) )

                ppos(1) = ppos(1) - this%per_focus
                ppos(2) = ppos(2) - this%per_center(1)

                call get_sdist_laser_2d( this%omega0, this%zr, ppos, snewpart )
                snewpart = sign * snewpart
        ...
        if ( this%multi_ion%f3(ion_idx,i,j,k) > this%multi_min )  then
          ...
          if (den_center > this%den_min) then
            ...
            do ix = 0, n_p_add-1
              ...
              ! calculate the spin sign
              sign = spin_sign( this%w%f3(1,i,j,k), this%w%f3(2,i,j,k), ixnewpart )
              ...
              case ( p_laser_phase )
                ...
                ppos(1) = g_xmin(1) + dx(1) * ( real((ixnewpart(1) + my_nx_p(1) - 2), p_double) + xnewpart(1) )
                ppos(2) = g_xmin(2) + dx(2) * ( real((ixnewpart(2) + my_nx_p(2) - 2), p_double) + xnewpart(2) )
                ppos(3) = g_xmin(3) + dx(3) * ( real((ixnewpart(3) + my_nx_p(3) - 2), p_double) + xnewpart(3) )

                ppos(1) = ppos(1) - this%per_focus
                ppos(2) = ppos(2) - this%per_center(1)
                ppos(3) = ppos(3) - this%per_center(2)

                call get_sdist_laser_3d( this%omega0, this%zr, ppos, snewpart )
                snewpart = sign * snewpart
...
function spin_sign( prob_up, prob_down, ix )

  implicit none

  real(p_double), intent(in) :: prob_up, prob_down
  integer, intent(in), dimension(:), optional :: ix

  real(p_double) :: spin_sign, rnd, prob

  prob = prob_up + prob_down
  if ( prob == 0.0_p_double ) then
    spin_sign = 1.0_p_double
  else
    prob = prob_up / prob
    call rng % harvest_real3( rnd, ix )
    spin_sign = merge( 1.0_p_double, -1.0_p_double, rnd < prob )
  endif

end function spin_sign

subroutine get_sdist_laser_2d( omega0, zr, x, s )

  implicit none

  real(p_double), intent(in) :: omega0, zr
  real(p_k_part), intent(in), dimension(:) :: x
  real(p_k_part), intent(inout), dimension(:) :: s

  real(p_double) :: zr2, z2, ptan, pcos, psin, tmp

  zr2 = zr*zr
  z2 = x(1)*x(1)
  tmp = zr2 + z2
  tmp = omega0 * ( tmp - 0.5_p_double*x(2)*x(2) * (z2-zr2) / tmp ) - zr
  ptan = omega0 * x(1) * x(2) / tmp ! tangent

  pcos = sqrt( 1.0_p_double / ( 1.0 + ptan*ptan ) )
  psin = pcos * ptan
  s(1) = real( pcos, p_k_part )
  s(2) = real( psin, p_k_part )
  s(3) = 0.0_p_k_part

end subroutine get_sdist_laser_2d

subroutine get_sdist_laser_3d( omega0, zr, x, s )
  ...
  real(p_double) :: zr2, z2, r, ptan, pcos, psin, tmp

  zr2 = zr*zr
  z2 = x(1)*x(1)
  r = sqrt( x(2)*x(2) + x(3)*x(3) )
  tmp = zr2 + z2
  tmp = omega0 * ( tmp - 0.5_p_double*r*r * (z2-zr2) / tmp ) - zr
  ptan = omega0 * x(1) * r / tmp ! tangent

  pcos = sqrt( 1.0_p_double / ( 1.0 + ptan*ptan ) )
  psin = pcos * ptan
  s(1) = real( pcos, p_k_part )
  s(2) = real( x(2) / r * psin, p_k_part )
  s(3) = real( x(3) / r * psin, p_k_part )

end subroutine get_sdist_laser_3d
...
!-----------------------------------------------------------------------------------------
  ...
  restart_io_wr( p_neutral_spin_rst_id, restart_handle, ierr )
